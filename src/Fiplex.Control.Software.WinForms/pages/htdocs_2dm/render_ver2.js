...
var chRfIn_settings_adj = [{min: -100, low_alarm: -121, low_warn: -121, high_warn: 0, high_alarm: 0, max: 0 }, 
		{min:  -80, low_alarm: -121, low_warn: -121, high_warn: -20, high_alarm: -20, max: -20 }];
var chRfInRemote_settings = [{min: -110, low_alarm: -121, low_warn: -121, high_warn: 0, high_alarm: 0, max: 0 }, 
		       {min:  -50, low_alarm: -60, low_warn: -120, high_warn: 10, high_alarm: 10, max: 10 }];
var chRfOutRemote_settings = [{min:  -50, low_alarm: -120, low_warn: -60, high_warn: 10, high_alarm: 10, max: 10 }, 
		       {min:   0, low_alarm: -128, low_warn:  -128, high_warn: 37, high_alarm: 40, max: 45 }];			   
function Page() {
	this.show = function(tags, fact, version, serNr, config, NFPAcfg) {
		...
		self.FilterValidSep = fact.commomUl? FilterValidSeparation[1]:FilterValidSeparation[0];
		self.maxRemotes = self.config.MAXREMOTES;
		self.lnaEnabledEdited = [];
		self.paEnabledEdited = [];
		self.rfOnOffEdited = [];
		for (var k=0;k<self.maxRemotes;k++){
			self.rfOnOffEdited.push(false);
			self.lnaEnabledEdited.push([]);
			self.paEnabledEdited.push([]);
			for (var band=0;band<2;band++){
				self.lnaEnabledEdited[k].push(false);
				self.paEnabledEdited[k].push(false);
			}
		}			
	...
	this.createUnit = function() {
		...
		headDiv = this.createBandHead("REMOTE DEVICES");
		unitDiv.appendChild(headDiv);
		contentDiv = document.createElement("div");
		unitDiv.appendChild(contentDiv);
		contentDiv.id = "contentDivRemotes";
		tab = this.createRemotesEnable();
		contentDiv.appendChild(tab);
		unitDiv.appendChild(self.createWarnRemote());
		unitDiv.appendChild(self.createWarnRemoteBandUnmatched());
		//REMOTE INTERFACE
		for (var r=0;r<self.maxRemotes;r++){
			var headDiv = this.createRemoteHead(r+1,self.tags.tagDevices[r+1]);
			headDiv.id = "headDivRemote_"+r;
			if (r>=self.maxRemotesToShow) headDiv.style.display="none";
			unitDiv.appendChild(headDiv);
			var contentDiv = document.createElement("div");
			unitDiv.appendChild(contentDiv);
			contentDiv.className = "contentbox";
			contentDiv.id = "contentDivRemote_"+r;
			if (r>=self.maxRemotesToShow) contentDiv.style.display="none";
			var tab = this.createContentRemoteCtrl(r);
			contentDiv.appendChild(tab);
		}
		return unitDiv;
	}
	...
	this.createRemoteHead = function(nr,tag) {
		var box = document.createElement("div");
		box.className = "headbox";
		var tab = document.createElement("table");
		box.appendChild(tab);
	...
	this.showTag = function() {
		try {
			for (var k=0;k<self.tags.MAXDEVICES;k++){
				...
				var name = entify(self.tags.tagDevices[k]);
				el.innerHTML = name.trim();
			}
		} catch (err) {}
	}
	this.showRemote = function(n, show) {
		if (n>=self.maxRemotesToShow) show=false;
		var id = "contentDivRemote_"+n;
		var contentDiv = document.getElementById(id);
		var id = "headDivRemote_"+n;
		var headDiv = document.getElementById(id);
		try {
			contentDiv.style.display = show ? "block" : "none";
			headDiv.style.display = show ? "block" : "none";
	...
	this.showContentRemote = function(n, show) {
		if (n>=self.maxRemotesToShow) show=false;
		var id = "contentDivRemote_"+n;
	...
	this.showTagRemote = function(r, forcedTag) {
		try {
			var k = r+1;
			var el = document.getElementById("tagName_"+k);
			if(typeof String.prototype.trim !== 'function'){
			...
			var name;
			if ( typeof(forcedTag) != "undefined" && forcedTag != null ) {
				name = forcedTag;
			} else {
				name = entify(self.tags.tagDevices[k]);
			}
			el.innerHTML = name.trim();
		} catch (err) {}
	}
	this.showTagBlocked = function() {
		try {
			var el = document.getElementById("tagName_0");
			el.innerHTML = "BLOCKED";
		} catch (err) {}
	}
	this.createRemotesEnable = function() {
		var tab = document.createElement("table");
		tab.className = "contenttable";
		var rowb = document.createElement("tr");
		tab.appendChild(rowb);
		for (var n = 0; n < self.maxRemotes; n++) {
			var cell = document.createElement("td");
			cell.className = "contentcell";
			cell.id = "remoteEnableBox_"+n;
			cell.style.paddingRight = cell.style.paddingLeft = "10px";
			rowb.appendChild(cell);
			cell.appendChild(this.createRemoteEnableBox(n));
			if (n>=self.maxRemotesToShow) cell.style.display="none";
		}
		return tab;
	}
	this.createRemoteEnableBox = function(n) {
		...
		row.appendChild(cell);
		cell.innerHTML = "REMOTE&nbsp;"+(n+1);
		cell.className = "thdrht";
		cell = document.createElement("td");
		row.appendChild(cell);
		var el = document.createElement("input");
		el.type = "checkbox";
		el.id = "remoteEnable_"+n;
		el.name = el.id;
		cell.style.paddingLeft=cell.style.paddingRight="10px";
		cell.appendChild(el);
		cell = createLedBox("remoteDisconnectAlarm_"+n);
		cell.style.paddingLeft="10px";
		row.appendChild(cell);
		return box;
	}
	this.getRemoteEnabled = function(n) {
		if (n>=self.maxRemotesToShow) return false;
		var id = "remoteEnable_"+n;
		var el = document.getElementById(id);
		try {
			return el.checked;
		} catch(e) {return false;}
	}
	this.setRemoteEnabled = function(n, enable) {
		var id = "remoteEnable_"+n;
		var el = document.getElementById(id);
		try {
			if (enable) {
			...
			// ocultar remoto
			self.showRemote(n, enable);

		} catch(e) {}
	}
	this.setRemoteDisconnectLed = function(n, alarm, enabled) {
		var id = "remoteDisconnectAlarm_"+n;
		var color;
		if ( !enabled ) {
			color = "grey";
		} else {
	...
	this.createAlarmInterface = function(){
		...
		var cell = this.createAlarmTableRemote();
		rowb.appendChild(cell);
		cell.className = "contentcell";
		cell.rowSpan = rspan;
	...
	this.createAlarmTableRemote = function(){ 
		var cellb = document.createElement("td");
		var tbl = document.createElement("table");
		tbl.style.marginRight = "6px";
		tbl.style.minWidth = "140px";
		...
		tb.appendChild(row);
		var cell = document.createElement("th");
		cell.innerHTML = "REMOTE&nbsp;ALARMS";
		cell.className = "cth";
		cell.colSpan = 2;
		row.appendChild(cell);
		for (var k=0;k<self.NFPA.remote[0].NUMALARM;k++){
			var idx = self.NFPA.remote[0].naturalOrder[k];
			var enable=false;
			for (var j=0;j<self.NFPA.MAXREMOTES;j++){
				if (self.config.connectedRemotes[j] && self.NFPA.remote[j].alarmEnabled[idx]) enable=true;
			}
			var name = self.NFPA.remote[0].alarmNames[idx];
			if (self.factory.commomUl && (idx==1 || idx==4 || idx==8 || idx==9)){
				name = name.replace("B0","");
				if (name.search("B1")>=0) enable = false;
			}else{
				name = name.replace("B0",factory.bandNames[0]);
				name = name.replace("B1",factory.bandNames[1]);
			}
			if (self.NFPA.remote[0].alarmNames[idx].search("Undefined")>=0) enable = false;			
			tb.appendChild(this.createGralRemoteAlarm(idx,name,enable));
		}	
		return cellb;
	}
	this.createAlarmTable = function(){
		...
		tb.appendChild(row);
		var cell = document.createElement("th");
		cell.innerHTML = "MASTER&nbsp;ALARMS";
		cell.className = "cth";
		cell.colSpan = 2;
		row.appendChild(cell);
		for (var k=0;k<self.NFPA.alarmEnabled.length;k++){
			var p = self.NFPA.naturalOrder[k];
			var enable = self.NFPA.alarmEnabled[p];
			var name = self.NFPA.alarmNames[p];
			if (self.factory.commomUl && (p>=6 && p<=9)){
				name = name.replace("B0","");
				if (name.search("B1")>=0) enable = false;
			}else{
				name = name.replace("B0",factory.bandNames[0]);
			...
			if (self.NFPA.alarmNames[p]=="Undefined") enable = false;
			if (p==14) enable = false;  // remote disconnect
			tb.appendChild(this.createGralAlarm(p,name,enable));
		}	
		return cellb;
	}
	this.createRelayTable = function(){
		...
		tb.appendChild(row);
		var cell = document.createElement("th");
		cell.innerHTML = "MASTER&nbsp;RELAY&nbsp;STATUS";
		cell.className = "cth";
		cell.colSpan = 9;
		row.appendChild(cell);
		var row = document.createElement("tr");
	...
	this.createContentRemoteCtrl = function(nr){
		var tab = document.createElement("table");
		tab.style.width = "100%";
		tab.className = "contenttable";
		var row = document.createElement("tr");
		...
		row.appendChild(cell);
		cell.className = "contentcell";
		var el = this.createContentRemoteGralCtrl(nr);
		cell.appendChild(el);
		var row = document.createElement("tr");
		tab.appendChild(row);
		var cell = document.createElement("td");
		...
		cell.className = "contentcell";
		cell.style.textAlign = "center";
		var el = this.createContentRemoteBandCtrl(nr);
		cell.appendChild(el);

		return tab;
	}
	this.createContentRemoteBandCtrl = function(nr){
		var mainTbl = document.createElement("table");
		mainTbl.style.marginTop = "2px";
		mainTbl.style.marginBottom = "2px";		
		mainTbl.style.width = "95%";
		...
		for (var i = 0; i < 2; ++i) {
			var cell = document.createElement("td");
			cell.appendChild(this.createContentRemoteUlDlCtrl(nr,i));
			row.appendChild(cell);
		}
		return mainTbl;
	}
	this.isRemoteBandMatched = function(nr) {
		/*self.config.remote[nr].band == (0 -> V/U , 1 -> 700/800)  */
		return ( ((self.config.remote[nr].band == 0) && !self.factory.commomUl) 
			|| ((self.config.remote[nr].band == 1) && self.factory.commomUl) );
	}
	this.isRemoteBandMatchedStatus = function(monitor, nr) {
		/*self.config.remote[nr].band == (0 -> V/U , 1 -> 700/800)  */
		return ( ((monitor.remote[nr].band == 0) && !self.factory.commomUl) 
			|| ((monitor.remote[nr].band == 1) && self.factory.commomUl) );
	}
	this.createContentRemoteUlDlCtrl = function(nr,uldl) {
		var tab = document.createElement("table");
		tab.className = "bt";
		tab.style.padding = "1px 1px 1px 1px";
		var tb = document.createElement("tbody");
		tab.appendChild(tb);
		var row = document.createElement("tr");
		tb.appendChild(row);
		...
		tb.appendChild(row);
		this.createRemoteUlDlCtrlHeader(row,nr,uldl);
		...
		var dual = b[0] && b[1];
		var commomUl = (dual && self.factory.commomUl && uldl==0);
		if (self.factory.commomUl && uldl == 0) {
				show[0] = true;
				show[1] = false;
		} 
		/* Cambiar nombres de banda de remoto si no coinciden las bandas con el master,
		 * es decir, remoto VHF/UHF en master 700/800 o viceversa */
		var bandNames = [];
		if (self.isRemoteBandMatched(nr)) {
			/* si banda de remoto coincide con master, mantener los nombres de banda de factory */
			for (var band = 0; band < 2; band++) {
				bandNames[band] =self.factory.bandNames[band];
			}
		} else {
			/* si banda de remoto no coincide con master, usar nombre de banda fijo */
			if (self.config.remote[nr].band == 0) {
				/* remoto V/U */
				bandNames[0] = "VHF";
				bandNames[1] = "UHF";
			} else {
		...
		for (var band = 0; band < 2; band++) {
			row = document.createElement("tr");
			row.style.height = (commomUl || !dual)?"46px":"22px";
			row.style.display = show[band]?"table-row":"none";
			row.id = "filterRow_"+nr+"_"+band+"_"+uldl;
			tb.appendChild(row);
			var bname = bandNames[band];
			if (!b[0] && b[1]) bname = bandNames[1];//si es single band PS800 se fuerza name PS800
			if (commomUl) bname = bandNames[0]+ "<br>+<br>" +bandNames[1];
			this.createRemoteUlDlInfoBands(row, band, nr, uldl, bname);
		}
		return tab;
	}
	this.createRemoteUlDlInfoBands = function(row,band,nr,uldl,bandName){
		var td = document.createElement("td");
		td.id = "remoteBandNameCell_"+nr+"_"+band+"_"+uldl;
		td.innerHTML = bandName;
		if (uldl==1) td.style.paddingLeft = "50px";
		td.style.color = (self.isRemoteBandMatched(nr)?"black":"red");
		td.style.fontWeight = "bold";
		td.style.textAlign = "center";
		row.appendChild(td);		
		if (uldl==0){
			row.appendChild(this.createAttRemote(nr, band));
		}
		// OCULTAR PowerIN UL/DL de remotos
		var el_met = this.createMetPowInRemote(nr, band, uldl);
		var el_txt = this.createTextPowInRemote(nr, band, uldl);
		//if (uldl == 1) {
			el_met.style.display = "none";
			el_txt.style.display = "none";
		//}
		row.appendChild(el_met);
		row.appendChild(el_txt);
		
		var el_met = this.createMetPowOutRemote(nr, band, uldl);
		var el_txt = this.createTextPowOutRemote(nr, band, uldl);
		// OCULTAR PowerOUT UL de remotos
		if (uldl == 0) {
			el_met.style.display = "none";
			el_txt.style.display = "none";
		}
		...
		// OCULTAR AGC sólo DL de remotos
		el_met = this.createMetAgcRemote(nr, band, uldl);
		el_txt = this.createTextAgcRemote(nr, band, uldl);
		if (uldl == 1) {
			el_met.style.display = "none";
			el_txt.style.display = "none";
		}
		...
		// solamente DL, en UL se sustituye por fiber optical loss
		if (uldl==1) {
			var td = this.createRemotePowerLimit(nr, band, uldl);
		...
		row.appendChild(createLedBox("remoteAlarm_"+nr+"_"+(1+3*band+uldl)));
		var alIndex = [[8,0],[9,3]];
		var el = createLedBox("remoteAlarm_"+nr+"_"+alIndex[band][uldl]);
		// //ocultar overload de remoto
		// if (uldl==0)
		// 	el.style.display = "none";
		row.appendChild(el);
	}
	this.createRemoteUlDlCtrlHeader = function(row,nr,uldl){
		...
		if (uldl==0) {
			...
			// OCULTAR ATENUADOR DE REMOTO
			td.style.display = "none";
			td.className = "thdcnt";
			td.innerHTML = "Att(dB)";
			td.style.minWidth = "37px";
			row.appendChild(td);
		}
		td = document.createElement("td");
		// OCULTAR PowerIN UL/DL de remotos
		...
		td = document.createElement("td");
		td.className = "thdcnt";
		td.innerHTML = uldl==0?"LNA&nbsp;Fail":"PA&nbsp;Fail";
		row.appendChild(td);		
		
		td = document.createElement("td");
		td.className = "thdcnt";
		td.innerHTML = uldl==0?"Overload":"VSWR";
		// NO Ocultar overload
		// if (uldl == 0) {
		// 	td.style.display = "none";
		// }
	...
	this.createContentRemoteGralCtrl = function(nr) {
		var box = document.createElement("div");
		var tab = document.createElement("table");
		...
		var cell = document.createElement("th");
		cell.className = "thdcnt";
		cell.innerHTML = "RF&nbsp;Enable";
		row.appendChild(cell);
		cell = self.createRemoteRfOnOff(nr);
		row.appendChild(cell);
		var cell = document.createElement("th");
		cell.className = "thdcnt";
		cell.innerHTML = "UL Fiber Optical Loss(dBo)";
		row.appendChild(cell);
		cell = self.createRemoteFiberOpticalLoss(nr);
		row.appendChild(cell);

		var alarmGralRemote = [7,10,11,12,13,6];
		for (var k=0;k<alarmGralRemote.length;k++){
			var cell = document.createElement("th");
			row.appendChild(cell);
			cell.className = "thdcnt";
			cell.innerHTML = self.NFPA.remote[0].alarmNames[alarmGralRemote[k]];
			cell.id = "remoteAlarmLabel_"+nr+"_"+alarmGralRemote[k];
			// if (alarmGralRemote[k]==6) cell.style.display = "none"; //se oculta temperatura
			var cell = document.createElement("td");
			cell = createLedBox("remoteAlarm_"+nr+"_"+alarmGralRemote[k]);
			cell.id = "remoteAlarmLed_"+nr+"_"+alarmGralRemote[k];
		...
		var cell = document.createElement("td");
		row.appendChild(cell);
		var el = this.createTempBoardRemote(nr);
		cell.appendChild(el);
		cell.className = "thdcnt";
		// cell.style.display = "none"; //se oculta temperatura
		return box;
	}
	this.createRemoteFiberOpticalLoss = function(nr) {
		// sólo hay un control, pero que escribe band0/band1 simultáneamente
		var cell = document.createElement("td");
		var el = document.createElement("input");
		cell.appendChild(el);
		...
		el.id = "remoteFiberOpticalLoss_"+nr;
		el.name = el.id;
		el.type = "text";
		el.style.width = "30px";
		el.className = "number";
		el.title = "Min: "+self.config.remoteFiberOpticalLossRange.MIN
			+", Max: "+ self.config.remoteFiberOpticalLossRange.MAX +" dBo";
		el.onkeypress = function(ev) {
		...
		el.onchange = function(ev) {
			...
			var min = self.config.remoteFiberOpticalLossRange.MIN;
			var max = self.config.remoteFiberOpticalLossRange.MAX;
			var step = self.config.remoteFiberOpticalLossRange.STEP;
			var v;
			if (num < min) {
				v = min;
			} else if (num > max) {
				...
				v = Math.round(num/step + 0.0001)*step;
			}
	...
	this.setRemoteFiberOpticalLoss = function(nr, num) {
		var min = self.config.remoteFiberOpticalLossRange.MIN;
		var max = self.config.remoteFiberOpticalLossRange.MAX;
		var step = self.config.remoteFiberOpticalLossRange.STEP;
		if (isNaN(num)) {
		...
		var id = "remoteFiberOpticalLoss_"+nr;
		try {
			var el = document.getElementById(id);
			el.value = v.toFixed(1);
		} catch(e) {}
	}
	this.getRemoteFiberOpticalLoss = function(nr) {
		var min = self.config.remoteFiberOpticalLossRange.MIN;
		var max = self.config.remoteFiberOpticalLossRange.MAX;
		var step = self.config.remoteFiberOpticalLossRange.STEP;
		var id = "remoteFiberOpticalLoss_"+nr;
		...
		if (num < min) {
			...
			v = Math.round(num/step + 0.00001)*step;
		}
		return v;
	}
	this.computePowerFromOpticalLoss = function(loss) {
		var min = self.config.remoteFiberOpticalLossRange.MIN;
		var max = self.config.remoteFiberOpticalLossRange.MAX;
		var step = self.config.remoteFiberOpticalLossRange.STEP;
		if (loss < min) {
		...
		loss = Math.round(loss/step + 0.00001)*step;
		var powerMax = self.config.powerULB0Range.MAX;
		var powerMin = self.config.powerULB0Range.MIN;
		var slope = (powerMax - powerMin) / (max - min);
		var power = powerMin + (loss - min) * slope;
		return power;
	}
	this.computeOpticalLossFromPower = function(power) {
		var min = self.config.remoteFiberOpticalLossRange.MIN;
		var max = self.config.remoteFiberOpticalLossRange.MAX;
		var step = self.config.remoteFiberOpticalLossRange.STEP;
		var powerMax = self.config.powerULB0Range.MAX;
		var powerMin = self.config.powerULB0Range.MIN;
		if (power > powerMin) { //se cambia el signo de desigualdad al ser MIN>MAX
			power = powerMin;
		} else if (power < powerMax) {
			power = powerMax;
		}
		var slope = (max - min) / (powerMax - powerMin);
		var loss = min + (power - powerMin) * slope;
		loss = Math.round(loss/step + 0.00001)*step;
		return loss;
	}
	...
	this.createShowFiltAdjTable = function(band) {
		...
		if (band==0 && factory.commomUl){
			var row = document.createElement("tr");
			tb.appendChild(row);
			var head = document.createElement("th");
			row.appendChild(head);
			head.innerHTML = "Assign 1st ADJBW to Band 14 Filter";
			head.className = "thdrht";
	...
	this.createAttRemote = function(nr, band) {
		var cell = document.createElement("td");
		// OCULTAR ATENUADOR DE REMOTO
		...
		el.title = "Min: "+self.config.attRemoteRange.MIN+", Max: "+ self.config.attRemoteRange.MAX +" dB";
		el.onchange = function(ev) {
			var num = parseInt(this.value);
			if (isNaN(num)) {
				num = 0;
			}
			if (num < self.config.attRemoteRange.MIN) {
				this.value = self.config.attRemoteRange.MIN;
			} else if (num > self.config.attRemoteRange.MAX) {
				this.value = self.config.attRemoteRange.MAX;
			} else {
				this.value = num;
			}
		}
	...
	this.setAttRemote = function(nr, band, val) {
		try {
			var el = document.getElementById("attR_"+nr+"_"+band);
			if (el && !isNaN(val)) {
				el.value = val.toString();
			}
		} catch (err) { }
	}
	this.getAttRemote = function(nr, band) {
		// OCULTAR ATENUADOR DE REMOTO
		// devolver siempre 0 dB
		return 0;
		try {
			var el = document.getElementById("attR_"+nr+"_"+band);
			if (el) {
	...
	this.createTempBoardRemote = function(nr) {
		var box = document.createElement("div");
		var tab = document.createElement("table");
		box.appendChild(tab);
		var tb = document.createElement("tbody");
		tab.appendChild(tb);
		var row = createMetRow("boardTempRemote_"+nr, board_temp_settings, Texts['TEMPERATURE'], "&ordm;C");
		tb.appendChild(row);
		return box;
	}	
	...
	this.boardTempRSet = function(nr,val) {
		setMetValue("boardTempRemote_"+nr, val);
	}
	...
	this.remoteAlarmSet = function(nr,num,alarm){
		// var k = num;
		// var r = nr;
		// if ( k == 1 && r == 0 ) {
		// 	// alert("remote#"+r+", alarm="+monitor.remote[r].alarmStatus[k]+", enable="+monitor.remote[r].alarmEnabled[k]);
		// 	alert("remote#"+r+", alarm="+alarm);
		// }
		ledSetColor("remoteAlarm_"+nr+"_"+num, alarm ? "red" : "grey");
	}
	this.createGralRemoteAlarm = function(num,txt,show){
		var row = document.createElement("tr");
		var cell = document.createElement("th");
		cell.style.width = "70%";
		row.appendChild(cell);
		cell.innerHTML = txt;
		cell.className = "thdrht";
		cell = createLedBox("remoteGralAlarm_"+num);
		cell.style.paddingLeft = "10px";
		row.appendChild(cell);
		row.style.display = show?"table-row":"none";
		return row;
	}
	this.remoteGralAlarmSet = function(num,alarm) {
		ledSetColor("remoteGralAlarm_"+num, alarm ? "red" : "grey");
	}
	...
	// remote LNA/PA on/off unificado
	this.isRemoteConfigRfOn = function(r) {
		return (self.config.remote[r].paStateOn[0]);
	}
	this.isRemoteStatusRfOn = function(r, monitor) {
		var isOn = true;
		for (var band = 0; band < 2; band++ ) {
			if ( !monitor.remote[r].lnaStateOn[band] ||
				!monitor.remote[r].paStateOn[band] )
			{
				isOn = false;
			}
		}
	...
	this.createRemoteRfOnOff = function (nr) {
		var cell = document.createElement("td");
		cell.style.textAlign = "center";
		cell.style.minWidth = "42px";
		cell.id = "remoteRfOnOffCell_"+nr;
		var rfOnOff = document.createElement("input");
		rfOnOff.type = "checkbox";
		rfOnOff.id = "remoteRfOnOff_"+nr;
		rfOnOff.name = rfOnOff.id;
		rfOnOff.nr = nr;
		cell.appendChild(rfOnOff);
		rfOnOff.onclick = function(ev) {
			self.rfOnOffEdited[this.nr]=true;
		}
		return cell;
	}
	this.setRemoteRfOnOff = function(nr, on, modify) {
		try {
			var id =  "remoteRfOnOff_"+nr;
			var el = document.getElementById(id);
			if (modify) el.checked = on? true: false;
			var state = el.checked;
			var el = document.getElementById("remoteRfOnOffCell_"+nr);
			el.style.backgroundColor = (state!=on)?"#e20000":"#E6E6E6";

			el.checked = !!on;
		} catch(e) {}
	}
	this.getRemoteRfOnOff = function(nr) {
		try {
			var id =  "remoteRfOnOff_"+nr;
			var el = document.getElementById(id);
			return el.checked;
	...
	this.createMainGainLim = function(dn, band) {
		...
		self.mainGainMin[band][dn] = self.mainGainMax[band][dn] - self.ATT_MAX;
		//el.title = "Min: "+self.mainGainMin[band][dn]+", Max: "+self.mainGainMax[band][dn]+" dB";
		el.title = "Min: "+self.ATT_MIN+", Max: "+self.ATT_MAX+" dB";
		el.onchange = function(ev) {
			var target = this;
			var band = target.band;
			var dn = target.dn;
			var num = self.mainGainLimGet(dn,band);
			if (num < self.mainGainMin[band][dn]) {
				num = self.mainGainMin[band][dn];
			} else if (num > self.mainGainMax[band][dn]) {
				num = self.mainGainMax[band][dn];
			}
			target.value = self.factory.gainlimit[2*band+dn]-num;
		}
		cell.appendChild(el);
		cell = document.createElement("td");
		row.appendChild(cell);
	...
	this.mainGainLimSet = function(dn, band, val) {
		try {
			...
			if (!isNaN(val)){
				var att = self.factory.gainlimit[2*band+dn]-val;
				el.value = att.toString();
			}
	...
	this.mainGainLimGet = function(dn, band) {
		try {
			var el = document.getElementById("mainGainLimit_"+band+"_"+dn);
			var g = self.factory.gainlimit[2*band+dn]-parseInt(el.value);
			return g;
		} catch (err) { return -128; }
	}
	this.statGainMainTitle = function(b, band, g) {
		try {
			if (b==1) g+= self.factory.gainlimit[2*band+1]-self.factory.gainlimit[2*band];//en DL se ajusta con diferencia de gainlimits
			var maxgain = self.factory.gainlimit[2*band+b];
			var mingain = maxgain - self.ATT_MAX;
			if (maxgain > g) {
				maxgain = g;
			}
			...
			var att = self.factory.gainlimit[2*band+b]-g;
			if (att>self.ATT_MAX) att=0;
			self.mainGainMax[band][b] = self.factory.gainlimit[2*band+b]-att;
			var title = "Min: "+att+", Max: "+self.ATT_MAX+" dB";
			var id = "mainGainLimit_"+band+"_"+b;
			var el = document.getElementById(id);
			el.title = title;
		} catch(err) {}
	}
	this.createRemotePowerLimit = function(nr, band, uldl){
		var cell = document.createElement("td");
		var el = document.createElement("input");
		cell.appendChild(el);
		el.id = "remotePowerLimit_"+nr+"_"+band+"_"+uldl;
		el.uldl = uldl;
		el.name = el.id;
		el.type = "text";
		el.style.width = "20px";
		...
		var powerRemoteRange = self.config.powerRemoteRangeGet(nr, band, uldl);
		el.title = "Min: "+powerRemoteRange.MIN+", Max: "+ powerRemoteRange.MAX +" dBm";
		el.onchange = function(ev) {
			var num = parseInt(this.value);
			var uldl = this.uldl;
			if (isNaN(num)) {
			...
			if (num < powerRemoteRange.MIN) {this.value = powerRemoteRange.MIN;
			} else if (num > powerRemoteRange.MAX) {
				this.value = powerRemoteRange.MAX;
			} else {
				this.value = num;
			}
		}
	...
	this.remotePowerLimSet = function(nr, band, uldl, val) {
		try {
			var el = document.getElementById("remotePowerLimit_"+nr+"_"+band+"_"+uldl);
			if (!isNaN(val))
				el.value = val.toString();
		} catch (err) { }
	}
	this.remotePowerLimGet = function(nr, band, uldl) {
		try {
			var el = document.getElementById("remotePowerLimit_"+nr+"_"+band+"_"+uldl);
			return parseInt(el.value);
		} catch (err) { return -128; }
	...
	this.createChBw = function(b, c, band) {
		...
		for (var i = 0; i < this.BWtable.length; i++) {
			...
			if (i==0 && !self.factory.commomUl) this.BWtable[i].include = false; //no hay filtro 150KHz en V/U
		}
		for (var i = 0; i < this.BWtable.length; i++) {
			...
			var opt = document.createElement("option");
			el.options.add(opt);
			var v = FilterTypes[self.factory.commomUl?1:0][band][i]['data'][3];
			if (!self.factory.commomUl && self.factory.fmodulo*self.factory.fstep == 10000000) v+=0.3; //delay incremented by 0.3 for clk = 480MHz
	...
	this.createMetPowInRemote = function(nr, band, uldl) {
		return createMetCell("rfInPowR_"+nr+"_"+band+"_"+uldl, chRfInRemote_settings[uldl]);
	}
	this.createTextPowInRemote = function(nr, band, uldl) {
		return createTextCell("rfInPowR_"+nr+"_"+band+"_"+uldl,0);
	}	
	...
	this.rfChInPowRSet = function(nr, band, uldl, val, color) {
		setMetValue("rfInPowR_"+nr+"_"+band+"_"+uldl, val, color);
	}
	this.rfChOutPowRSet = function(nr, band, uldl, val, color) {
		var id = "rfOutPowR_"+nr+"_"+band+"_"+uldl;
		if (val<0){
			setMetValue(id, "---",color);
		} else {
			setMetValue(id, val, color);
		}
	...
	this.createMetPowOutRemote = function(nr, band, uldl) {
		return createMetCell("rfOutPowR_"+nr+"_"+band+"_"+uldl, chRfOutRemote_settings[uldl]);
	}
	this.createTextPowOutRemote = function(nr, band, uldl) {
		return createTextCell("rfOutPowR_"+nr+"_"+band+"_"+uldl,0);
	}	
	...
	this.createMetAgcRemote = function(nr, band, uldl) {
		return createMetCell("agcR_"+band+"_"+nr+"_"+uldl, agc_settings[uldl]);
	}
	this.createTextAgcRemote = function(nr, band, uldl) {
		return createTextCell("agcR_"+band+"_"+nr+"_"+uldl,0);
	}
	this.agcRSet = function(nr, band, uldl, val) {
		setMetValue("agcR_"+band+"_"+nr+"_"+uldl, val);
	}
	...
	this.showConfs = function() {
		...
		for (var band = 0; band < 2; ++band) {
			...
			//REMOTES
			for (var r=0; r<self.maxRemotes; r++){
				if (band==0) self.displayRemoteAlarms(r, self.NFPA.remote[r].alarmEnabled);
				self.setAttRemote(r, band, self.config.remote[r].ulAtt[band]);
				// for (var b=0;b<2;b++){
				// solamente DL, en UL se sustituye por fiber optical loss
				var b = 1;
				self.remotePowerLimSet(r, band, b, self.config.remote[r].pmax[band][b]);
				// }
				// optical loss solamente en band=0, UL
				var num = self.computeOpticalLossFromPower(self.config.remote[r].pmax[0][0]);
				self.setRemoteFiberOpticalLoss(r, num);
			}
		}
		for (var n = 0; n < self.maxRemotes; n++) {
			self.setRemoteEnabled(n, self.config.connectedRemotes[n]);
			self.setRemoteRfOnOff(r, self.isRemoteConfigRfOn(n), true);
		}
	...
	this.isFilterOverlap = function(cnf, b, n, c, band) {
		...
		if (stp<=1.5e3) stp/=2;
		filtSep /= stp;
		var filtMax = self.factory.commomUl? 150:100
	...
	this.getConf = function(cnf) {
		...
		for (var r=0;r<self.maxRemotes;r++){
			cnf.connectedRemotes[r] = self.getRemoteEnabled(r);
			if ( cnf.connectedRemotes[r] && self.config.remote[r].isConfigValid) {
				self.getRemoteConf(cnf.remote[r], r);
			}
		}
	}
	this.getRemoteConf = function(cnfr, nr){
		var stateToForce;
		// optical loss solamente en band=0, UL
		var power = self.computePowerFromOpticalLoss(self.getRemoteFiberOpticalLoss(nr));
		for (var band=0;band<2;band++){
			cnfr.ulAtt[band] = self.getAttRemote(nr,band);
			// for (var b=0;b<2;b++){
			// solamente DL, en UL se sustituye por fiber optical loss
			var b = 1;
			cnfr.pmax[band][b] = self.remotePowerLimGet(nr,band,b);
			// }
			// en UL optical loss
			var b = 0;
			cnfr.pmax[band][b] = power;
		}
		stateToForce = self.getRemoteRfOnOff(nr);
		if (stateToForce != self.isRemoteConfigRfOn(nr)
			|| self.rfOnOffEdited[nr])
		{
			if( stateToForce)
				cnfr.forcePaOn[0] = true;
			else
				cnfr.forcePaOff[0] = true;
		}
	}
	this.getBandConf = function(cnf, b, band) {
		...
		/*if (b==0){
			if (gainMax>monitor.maxAllowGain[band]) gainMax = monitor.maxAllowGain[band];
		}else{
			if (gainMax>monitor.maxAllowGain[band]) gainMax = monitor.maxAllowGain[band]+self.factory.gainlimit[2*band+1]-self.factory.gainlimit[2*band];
		}*/
		var gainMin = self.factory.gainlimit[2*band+b] - self.ATT_MAX;
	...
	this.showStatus = function(monitor) {
		...
		for (var k=0;k<monitor.NUMALARM;k++)
			self.gralAlarmSet(k, monitor.alarmStatus[k]);
		for (var k=0;k<4;k++){
		...
		for (var band = 0; band < 2; ++band) {
			...
			for (var r=0;r<self.maxRemotes;r++){
				if (band==0){
					self.displayRemoteAlarms(r, monitor.remote[r].alarmEnabled);
					self.boardTempRSet(r, monitor.remote[r].temperature);
				}
				for (var b = 0; b < 2; b++) {
					self.rfChInPowRSet(r, band, b, monitor.remote[r].rfIn[band][b]);
					self.rfChOutPowRSet(r, band, b, monitor.remote[r].rfOut[band][b]);
					self.agcRSet(r, band, b, monitor.remote[r].agc[band][b]);
					
				}
			}
			for (var k=0;k<monitor.remote[0].NUMALARM;k++){
				var alarm = false;
				for (var r=0;r<self.maxRemotesToShow;r++){
					if (monitor.remote[r].alarmEnabled[k] && monitor.remote[r].alarmStatus[k]) alarm = true;
					self.remoteAlarmSet(r, k, monitor.remote[r].alarmEnabled[k] && monitor.remote[r].alarmStatus[k]);
				}
				self.remoteGralAlarmSet(k,alarm);
			}
			self.remoteGralAlarmSet(14,monitor.alarmStatus[14]);
		}
		if (self.doFrequencyCheck) {
			self.checkFreqs(false);
			self.doFrequencyCheck = false;
		}
		var showWarnRemote = false;
		for (var r=0;r<self.maxRemotes;r++) {
			self.setRemoteDisconnectLed(r, monitor.isRemoteConnectionAlarm(r), self.config.connectedRemotes[r]);
			self.setRemoteRfOnOff(r, self.isRemoteStatusRfOn(r, monitor), !self.rfOnOffEdited[r]);
			if ( !self.config.connectedRemotes[r] ) {
				continue;
			}
			if ( monitor.isRemoteConnectionAlarm(r)) {
				self.showTagRemote(r, "DISCONNECTED");
				self.showContentRemote(r, false);
			} else if ( !self.config.remote[r].isConfigValid ) {
				self.showTagRemote(r, "RELOAD SETTINGS");
				self.showContentRemote(r, true);
				showWarnRemote = true;
			} else {
				self.showTagRemote(r);
				self.showContentRemote(r, true);
			}
		}
		self.showWarnRemote(showWarnRemote);
		var showWarnRemoteBandUnmatched = false;
		for (var r=0;r<self.maxRemotes;r++) {
			if ( !self.config.connectedRemotes[r] || monitor.isRemoteConnectionAlarm(r) ) {
				continue;
			}
			var remoteBandUnmatch = !self.isRemoteBandMatchedStatus(monitor, r);
			self.showRemoteBandNameWarning(remoteBandUnmatch, r);
			if (remoteBandUnmatch) {
				showWarnRemoteBandUnmatched = true;
			}
		}
		self.showWarnRemoteBandUnmatched(showWarnRemoteBandUnmatched);
	}
	...
	this.displayRemoteAlarms = function(nr, v){
		//se oculta sólo door open. Estará disponible? y External inputs. Números de alarma: 7, 10-13
		var alarmsToShow = [7,10,11,12,13];
		for (var k=0;k<alarmsToShow.length;k++){
			var el = document.getElementById( "remoteAlarmLabel_"+nr+"_"+alarmsToShow[k]);
			el.style.display = v[alarmsToShow[k]]?"table-cell":"none";
			var el = document.getElementById( "remoteAlarmLed_"+nr+"_"+alarmsToShow[k]);
			el.style.display = v[alarmsToShow[k]]?"table-cell":"none";
		}
	...
	this.createExtremeTempAction = function(row) {
		...
		el.name = el.id;
		el.self = this;
		var opts = [ {val:0,text:"No Action"}, {val:2,text:"RF OFF"} ];
	...
	this.createIsolGain = function(row, band) {
		var cell = document.createElement("th");
		var b = [false,false];
		for (var k=0;k<2;k++)
			b[k] = self.factory.adjBandEnabled[k]||self.factory.chBandEnabled[k];
		...
		cell.innerHTML = "Min.&nbsp;Allowable&nbsp;Attenuation&nbsp;("+self.factory.bandNames[band]+")";
		cell.className = "thdrht";
		cell.style.height = "20px";
		row.appendChild(cell);
		cell = createTextCell("isolGain_"+band,1);
		row.appendChild(cell);
		cell = document.createElement("td");
		cell.innerHTML = "dB";
		cell.style.fontWeight = "normal";
	...
	this.setIsolGain = function(g) {
		try {
			...
			var attmax = self.ATT_MAX;
			var attAllow = [0,0];
			for (var k=0;k<2;k++){
				// la gain limit de UL es sustituida por la máxima de UL y DL
				// para el caso UL Low Gain Mode
				// attAllow[k] = self.factory.gainlimit[2*k]-g[k];
				attAllow[k] = maxGLimits[k]-g[k];
				attAllow[k] = (attAllow[k]>attmax?">"+attmax:attAllow[k])+".0"
				setTextCell("isolGain_"+k,attAllow[k]);
			}
		} catch(err) {}
	}
	...
	this.createWarnRemote = function() {
		var box = document.createElement("div");
		...
		box1.style.outline = "1px solid #004a98";
		var t = document.createElement("h");
		t.innerHTML = "OUTDATED&nbsp;REMOTE&nbsp;SETTINGS&nbsp;DATA&nbsp;-&nbsp;";
	...
	this.showWarnRemote = function(val) {
		try {
	...
	this.createWarnRemoteBandUnmatched = function() {
		var box = document.createElement("div");
		box.style.margin = "6px";
		box.id = "warnRemoteBand";
		box.style.display = "none";
		box.style.minHeight = "30px";
		var box1 = document.createElement("div");
		box1.style.minHeight = "25px";
		...
		box1.style.outline = "1px solid #004a98";
		var t = document.createElement("h");
		t.innerHTML = "INCOMPATIBLE&nbsp;BAND&nbsp;REMOTES&nbsp;";
		t.style.fontWeight = "bold";
		t.style.fontSize = "13px";
		t.style.color = "red";
		t.style.position = "relative";
	...
	this.showWarnRemoteBandUnmatched = function(val) {
		try {
			var el = document.getElementById("warnRemoteBand");
			var box = el.parentElement;
			el.style.display = val ? "block" : "none";
		} catch(err) {};
	}
	this.showRemoteBandNameWarning = function(val, nr) {
		try {
			for (var band = 0; band < 2; band++) {
				for (var uldl = 0; uldl < 2; uldl++) {
					var id = "remoteBandNameCell_"+nr+"_"+band+"_"+uldl;
					var el = document.getElementById(id);
					el.style.color = (val?"red":"black");
				}
			}
		} catch(err) {};
	}
	...
	this.maxRemotes = 0;
	this.maxRemotesToShow = 4;
	this.rfOnOffEdited = [];
	this.lnaEnabledEdited = [];
	this.paEnabledEdited = [];	
}
...
function WarningBox() {
	...
	this.setWarningMessage = function(fovlp, filtSepKhz, filtAdjSepKhz, filtNbAdjSepKhz, maxch, fact, uldlLinked) {
		...
		if (checkB){
			message += '<table style="width:100%"><body><tr><td style="width:38%;">';
	...
	this.filterWarnText = function(filtSepKhz,fact) {
		var filtmax = fact.commomUl ? 150:100;
		var str =
		...
		if (fact.chBandEnabled[0] && fact.chBandEnabled[1] && !fact.commomUl){
...
function createLedBox(id,align) {
	var tdNode = document.createElement("td");
