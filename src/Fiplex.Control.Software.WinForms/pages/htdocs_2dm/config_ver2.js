...
function NFPAconf(){
	this.MAXREMOTES 			= 8;
	this.NUMALARM				= 32;	// 8 alarmas más de master
	...
	this.delayTimer				= []; // 7 --> 1int/relay ---
	this.latchTimer				= []; // 7 --> 1int/relay ---
	// se añade "Door switch" en una alarma sin usar y LD/PD fail se ponen en un byte añadido (6 alarmas sin usar)
	// microcontroller fw version with Overlod UL: fip446 v1.16, f467 v1.15
	this.alarmNames;
	this.naturalOrder;
	this.overloadCapable = false;
	this.setAlarmNames = function(isOverloadULcapable) {
		this.overloadCapable = isOverloadULcapable;
		if (this.overloadCapable) {
			this.alarmNames	= [
				"HW Fail","High.Temp","Antenna Isolation","VSWR B0","Oscillation Detection","VSWR B1","Antenna Disconnection B0","Antenna Disconnection B1",
				"UL PA Fail B0", "UL PA Fail B1", "Overload DL B0", "Overload DL B1","Rx Power Low B0", "Rx Power Low B1", "Remote Disconnection","Undefined",
				"DL PA Fail B0","DL PA Fail B1", "Tx Power Low B0", "Tx Power Low B1","External Alarm 1","External Alarm 2","External Alarm 3","Force RF OFF",
					"Laser Fail","Photodiode Fail","DL AGC Fail B0","DL AGC Fail B1","Overload UL B0", "Overload UL B1","Undefined","Undefined"];
			this.naturalOrder = [0,1,2,4,3,5,6,7,8,9,28,10,29,11,12,13,16,17,18,19,26,27,24,25,20,21,22,23,15,14,30,31];
		} else {
			this.alarmNames	= [
				"HW Fail","High.Temp","Antenna Isolation","VSWR B0","Oscillation Detection","VSWR B1","Antenna Disconnection B0","Antenna Disconnection B1",
				"UL PA Fail B0", "UL PA Fail B1", "Overload DL B0", "Overload DL B1","Rx Power Low B0", "Rx Power Low B1", "Remote Disconnection","Undefined",
				"DL PA Fail B0","DL PA Fail B1", "Tx Power Low B0", "Tx Power Low B1","External Alarm 1","External Alarm 2","External Alarm 3","Force RF OFF",
				   "Laser Fail","Photodiode Fail","DL AGC Fail B0","DL AGC Fail B1","Undefined","Undefined","Undefined","Undefined"];
			   this.naturalOrder = [0,1,2,4,3,5,6,7,8,9,10,11,12,13,16,17,18,19,26,27,24,25,20,21,22,23,15,14,28,29,30,31];
		}
	}
	this.setAlarmNames(this.overloadCapable);
	this.isOverloadULcapable = function() {
		return this.overloadCapable;
	}
	...
	this.parse = function(s) {
		...
		version.parse(str);
		var isOverloadUlcapable = (version.compareSw(1, (factory.commomUl?16:15)) >= 0);
		this.setAlarmNames(isOverloadUlcapable);
		// trama de configuración ampliada en 8 bytes (16 char) para asignación de relays de alarmas añadidas al master
		if (s.length<514) return -1; 
		var res;
		var ind = 0;
		this.frm = s;
		for (var k=0;k<2;k++){
		...
		res |= parseInt(s.substring(ind,ind+2), 16)<<24; ind+=2; // nuevo byte de alarmas de master
		
		ind+=4; //2bytes sin uso
		for (var k=0;k<this.NUMALARM;k++){
		...
		this.alarmEnabled[15]=false; //door open deshabilitada
		for (var k=0;k<this.NUMALARM;k++){
			res = parseInt(s.substring(ind,ind+2), 16); ind+=2;
			for (var j=0;j<7;j++){
		...
		for (var j=0;j<7;j++){
			this.relayAssignAlarm[15][j] = false; //door open sin relay assign
			if (factory.commomUl){
		...
		for (var k=0;k<3;k++){ // excluye alarma FORCE RF OFF
			this.alarmNames[20+k] = s.substring(ind,ind+30).trim();ind+=30;
		}
		ind+=30;  // alarma FORCE RF OFF
		for (var k=0;k<this.NUMALARMREMOTE;k++){
			res = parseInt(s.substring(ind,ind+2), 16); ind+=2;
			for (var j=0;j<7;j++){
	...
	this.getFrm = function(){
		...
		cfg += hexformat((mask >> 8) & 0xff,2);
		cfg += hexformat((mask >>16) & 0xff,2);
		cfg += hexformat((mask >>24) & 0xff,2);  // byte de alarmas aÃ±adidas de master
		cfg += "0000";//2bytes sin uso
		for (var j=0;j<7;j++){
			this.relayAssignAlarm[15][j] = false; //door open relay assign = false
			if (factory.commomUl){
				this.relayAssignAlarm[7][j] = false; //para 7/8 ant.disconnectB1 deshabilitada
				this.relayAssignAlarm[9][j] = false; //para 7/8 ulpafail B1 deshabilitada	
			}
...
function overallConfigToTextFile(config, nfpacfg, factory, tagstr, ethstr, serial, version){
	...
	cfg += versionToText(version,config,factory);
...
function versionToText(version, config, factory){
	var j,valid,cfg,v;
	cfg  = "\nVERSION\n";
...
function alarmSettingsToText(nfpacfg,factory){
	...
	for (var k = 0; k < nfpacfg.remote[0].alarmNames.length; k++) {
		...
		if ( name.search("Undefined") >= 0 || name.length == 0)
			continue;
		// en los remotos se dedició no usar la cuarta alarma externa
		if (p == 13) {
			continue;
		}
