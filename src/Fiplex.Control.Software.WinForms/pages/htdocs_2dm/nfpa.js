...
function refreshCtrlEnable(num,nr,val,valrelay){
	...
	var isExtGlobalAlarm =  (nr > 0 && num >= 10 && num <= 13);
...
function GUI_NFPA() {
	this.read = function(nfpa){
		...
		for (var k=23;k<nfpa.NUMALARM;k++) {
	...
	this.show = function(nfpa,tags){
		...
		for (var r=0;r<=nfpa.MAXREMOTES;r++){
			...
			for (var k=0;k<(r==0?nfpa.NUMALARM:nfpa.remote[0].NUMALARM);k++){
				...
				var extIndex = r==0?23:10;
				if ( k >= extIndex && k < extIndex+4) {
					if (r==0) {
						var externalAlarmNr = k - extIndex + 3;
	...
	this.createRelayConfigTable = function(tb, nr){
		...
		for (var j=0;j<(nr==0?12:8);j++) {
			...
			for (var k=0;k<2;k++){
				var hideCell = (nr>0 && (2*j+k)==15) || (nr==0 && (2*j+k)==22); //se oculta door open
				// hideCell = hideCell || (nr>0 && (j==2 || j==4)) // se ocultan filas de overload y temperatura en remoto
				...
				if ( isUserAlarm ) {
					var userAlarmNr = 2*(j-(nr==0?10:5)) + k + 1;
					if (nr > 0) {
