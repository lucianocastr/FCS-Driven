...
function Page() {
	...
	this.createMainPowerLim = function(dn, band) {
		...
		if ( this.factory.ULlowGainMode ) {
			titleStart = "Power per channel ";
		} else {
			titleStart = "Composite Power ";
		}
		el.title = titleStart+"Min: "+min+", Max: "+max+" dBm";
		el.onchange = function(ev) {
			var target = this;
			var band = target.band;
			var dn = target.dn;
	...
	this.createChBw = function(b, c, band) {
		...
		for (var i = 0; i < this.BWtable.length; i++) {
			...
			var opt = document.createElement("option");
			el.options.add(opt);
			opt.text = this.BWtable[i].txt+" "+FilterTypes[self.factory.commomUl?1:0][band][i]['data'][3]+"us";
