...
var saveFileCmd = 
[
	// 16 char más por 8 bytes de asignación de relés a alarmas de master añadidas
	{get: "/global_conf.shtml", post:"nosend" , postReg: "global_req", len: 3658, decode: 0},
	{get: "/updt_comm.shtml", post:"nosend" , postReg: "", len:  157, decode: 0}
];

var loadFileCmd = 
[
	{get: "/update_conf.shtml", post:"ctl_conf_str" , postReg: "conf_req", len: 1724, decode: 0},
	{get: "/update_tags.shtml", post:"ctl_tags_str" , postReg: "tags_req", len:  60*9, decode: 1},
	// 16 char más por 8 bytes de asignación de relés a alarmas de master añadidas
	{get: "/update_nfpa.shtml", post:"nfpa_str" , postReg: "nfpa_req", len:  570, decode: 0},
					...
					if (postget==0){
						...
						if (num<(saveFileCmd.length-1)){
							...
							if (format==0)
								...
								var strFile = overallConfigToTextFile(settings.config,settings.NFPAcfg,settings.factory,settings.tags.tag,fdata[3],
																	  settings.serNrFrame,settings.versionFrame);
								fdata = strFile.split("\n");
...
function fileParseGlobalConfig(str) {
	...
	settings.versionFrame = srVersion;
