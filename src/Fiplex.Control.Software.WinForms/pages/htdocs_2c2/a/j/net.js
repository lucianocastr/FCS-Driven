...
var nfpaStatusRequestRate = 0;	// 5  load status for each nfpa status
var NFPAstat;
...
function end_loading() {
	...
	for (var band=0;band<2;band++){
		...
		page.controlChannelFirstnetDisable();
	}
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState==4) {
				if (this.status === 200) {
					...
					var d = Date.now();
					var frm = this.responseText;
					localStorage.setItem("StatDate"+Prjstr+window.location.host, d);
					localStorage.setItem("Stat"+Prjstr+window.location.host, frm);
					monitor.parse(frm);
					if ( monitor.isZeros ) {
				...
				if ( !uCSupportsBbuMvo2Mode(version) && isBbuSerialMode(config, factory, version) ) {
					// uC v2.00 && factory MMS mode ON, request NfpaStat once for 5 times load_status
					if (++nfpaStatusRequestRate < 5) {
						tmrIdStat = setTimeout(function() { load_status(); }, timeoutMs);
					} else {
						nfpaStatusRequestRate = 0;
						tmrIdStat = setTimeout(function() { load_nfpa_status_mvo1(); }, timeoutMs);
					}
				} else {
					tmrIdStat = setTimeout(function() { load_status(); }, timeoutMs);
				}
...
function load_nfpa_status_mvo1() {
	try {
		if (typeof(tmrIdStat) !== "undefined" && tmrIdStat) {
			clearTimeout(tmrIdStat);
		}
		...
		xhr.onerror = function(ev) {
			if (typeof(tmrIdStat) !== "undefined" && tmrIdStat)
				clearTimeout(tmrIdStat);
			tmrIdStat = setTimeout(function() { load_nfpa_status_mvo1(); }, 100);
		}
		xhr.ontimeout = function(ev) {
			if (typeof(tmrIdStat) !== "undefined" && tmrIdStat)
				clearTimeout(tmrIdStat);
...
function submitform(isReset, isolVerif, isolClear, band, forceSend, forcePaOn, forcePaOff, setDefaultRelay) {
	var doReset = false;
	...
	if (!checkChange() && !doReset && !doIsolVerif && !doIsolClear && !doForceSend && !doSetDefaultRelay){
		return;
	}
