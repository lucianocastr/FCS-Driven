...
function Page() {
	...
	this.createAlarmInterface = function(){
		...
		var rspan = 3;
		if (!self.factory.extremeTempActionEnable && !self.factory.oscFeatureEnable) {
			rspan = 2;
		} else {
		...
		else if (self.factory.oscFeatureEnable){
	...
	this.createAlarmTableBand = function(){
		...
		for (var band=0;band<2;band++){
			...
			row.appendChild(cell);
			if ( self.factory.limitTo32CH ) {
				/** no DL Output Power alarm */
				self.NFPA.bandAlarmsInstalled[band][7] = false;
				self.NFPA.bandAlarmsEnabled[band][7] = false;
			}
			self.NFPA.bandAlarmsInstalled[band][5] = self.factory.masterRxLowAlarm;
	...
	this.createAlarmTable = function(){
		...
		for (var k=0;k<self.NFPA.globalAlarmsInstalled.length;k++){
			var nosh = self.NFPA.globalAlarmsInstalled[k];
			if (k==12 && !self.NFPA.globalAlarmsEnabled[k]) nosh=false;
			tb.appendChild(this.createGralAlarm(k,self.NFPA.alarmNames[0][k],nosh));
		}	
		...
		for (var k=0;k<self.NFPA.bbuAlarmsInstalled.length;k++){
			...
			tb.appendChild(this.createBbuAlarm(k,self.NFPA.alarmNamesBbu[k],show));
		}	
		return cellb;
	}
	...
	this.relayStateSet=function(nrelay,openclose,onoff){
		var isNormalACPowerRelay = isBbuSerialMode(self.config) && self.NFPA.isRelayAssignNormalACpowerExclusive(nrelay);
	...
	this.createShowFiltTable = function(band) {
		...
		tab.appendChild(tb);
		var n = 0;
		var nrows = self.factory.limitTo32CH?2:3;
		var nch = self.factory.limitTo32CH?16:24;
		var ncols = 0;
	...
	this.createBandOutBox = function(dn,band) {
		...
		if (dn==1){
			var bbAgcSettings = {min: 0, low_alarm: -128, low_warn: -128, high_warn: 127, high_alarm: 127, max: 32};
			row = createMetRow("bbAgcOut_"+band, bbAgcSettings, "Output&nbsp;Broadband&nbsp;AGC", "dB" ,40);
			body.appendChild(row);
		}
		return box;
	}
	...
	this.bbOutAgcSet = function(band, val) {
		setMetValue("bbAgcOut_"+band, val, "undefined", 1);
	}
	...
	this.createMainGainLim = function(dn, band) {
		...
		var cell = document.createElement("th");
		row.appendChild(cell);
		if (this.factory.limitTo32CH) {
			cell.innerHTML = (dn==0?"Main Gain":"DL Output Attenuation");
		} else {
		...
		var gmin,gmax;
		if (self.factory.limitTo32CH && dn==0) {
			// gain
			var gmin = self.factory.gainlimit[2*band+dn]+self.config.GmainRange[dn];
			var gmax = self.mainGainMax[band][dn];
		} else {
			// attenuation
			var gmin = self.factory.gainlimit[2*band+dn]-self.mainGainMax[band][dn];
			var gmax = -self.config.GmainRange[dn];
		}
		...
		el.onchange = function(ev) {
			...
			target.value = num;
			if ( !(self.factory.limitTo32CH && dn==0) ) {
				var g = self.factory.gainlimit[2*band+dn]-num;
				var max = self.factory.powerlimit[2*band+dn]-g;
				var min = self.factory.powerlimit[2*band+dn] - self.factory.gainlimit[2*band+dn] - self.MAIN_POWER_RANGE; //El min es absoluto, no depende de conf.gain
	...
	this.statGainMainTitle = function(b, band, g) {
		try {
			...
			var gmin,gmax;
			if (self.factory.limitTo32CH && b==0) {
				// gain
				var gmin = self.factory.gainlimit[2*band+b]+self.config.GmainRange[b];
				var gmax = self.mainGainMax[band][b];
			} else {
				// attenuation
	...
	this.createMainPowerLim = function(dn, band) {
		...
		var cell = document.createElement("th");
		row.appendChild(cell);
		if (this.factory.limitTo32CH && dn==0) {
			if ( this.factory.ULlowGainMode ) {
				cell.innerHTML = dn==0?"Maximum Power<br>per channel":"Input AGC per channel<br>Composite Power Set";
			} else {
		...
		var max,min;
		if (this.factory.limitTo32CH && dn==0) {
			max = this.factory.powerlimit[2*band+dn];
			min = max - this.MAIN_POWER_RANGE; //El mínimo es absoluto, no depende de conf.gain	
		} else {
			max = this.factory.powerlimit[2*band+dn]-this.config.gain[band][dn];
			min = this.factory.powerlimit[2*band+dn] - this.factory.gainlimit[2*band+dn] - this.MAIN_POWER_RANGE; //El mínimo es absoluto, no depende de conf.gain	
		}
		el.title = "Min: "+min+", Max: "+max+" dBm";
		el.onchange = function(ev) {
			var target = this;
			...
			var g = self.mainGainLimGet(dn,band);
			g = self.factory.gainlimit[2*band+dn]-g; //ATT --> gain
			var max,min;
			if (self.factory.limitTo32CH && dn==0) {
				max = self.factory.powerlimit[2*band+dn];
				min = max - self.MAIN_POWER_RANGE;
			} else {
				max = self.factory.powerlimit[2*band+dn]-g;
				min = self.factory.powerlimit[2*band+dn] - self.factory.gainlimit[2*band+dn] - self.MAIN_POWER_RANGE; //El mínimo es absoluto, no depende de conf.gain
			}
			if (num < min) {
				target.value = min;
			} else if (num > max) {
	...
	this.createChBw = function(b, c, band) {
		...
		for (var i = 0; i < this.BWtable.length; i++) {
			...
			if (!self.factory.commonUl && self.factory.fmodulo*self.factory.fstep == 10000000) v+=0.3; //delay incremented by 0.3 for clk = 480MHz
	...
	this.showConfs = function(onlyFilterFields) {
		...
		for (var band = 0; band < 2; ++band) {
			...
			for (var b = 0; b < 2; ++b) {
				var gatt = self.config.gain[band][b];
				if (self.factory.limitTo32CH && b==0) {
					self.mainGainLimSet(b, band, gatt);
					self.mainPowerLimSet(b, band, self.config.power[band][b]);
				} else {
					gatt = self.factory.gainlimit[2*band+b]-gatt;
					self.mainGainLimSet(b, band, gatt);
					self.mainPowerLimSet(b, band, self.config.power[band][b]-self.config.gain[band][b]);
					try {
						var max = self.factory.powerlimit[2*band+b]-self.config.gain[band][b];
						var min = self.factory.powerlimit[2*band+b] - self.factory.gainlimit[2*band+b] - self.MAIN_POWER_RANGE; //El min es absoluto, no depende de conf.gain
	...
	this.getBandConf = function(cnf, b, band) {
		...
		if ( !(self.factory.limitTo32CH && b==0) ) {
			gmain = self.factory.gainlimit[2*band+b]-gmain; //ATT
		}
		...
		if (self.factory.limitTo32CH && b==0) {
			...
			var agcSensMin = self.factory.powerlimit[2*band+b] - self.factory.gainlimit[2*band+b] - self.config.limitPowerRange[b]; 
		...
		if (b==0) cnf.power[band][b] = pmain; //keep unused DL value
		//simplex
		if (simplex){
	...
	this.getChConf = function(cnf, c, band) {
		...
		for (var b = 0; b < 2; ++b) {
			...
			if (on){//si canal inactivo se conserva bw,gfine
	...
	this.showStatus = function(monitor) {
		...
		for (var band = 0; band < 2; ++band) {
			...
			self.bbOutAgcSet(band, monitor.statePaOn[band][1]?monitor.gain[1][band][1][0]:0);//AGC BB en ADJ1 DL de cada banda
			for (var k=0;k<8;k++)
	...
	this.createOpfSettingsAntIsol = function() {
		...
		tb.appendChild(row);
		this.createIsol(row);
		if (!self.factory.limitTo32CH) row.style.display = "none"; // absolute isolation only shown in miniDAS
		for (var band=0;band<2;band++){
			var row = document.createElement("tr");
			this.createCentricIsolGain(row,band);
			if (self.factory.limitTo32CH) row.style.display = "none"; // min allowable att per band only shown in centric
			tb.appendChild(row);
		}
		var row = document.createElement("tr");
		this.createIsolGain(row);
		if (!self.factory.limitTo32CH) row.style.display = "none"; // only shown in miniDAS
		tb.appendChild(row);
		var row = document.createElement("tr");
		this.createIsolAtt(row);
		if (!self.factory.limitTo32CH) row.style.display = "none"; // only shown in miniDAS
		tb.appendChild(row);
		return box;
	}
	...
	this.createIsolGain = function(row) {
		...
		var dual = b[0] && b[1];
		if (dual){
			cell.innerHTML = "Max.&nbsp;Allowable&nbsp;UL&nbsp;Gain("+self.factory.bandNames[0]+"/"+self.factory.bandNames[1]+")";
		}else{
		...
		cell.style.height = "20px";
		row.appendChild(cell);
		cell = createTextCell("isolGain_",1);
		row.appendChild(cell);
		cell = document.createElement("td");
		cell.innerHTML = "dB";
		cell.style.fontWeight = "normal";
	...
	this.createIsolAtt= function(row) {
		var cell = document.createElement("th");
		var b = [false,false];
		for (var k=0;k<2;k++)
			b[k] = self.factory.adjBandEnabled[k]||self.factory.chBandEnabled[k];
		var dual = b[0] && b[1];
		if (dual){
			cell.innerHTML = "Min.&nbsp;Allowable&nbsp;DL&nbsp;Att.("+self.factory.bandNames[0]+"/"+self.factory.bandNames[1]+")";
		}else{
		...
		cell.style.height = "20px";
		row.appendChild(cell);
		cell = createTextCell("isolAtt_",1);
		row.appendChild(cell);
		cell = document.createElement("td");
		cell.innerHTML = "dB";
		cell.style.fontWeight = "normal";
	...
	this.setIsolGain = function(g) {
		try {
			...
			for (var k=0;k<2;k++){
				...
				attAllow[k] = (att>attmax?">"+attmax:att)+".0";
			}
			var b = [false,false];
			for (k=0;k<2;k++)
				b[k] = self.factory.adjBandEnabled[k]||self.factory.chBandEnabled[k];
			...
			if (dual){
				...
				setTextCell("isolGain_",gainAllow[0] + " / " + gainAllow[1]);
				setTextCell("isolAtt_",attAllow[0] + " / " + attAllow[1]);
			}else{
				var ind = b[0]?0:1;
				setTextCell("isol",isol[ind]);
				setTextCell("isolGain_",gainAllow[ind]);
				setTextCell("isolAtt_",attAllow[ind]);
			}
			for (k=0;k<2;k++){
	...
	this.createFOAlarmTable = function() {
		...
		for (var k = 0; k < 2; k++) {
			...
			if (this.factory.limitTo32CH){
				/** delay adjust disabled in mini DAS microcontroller fw */
				cell.style.display = "none";
			}		
		row.appendChild(cell);
		}
		for (var port = 0; port < 2; port++) {
			...
			el.innerHTML = "OPTICAL PORT "+(port+1);
			...
			try {
				for (var k = 0; k < 3; k++) {
					for (var b = 0; b < 2; b++) {
						if (self.factory.limitTo32CH){
							/** delay adjust disabled in mini DAS microcontroller fw */
							document.getElementById("delayCell_"+k+"_"+b).style.display = "none";
							this.checked = false;
						} else {
