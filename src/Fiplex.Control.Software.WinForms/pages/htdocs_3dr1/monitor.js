function Monitor() {
	...
	this.isZeros = false;
	this.factory_mini_das_mode 		= false;
	...
	for (var band=0;band<2;band++){
		this.bandAlarm.push([]);
		for (var i=0;i<2;i++) {
			this.bandAlarm[band].push([]);
			for (var k=0;k<8;k++){
				this.bandAlarm[band][i].push(false);
			}
		}
	}
	for (var k=0;k<7;k++){
	...
	this.parse = function(s,limitTo32CH) {
		if ( this.checkIsZeros(s) ) {
			this.isZeros = true;
			return;
		}
		var statusLengthCentric = 1528;
		var statusLengthMini = 1512;
		var statusLengthBbuMvo2 = 33*2;
		var statusLength = s.length;
		this.isZeros = false;
		if (statusLength<(statusLengthMini+statusLengthBbuMvo2)) return -1; 
		var res,res2;
		var ind = 0;
		this.frm = s;
		//GLOBAL
		...
		for (var band=0;band<2;band++){
			...
			if (band == 0) {
				...
				this.factory_mini_das_mode = (res & 0x80) != 0;
			}
			...
			if (statusLength >= (statusLengthCentric+statusLengthBbuMvo2)) {
				res = parseInt(s.substring(ind,ind+8), 16); ind+=8;
			...
			for (var uldl=0;uldl<2;uldl++){
				for (var ch=0;ch<this.CHNR/2;ch++){
					this.level[0][band][0][ch+this.CHNR/2*uldl] = to_float(parseInt(s.substring(ind,ind+4),16)); ind+=4; //nb
					this.gain[0][band][0][ch+this.CHNR/2*uldl] = to_ufloat(parseInt(s.substring(ind,ind+4),16)); ind+=4; //nb
				}
				res = parseInt(s.substring(ind,ind+2), 16); ind+=2;
		...
		//NFPA Status
			...
			for (var b = 0; b < 2; b++) {
				for (var i=0;i<2;i++) {
					res = parseInt(s.substring(ind,ind+2), 16); ind+=2;
					for (var k=0;k<8;k++){
						this.bandAlarm[b][i][k] = (res & (1<<k))!=0;
					}
				}
			}
			for (var k=0;k<7;k++){
