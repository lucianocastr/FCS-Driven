...
function Config() {
	...
	this.uldlLinkedFreq				= [false,false]; //Sin uso
	this.muteModeLinked				= [false,false]; //Sin uso
	this.numberOfFilterNonGrouped	= [[0,0],[0,0]]; //En realidad, array sin dependencia UL/DL
	this.simplexMode				= [false,false]; //Sin uso
	...
	this.power						= []; //band0/1 ul/dl [2][2] //En realidad, sólo UL
	...
	this.parse = function(s) {
		...
		for (var b=0;b<2;b++){ //Band0/1
			...
			res = parseInt(s.substring(ind,ind+2), 16); ind+=2;
			this.uldlLinkedFreq[b] = false; //se fuerza a false para no tener problemas con GUI
			...
			for (var i=0;i<2;i++){//UL/DL
				...
				for (var ch=0;ch<this.CHNR/2;ch++){//32CH (los filtros UL son 1-32, los filtros DL 33-64)
					//CHON,GROUPED,BW
					res = parseInt(s.substring(ind,ind+2), 16); ind+=2;
					this.filterEnabled[0][b][0][ch+i*32] = (res & 0x80)==0;
					this.isFilterGrouped[b][0][ch+i*32] = (res & 0x40)!=0;
					this.freqHz[b][0][ch+i*32] = ((res & 0x20)>>5)*factory.fstep/2; //1 extra bit for freq
					this.bwIndex[b][0][ch+i*32] = (res & 0x7);
					this.bwKHz[b][0][ch+i*32] = this.computeBWFromIndex(res & 0x7);
					//FINE GAIN
					this.fineGainFilter[0][b][0][ch+i*32] = parseInt(s.substring(ind,ind+2), 16); ind+=2;
					if (this.fineGainFilter[0][b][0][ch+i*32]>127) this.fineGainFilter[0][b][0][ch+i*32]-=256;
					//FREQ
					res = parseInt(s.substring(ind,ind+4), 16); ind+=4;
					if (res>32767) res-=65536;
					this.freqHz[b][0][ch+i*32] += factory.fref[2*b]+res*factory.fstep;
				}
		...
		//Squelchs
		for (b=0;b<2;b++){
			for (var ch=0;ch<32;ch++){//se fuerza a 32 porque la trama se mantiene aunque no tenga uso
				res = parseInt(s.substr(ind,2), 16); ind+=2; if (res>127) res-=256;
				this.sqChThreshold[0][b][1][ch] = res; //sin uso (sq DL)
			}
			for (i=0;i<2;i++){
				for (var ch=0;ch<this.ADJNR;ch++){//4ADJ
					res = parseInt(s.substr(ind,2), 16); ind+=2; if (res>127) res-=256;
			...
			for (var ch=0;ch<32;ch++){//se fuerza a 32 porque la trama se mantiene aunque no tenga uso
				this.sqChEnabled[0][b][1][ch] = (res & (1<<ch))!=0; //sin uso (sq DL)
			}
		}
	...
	this.getFrm = function(){
		...
		for (b=0;b<2;b++){ //Band0/1
			//SIMPLEX,NGROUPS,MUTEMODE,LINKEDULDL
			mask=0;
			this.uldlLinkedFreq[b] = false;//se fuerzan a false
			...
			for (i=0;i<2;i++){//UL/DL
				...
				for (ch=0;ch<this.CHNR/2;ch++){//32CH (los filtros UL son 1-32, los filtros DL 33-64)
					mask=0;
					//CHON,GROUPED,BW
					if (!this.filterEnabled[0][b][0][ch+i*32]) mask|=0x80;
					if (this.isFilterGrouped[b][0][ch+i*32]) mask|=0x40;
					mask|=this.bwIndex[b][0][ch+i*32] & 0x7;
					this.bwKHz[b][0][ch+i*32] = this.computeBWFromIndex(this.bwIndex[b][0][ch+i*32]);
					fr = ~~Math.round((this.freqHz[b][0][ch+i*32]-factory.fref[2*b])/factory.fstep*2);if (fr<0) fr+=131072; //1extra bit for freq
					mask|= (fr & 0x1)<<5;
					cfg += hexformat(mask,2);
					//FINE GAIN
					res=this.fineGainFilter[0][b][0][ch+i*32];if (res<0) res+=256;
					cfg += hexformat(res,2);
					//FREQ
		...
		for (b=0;b<2;b++){
			...
			mask = 0;
			for (var ch=16;ch<32;ch++){//CH 16-31
				if (this.sqChEnabled[0][b][1][ch])  mask|=1<<(ch-this.CHNR/4);
			}
			cfg += hexformat(mask,4);
			mask = 0;
			for (var ch=0;ch<16;ch++){//CH 0-15
		...
		this.set_data_frame_index = 1;
		cfg += hexformat(this.set_data_frame_index, 2);
...
function NFPAconf(){
	...
	this.alarmNames				= [["HW Fail","High Temperature","Antenna Isolation","Oscillation Detection","UL PA Fail","Antenna Disconnection","Fiber Optic 1","Fiber Optic 2",
								   ...
								   ["Overload UL","Overload DL","DL PA Fail","Tx Power Low","VSWR","Rx Power Low","DL AGC Fail","DL Output Power","","","","","","","",""]];
	this.alarmNamesBbu =
	...
	for (var k=0;k<8;k++) { 
		...
		for (var j=0;j<this.NR_OF_RELAYS_MAX;j++) this.relayAssignFiberOpticAlarm[k].push(false);
	}
	...
	this.setDefaultRelayAssign = function(mode){ //mode = 0 - dry contacts / 1 - serial
		...
		if (mode==1){
			...
			var alr = [0,2,3,4,6];
			for (var k=0;k<alr.length;k++) this.relayAssignBandAlarm[0][alr[k]][6] = true;
		}else{
			//relay 2: VSWR
			this.relayAssignBandAlarm[0][4][1] = true;
			...
			var alr = [0,2,3,6];
			for (var k=0;k<alr.length;k++) this.relayAssignBandAlarm[0][alr[k]][3] = true;
		}
	}
	this.clearRelayAssign = function(){
		...
		for (var b=0;b<2;b++){
			for (var i=0;i<this.relayAssignBandAlarm[0].length;i++){
				for (var j=0;j<this.NR_OF_RELAYS_MAX;j++){
					this.relayAssignBandAlarm[b][i][j] = false;
				}
			}
		}
	...
	this.parse = function(s) {
		...
		for (var i = 0; i < 2; i++ ) {
			...
			for (var k=0;k<8;k++){
				...
				this.bandAlarmsInstalled[i][k] = (i==0 && k<8); //Se fuerzan "available" todas las del primer byte
			}
		}
		this.bandAlarmsInstalled[0][1] = false; //no hay Overload DL en remoto
		// this.bandAlarmsInstalled[0][5] = false; //no hay Rx Power Low en remoto
		...
		for (var k=0;k<8;k++){
			...
			res = parseInt(s.substring(ind,ind+2), 16); if (m==6) ind+=2;
			for (var j=0;j<2;j++){
				this.relayAssignFiberOpticAlarm[k][8+j] = (res & (1<<(j+m)))!=0;
			}
		}
		for ( var i = 0; i < 2; i++ ) {
			for (var k=0;k<8;k++){
				...
				res = parseInt(s.substring(ind,ind+2), 16); if (m==6) ind+=2;
				for (var j=0;j<2;j++){
					this.relayAssignBandAlarm[i][k][8+j] = (res & (1<<(j+m)))!=0;
				}
			}
		}
		for (var k=0;k<16;k++){
		...
		for (var k=0;k<8;k++){
			...
			res = parseInt(s.substring(ind,ind+2), 16); if (m==7) ind+=2;
			for (var j=0;j<1;j++){
				this.relayAssignFiberOpticAlarm[k][7] = (res & (1<<(j+m)))!=0;
			}
		}
		for ( var i = 0; i < 2; i++ ) {
			for (var k=0;k<8;k++){
				...
				res = parseInt(s.substring(ind,ind+2), 16); if (m==7) ind+=2;
				for (var j=0;j<1;j++){
					this.relayAssignBandAlarm[i][k][7] = (res & (1<<(j+m)))!=0;
				}
			}
		}
		for (var k=0;k<16;k++){
	...
	this.getFrm = function(){
		...
		for (var k=0, mask=0;k<16;k++){
			if (!(factory.limitTo32CH && (k>=12 && k<15))){ //gen sys alarms disabled for mini DAS
		...
		for (var i = 0; i < 2; i++ ) {
			if (factory.masterRxLowAlarm) this.bandAlarmsEnabled[i][5] = false; //if masterRxLowAlarm = false, rx powerlow alarm disabled
		...
		for (var k=0, mask2=0;k<8;k++){
			...
			mask = 0;
			for (var j=0;j<2;j++){
				if (this.relayAssignFiberOpticAlarm[k][8+j]) mask|= 1<<j;
			}
			mask2 |= (mask << m);
			if (m==6) {
				cfg += hexformat(mask2,2);
		...
		for ( var i = 0; i < 2; i++ ) {
			for (var k=0, mask2=0;k<8;k++){
				...
				mask = 0;
				for (var j=0;j<2;j++){
					if (this.relayAssignBandAlarm[i][k][8+j]) mask|= 1<<j;
				}
				mask2 |= (mask << m)
				if (m==6) {
					cfg += hexformat(mask2,2);
		...
		for (var k=0, mask2=0;k<8;k++){
			var n = ~~Math.floor(k/8);
			var m = (k - n*8);
			if (this.relayAssignFiberOpticAlarm[k][7]) mask2|= 1<<m;
			if (m==7) {
				cfg += hexformat(mask2,2);
				mask2 = 0;
			}
		...
		for ( var i = 0; i < 2; i++ ) {
			for (var k=0, mask2=0;k<8;k++){
				var n = ~~Math.floor(k/8);
				var m = (k - n*8);
				if (this.relayAssignBandAlarm[i][k][7]) mask2|= 1<<m;
				if (m==7) {
					cfg += hexformat(mask2,2);
					mask2 = 0;
				}
	...
	this.isRelayAssignNormalACpowerExclusive = function(relayNr) {
		...
		for (var k=0;k<15;k++){ //alarm[15] do not exist in any device
			var en = !(k==4||k==5); //ul pa fail and antenna disconnection do not exist in remotes
		...
		var alarmsToConsider=[];
		for (var k=0;k<8;k++){ 
			var en = !(k==1||k==5); //overload DL and Rx Power Low do not exist in remotes
			alarmsToConsider.push(en);
		}
		for (var k=0;k<this.bandAlarmsEnabled[0].length;k++){
			if (alarmsToConsider[k] && this.relayAssignBandAlarm[0][k][relayNr]) {
				return false;
			}
		}
...
function overallConfigToTextFile(config, nfpacfg, factory, tagstr, ethstr, serial){
	...
	if (!factory.limitTo32CH) cfg += fiberConfigToText(config, nfpacfg, factory) + "\n"; //only for DASCentric
	cfg += alarmSettingsToText(nfpacfg, config, factory) + "\n";
...
function alarmSettingsToText(nfpacfg, config, factory){
	var j,k;
	var exist_el;
	var cfg = "";
	...
	var bandAlarmNames = ["Overload UL","Overload DL","DL PA Fail","Tx Power Low","VSWR","RxPower Low / Donor Antenna", "DL AGC Fail","DL Output Power"];
	...
	for (k=0;k<15;k++) {
		if (k==4 || k==5 || (k>=8 && k<12)) { //remotos no tienen UL PA Fail ni Antenna Disconnection, no se reportan aquí las external inputs
			continue;
		}
		if (factory.limitTo32CH && (k>=12 && k<15)) continue; //General System Alarms does not existe in miniDAS
		cfg+="\tAlarm Enable " + gralAlarmNames[k] + ": " + boolToYN(nfpacfg.globalAlarmsEnabled[k]) + "\n";
	}
	for (k=0;k<8;k++){
		if (k==1) continue; //No hay Overload DL
		if (!factory.masterRxLowAlarm && k==5) continue; //report Rx Power Low of master if applicable
		if (factory.limitTo32CH && k==7) continue; //miniDAS does not have DL Output Power alarm
		cfg+="\tAlarm Enable " + bandAlarmNames[k] + ": " + boolToYN(nfpacfg.bandAlarmsEnabled[0][k]) + "\n";
	}
	...
	for (k=0;k<8;k++){
		if (k==1) continue; //No hay Overload DL ni Rx Power Low
		if (!factory.masterRxLowAlarm && k==5) continue; //report Rx Power Low of master if applicable
		if (factory.limitTo32CH && k==7) continue; //miniDAS does not have DL Output Power alarm
	...
	if (!factory.limitTo32CH){
		for (k=0;k<15;k++){
			if (k==4 || k==5 || (k>=8 && k<=14)) { //remotos no tienen UL PA Fail ni Antenna Disconnection, no se reportan aquí las external inputs ni general system alarm
				continue;
			}
...
function rfConfigToText(config, nfpacfg, factory){
	...
	for (i=0;i<2;i++){
		if (factory.chBandEnabled[i] || factory.adjBandEnabled[i]){
			...
			//RF Alarm Settings
			cfg += rfConfigAlarmToText(nfpacfg, factory, i);
		}
	}
	cfg += rfConfigGeneralAlarmToText(config, nfpacfg, factory);
	return cfg;
}
function rfConfigGeneralAlarmToText(config, nfpacfg, factory){
	var cfg = "";
	cfg+="CONFIG GENERAL:\n";
...
function fiberConfigToText(config, nfpacfg, factory){
	var cfg = "";
	if (!factory.limitTo32CH){
...
function rfConfigGeneralBandToText(config, factory, band){
	...
	cfg+="\tGeneral\n";
	if (!factory.limitTo32CH){
		for (j=0;j<2;j++) cfg+="\t\t"+uldl[j]+" "+inout[j]+" Attenuation (dB): " + (factory.gainlimit[2*band+j] - config.gain[band][j]) +"\n";
		cfg+="\t\tInput AGC per channel Composite Power Set " + uldl[0]+ " (dBm): " + (config.power[band][0]-config.gain[band][0])+"\n";
	}else{
		cfg+="\t\t"+uldl[0]+"  Main Gain (dB): " + (config.gain[band][0]) +"\n";
		cfg+="\t\t"+uldl[1]+" "+inout[1]+" Attenuation (dB): " + (factory.gainlimit[2*band+1] - config.gain[band][1]) +"\n";
		cfg+="\t\tPower Limit " + uldl[0]+ " (dBm): " + (config.power[band][0])+"\n";
	}
...
function rfConfigNarrowBandFiltersToText(config, factory, band){
	...
	if (factory.limitTo32CH) maxch/=2;
...
function rfConfigAlarmToText(nfpacfg, factory, band){
	var cfg = "";
