...
function parseGlobalConfig(str) {
	...
	localStorage.setItem("Factory_limitTo32CH"+Prjstr+window.location.host, factory.limitTo32CH); //se guarda este campo para el popup
					...
					if (!waitACK){
						...
						if ( monitor.isZeros ) {
							...
							if ( isGeneralFail ) {
								...
								if (monitor.factory_mini_das_mode != factory.limitTo32CH) {
									onloadInit();
								} else {
...
function submitform(isReset, isolVerif, isolClear, band, forceSend, forcePaOn, forcePaOff, setDefaultRelay) {
	...
	if (doSetDefaultRelay){ 
		...
		NFPAcfg.setDefaultRelayAssign(cnf.bbu_serial_mode);
...
function submitClearFiberErrorCounters(frmcnf) {
	if (typeof window.top.submitLocked === "undefined")
		window.top.submitLocked = false;
	if (window.top.submitLocked) {
		setTimeout(function() { guiBlocked(false); }, 15000);
