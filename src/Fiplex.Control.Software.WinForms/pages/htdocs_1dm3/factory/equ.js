...
function Equ() {
	...
	this.render = function() {
		for (var j = 0; j < 2; ++j) {
			for (k = 0; k < this.tableLen; ++k) {
				try {
					var el = document.getElementById('coeff'+j.toString()+k.toString());
					el.value = this.data[j][k];
				} catch (err) {}
			}
		}
	}
	...
	this.createForm = function() {
		...
		for (var i = 0; i < this.tableLen; ++i) {
			...
			for (var j = 0; j < 2; ++j) {
				...
				el.size = 10;
				el.className = "number";					
				el.id = 'coeff'+j.toString()+i.toString();
				el.name = el.id;
