...
function Spectrum(maxChNr) {
	this.parse = function(sr) {
		...
		for (var ch = 0, n = 0; ch <= this.nchannels && n < sr.length; ++ch) {
			try {
				...
				this.rawInput[ch] = (isNaN(num) ? dnYscale.min : to_float(num));
				n += 4;
			} catch (err) { }
		}
	...
	this.createBandLimitsForm = function() {
		...
		var center = ~~(Math.round((factory.data.band[0].fStartDL + factory.data.band[0].fStopDL)/2e5)*1e5);
		var span = ~~(Math.round((factory.data.band[0].fStopDL - factory.data.band[0].fStartDL)/1e5)*1e5);
