...
function createChannelEnable(nr) {
	...
	//lbl.setAttribute("class", "unselectable");
...
function createChFreqCtlBox(maxChannels, chMask) {
	...
	for (var i = 1; i <= maxChannels; ++i) {
		el = createChFreq(i, chMask);
		tab.appendChild(el);
	}
	return box;
}

function createFreqOffsetBox() {
	var chFreqOffsetTxt = ['0', '12.5'];
	var row = document.createElement("tr");
	var cell = document.createElement("td");
	row.appendChild(cell);
	cell.innerHTML = Texts['FREQOFFSET'];
	cell = document.createElement("td");
	row.appendChild(cell);
	var el = document.createElement("select");
	el.id = "freqChannelOffset";
	el.name = el.id;
	for (var i = 0; i < 2; i++) {
		var opt = document.createElement("option");
		el.options.add(opt);
		opt.text = chFreqOffsetTxt[i];
		opt.value = i;
	}
	el.selectedIndex = 0;
	el.onchange = function() { 
		for (var i = 1; i <= window.top.MaxChNr; ++i) {
			var fr = getFreqCh(i, 0);
			var num = config.computeChNum(fr, 0);
			if (isNaN(num)) {
				num = config.conf.channel[i];
				num <<= 1;
				num |= config.conf.freqOffset;
			}
			for (var j = 0; j < 2; ++j) {
				fr = config.computeChFreq(num, j);
				setFreqCh(i, j, fr);
			}
		}
	}
...
function setChannelFreqOffset(index) {
	try {
		var el = document.getElementById("freqChannelOffset");
		if (index >= 0 && index <= 1) {
...
function getChannelFreqOffset() {
	try {
		return document.getElementById("freqChannelOffset").selectedIndex;
	} catch(err) {}
}
...
function createChFreq(nr, chMask) {
	...
	for (i = 0; i < 2; ++i) {
		...
		fr.onchange = function(ev) {
			...
			var num = config.computeChNum(fr, target.path);
			if (isNaN(num)) {
				num = config.conf.channel[target.channel];
				num <<= 1;
				num |= config.conf.freqOffset;
			}
			for (var i = 0; i < 2; ++i) {
				fr = config.computeChFreq(num, i);
				setFreqCh(target.channel, i, fr);
			}
		}
		cell.appendChild(fr);
	}
...
function getFreqCh(nr, dn) {
	try {
		var el = document.getElementById("chFreq"+nr+(dn? 1: 0));
		return parseFloat(el.value) * 1.0e6;
...
function createMainGainLim(dn) {
	...
	el.className = "number";
	var tooltip = "Max: "+config.conf.band[dn].mainGainMax+", Min: "+
...
function createMainPowerLim(dn) {
	...
	el.className = "number";
	var tooltip = "Max: "+config.conf.band[dn].mainPowerMax+", Min: "+
		(config.conf.band[dn].mainPowerMax-config.conf.band[dn].mainPowerRange)+" dBm";
	el.title = tooltip;
	cell.appendChild(el);
	cell = document.createElement("td");
	row.appendChild(cell);
...
function createUnitReset() {
	...
	reset.onclick = function() { config.doReset(); } 
...
function createFineGainLim(nr, dn) {
	...
	var tooltip = "Max: 0, Min: "+config.conf.fineRange+" dB";
	el.title = tooltip;
	return box;
}
