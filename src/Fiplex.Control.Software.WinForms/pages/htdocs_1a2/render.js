...
function setChannelsDisplayEn() {
	...
	for (var i = 0; i < window.top.MaxChNr; ++i) {
		try {
			...
			el.checked = on;
			for (var j = 0; j < 2; ++j) {
				var f = document.getElementById("chFreq_"+c+"_"+j);
				var blocked = !on || (j == 0 && fsplitFixed);
				f.disabled = blocked;
				f.style.backgroundColor = blocked ? "#CCCCCC" : "white";
			}
		} catch (err) {}
	}
}
	...
	for (var i = 1, mask = 1; i <= window.top.MaxChNr; ++i) {
		try {
			...
			if (show)
				currentMask |= mask;
			for (var j = 0; j < 2; ++j) {
				var el = document.getElementById("chFreq_"+i+"_"+j);
				var blocked = !show || (j == 0 && fsplitFixed);
				el.disabled = blocked;
				el.style.backgroundColor = blocked ? "#CCCCCC" : "white";
			}
			mask <<= 1;
		} catch(err) { }
	}
...
function squelchEnSet(dn, on, tracking) {
	try {
		var el = document.getElementById("sqEn"+(dn? 1 : 0));
		var dis = (dn == 1 && tracking);
		var chk = on || dis;
		el.isSet = on;
		el.checked = chk;
		el.disabled = dis;
		el.style.backgroundColor = dis ? "grey" : "white";
	} catch (err) {}
}
