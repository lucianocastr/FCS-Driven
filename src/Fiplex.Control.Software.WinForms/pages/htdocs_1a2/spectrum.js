...
function Spectrum(maxChNr) {
	...
	this.computeInputCorrection = function(fr) {
		...
		if (idx < 0)
			return 0;
		var g2alpha = Math.pow(10, (monit.stat.band[1].ch[idx].gain - monit.stat.band[1].ch[idx].isolMod) / 20);
		var bw = config.conf.band[1].ch[idx].bandwidth;
		var tau = 0.0000116 * (bw + 1) + 0.0000034 + 0.000004 * factory.data.band[1].delayOffset;
		var ch = config.conf.band[1].ch[idx].frqNr;
		var ch_offset = ~~Math.round((frqNr - config.conf.band[1].ch[idx].frqNr)/fstepfactor);
		...
		var ph = monit.stat.band[1].ch[idx].isolPh * Math.PI / 180;
