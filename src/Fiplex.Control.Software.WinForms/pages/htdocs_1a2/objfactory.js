function Factory() {
	...
	this.frameMap = [
		...
		{id:'spectWarn',label:'PA&nbsp;UL&nbsp;CURRENT&nbsp;/&nbsp;SPECTRUM&nbsp;DL&nbsp;WARNING&nbsp;',len: 2, data: [], parse: function(s) {return cSignedByte(s);}, format: function(s) {return rSignedByte(s);} },
	...
	this.createForm = function() {
		...
		for (var i = 0; i < this.factbandmapLen; ++i) {
			if (this.frameMap[i].id == 'power') {
				...
				this.createForceULcurrentMonitor(mainTb);
	...
	this.createForceULcurrentMonitor = function(mainTb) {
		var row = document.createElement("tr");
		mainTb.appendChild(row);
		var cell = document.createElement("th");
		row.appendChild(cell);
		...
		el.type = "checkbox";
		el.id = el.name = "paUlCurrMonForce";
		el.className = "centered";
		cell.appendChild(el);
	}
	...
	this.render = function() {
		...
		try {
			...
			el = document.getElementById("paUlCurrMonForce");
			el.checked = this.data.pa_ul_curr;
		} catch(err) { }
	}
