...
var nrOfRemotes = 8; //provisional
var nGUIRemotes = 2;
var masterGlobalConfigLength = 2809; //flex2.0
var remoteGlobalConfigLength = 2445;
var expGlobalConfigLength = 1145;
var remoteBasicGlobalConfigLength = 343;
var expBasicGlobalConfigLength = 209;
var remoteHeaderLength = 6;
var masterConfigLength = 2172;
var expConfigLength = 598;
var remoteConfigLength = 1898;
var statMasterLength = 2350;
var statExpLength = 778;
var statRemoteLength = 1964;
var statBasicExpLength = 168;
var statBasicRemoteLength = 150;
var tagLength = 30;
var factoryLength = 482;
var eqLength = 384;
var remoteGlobalConfResponseValid = [];
var partialConfCommands = ["#","$","$","`","'"];
...
var last;
//window.top.pageloading = true;
function h(t) {
	/*if (t.id == 'start' || t.id == 'adv' || t.id == 'tags'){
		if (window.top.pageloading){
			alert("page not loaded");
			return; //navigation actions not allowed if page is not loaded
		}
	}*/
	...
	t.className = 'nh';
	t.blur();
	if (typeof window.top.monitorMode === "undefined")
		window.top.monitorMode = 0;
	else{
		window.top.monitorMode = 4;
		switch (t.id) {
			case 'start': window.top.monitorMode = 0; break;
			case 'adv': window.top.monitorMode = 1; break;
			case 'tags': window.top.monitorMode = 2; break;
			case 'spectrum': window.top.monitorMode = 3; break;
		}
	}
	if (t.id == 'start' || t.id == 'adv' || t.id == 'tags'){
		if ((last.id == 'start' || last.id == 'adv' || last.id == 'tags') && typeof(window.parent.content.forceReload) !== "undefined"){ //navigation between DAS Overview / Advanced Setting / Device List does not reload page
			//loadJS("/dummy.js"); //to force FCS to cancel current command if exists
			t.setAttribute('href','javascript:void(0)');
			window.parent.content.forceReload = true;
			if (window.parent.navi.isEthernetMode) {
	            window.parent.content.nextRequest();    // required by Rabbit to actually reload the page, otherwise it works randomly
			}
			//if (typeof(window.top.pageloading) === "undefined") window.top.pageloading = true;
			//window.top.pageloading = true;
		}else{
			t.setAttribute('href','start.zhtml');
		}
	}
	last = t;
}
...
function isKeyNumber(c) {
	return (c >= 48 && c <= 57);
}
function isKeyMinusSign(c) {
	return (c == 45		// hyphen
		|| c == 109	// numpad minus
		|| c == 189	// dash
		);
}
function isKeySignedNumber(ev) {
	ev = ev || window.event;
	var c = ev.which || ev.keyCode;
	var charcode = ev.charCode ? ev.charCode : ev.keyCode;
	if (isKeyNumber(c) || isKeyMinusSign(c)) {
		return true;
	} else {
		return charcode === 0;
	}
...
function showResultIcon(num) {
	try {
		...
		var txt = window.parent.navi.document.getElementById("cfgprog");
		...
		switch (num) {
		default:
		case ERR_NONE:  el.src = "/blank.gif"; txt.innerHTML = ""; break;
		case ERR_OK: 	el.setAttribute('style','transform:rotate(0deg)'); el.src = "/tick.png"; break;
		case ERR_FAIL: 	el.setAttribute('style','transform:rotate(0deg)'); el.src = "/cross.png"; break;
		case ERR_RELOAD: 	el.setAttribute('style','transform:rotate(0deg)'); el.src = "/reload.png"; break;
		case ERR_PENDING: el.src = "/processing.png"; waitIcon.redraw(); break;
		}
	} catch (err) {}
}
...
function guiBlocked(doBlock) {
	if (doBlock) {
		...
		if (typeof(blockRemotes) !== "undefined")
			blockRemotes();
		if (typeof(refreshEnables) !== "undefined")
			refreshEnables();
		if (typeof(refreshNfpaEnables) !== "undefined")
			refreshNfpaEnables();
		cursorClear();
