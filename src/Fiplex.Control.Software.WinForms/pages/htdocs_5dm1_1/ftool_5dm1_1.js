function ftoolGui() {
	...
	this.initialize = function() {
		...
		this.fibInfo = new fiberInfo(0);
		this.basicCfg = [];
		this.validCfg = [];
		var errloading = false;
		var str = localStorage.getItem("GConf"+Prjstr+window.location.host);
		if (typeof(str) !== 'undefined' && str !== null) {
			var deviceStr = str.split('\t\t\t');
			this.validCfg[0]=self.parseGlobalConfigUnit(deviceStr[0].substr(6));
		}
		if (this.validCfg[0]){
			for (var k=1;k<deviceStr.length;k++){
				...
				if (this.fibInfo.FOmoduleConnected[indexRemote]){
					if (deviceStr[k].length < 342) {
						this.validCfg[indexRemote] = false;
					}else{
						this.validCfg[indexRemote] = (1 === parseInt(deviceStr[k].substring(4,6),16));
						if (this.validCfg[indexRemote]){
							this.basicCfg[indexRemote] = new BasicConf(indexRemote);
							this.basicCfg[indexRemote].parse(deviceStr[k].substr(6));
						}
					}
				}
			}
		}else{
			alert("Error retrieving DAS info");
			window.close();
			return;
		}
		...
		for (var band=0;band<2;band++)
			this.toolband.push(this.factory.chBandEnabled[band]);
		...
		this.BWKHZ = this.factory.commonUl ? 150:100;
		this.bwIndex = this.factory.commonUl ? 0:1;
		this.type = this.factory.commonUl ? 1:0;
	...
	this.parseGlobalConfigUnit = function(str){
		var srarr = str.split('\t');
		if (srarr.length < 7) return false;
		this.factory.parse(srarr[1]);
		this.config.parse(srarr[0],this.factory);
		this.fibInfo.parse(srarr[5]);
		return true;
	}
	...
	this.makeSubmitFrm = function(band) {
		self.frms = [];
		var header = "000001";
		self.frms.push({type: 'ctl_conf_str=', frame: header+self.config.getFrm(self.factory)});
		var serialAlreadyConf = [];
		if (self.config.uldlLinkedFreq[band]){
			for (var nr in self.fibInfo.FOmoduleConnected){
				if ((nr & 0xff)==0) continue;//discarding master and expansions
				if (self.fibInfo.FOmoduleConnected[nr] && self.validCfg[nr]){
					var isAlreadyConf= false;
					for (var k=0;k<serialAlreadyConf.length;k++){
						if (serialAlreadyConf[k]==self.basicCfg[nr].serNr.sernr){
							isAlreadyConf = true;
							break;
						}
					}
					if (isAlreadyConf) continue; //discard redundant remotes
					serialAlreadyConf.push(self.basicCfg[nr].serNr.sernr);
					header = partialConfCommands[1];//freq partial frame
					header += hexformat(nr,4) + "01";//header + valid
					header += self.band==0?"03":"0C";//added flags with band freqs to modify
					var cfg = new Config(1);
					cfg.genConfigFilterULFromDL(self.config,self.factory,self.basicCfg[nr]);
					var frmFreq = cfg.getFrmFilterWithBasicConfig(self.factory,self.basicCfg[nr]);
					self.frms.push({type: 'ctl_conf_str=', frame: header+frmFreq});
				}
			}
		}
	...
	this.saveSettingVals = function() {
		...
		&& this.config.uldlLinkedFreq[0] && this.config.uldlLinkedFreq[1]){
