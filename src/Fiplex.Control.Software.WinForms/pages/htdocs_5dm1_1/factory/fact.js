...
function factReq(fw) {
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState === 4 && this.status === 200) {
				setTimeout(function() { load_fact(); }, computeTimeoutReqMs());
			}
		}
		xhr.onerror = function(ev) {
			if (typeof(timerId) !== "undefined" && timerId)
				clearTimeout(timerId);
			timerId = setTimeout(function() { factReq(fw); }, 2000);
		}
		xhr.ontimeout = function(ev) {
			if (typeof(timerId) !== "undefined" && timerId)
				clearTimeout(timerId);
			timerId = setTimeout(function() { factReq(fw); }, 100);
		}
		xhr.open("POST", "/start.zhtml", true);
		xhr.timeout = 5000;
		var p = "fact_fw="+hexformat(fw,2)+"&fact_req=1";
		xhr.send(p);
		xhr = null;
	} catch (err) {}
}
	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState == 4 && this.status == 200) {
				...
				if (serverResponse.length<factoryLength){
...
function parseGlobalConfig(str) {
	...
	// master
	parseGlobalConfigUnit(deviceStr[0].substr(remoteHeaderLength), 0);
	serNr.render();
	version.render();
}
...
function clearDeepDischargeButtonClick(ev){
	clearBBUCounterAction = true;
}
function submitOnClearDeepDischargeButton() {
	if (typeof window.top.submitLocked === "undefined")
		window.top.submitLocked = false;
	if (window.top.submitLocked) {
		setTimeout(function() { guiBlocked(false); }, 15000);
	...
	xhrOnStart();
	var fact = new Factory(factory.frm);
	fact.clearDeepDischargeCounter = true;
	var frm = fact.getFrm();
	var currentFw = ~~(localStorage.getItem("FW"+Prjstr+window.location.host));
	var factToConfigure = (document.getElementById("fwSel").selectedIndex + currentFw) % 2;
	frm = hexformat(factToConfigure,2) + frm;
...
function statusReq() {
	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState === 4 && this.status === 200) {
				countReqs = 0;
				timerId = setTimeout(function() { load_status(); }, postGetTimeout);
			}
		}
		xhr.onerror = function(ev) {
			if (typeof(timerId) !== "undefined" && timerId)
				clearTimeout(timerId);
			timerId = setTimeout(function() { statusReq(); }, 2000);
		}
		xhr.ontimeout = function(ev) {
			if (typeof(timerId) !== "undefined" && timerId)
				clearTimeout(timerId);
			timerId = setTimeout(function() { statusReq(); }, 100);
		}
		xhr.open("POST", "/start.zhtml", true);
		xhr.timeout = 5000;
		var params = hexformat(0,4);
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState ===4 && this.status === 200) {
				if (this.responseText.length<remoteHeaderLength){ //fcs returns string "WAIT" if USB response is not ready
					...
					tmrIdFact = setTimeout(function() { nextRequest(); }, 200);
				}
			}
		}
		xhr.onerror = function(ev) {
			if (typeof(tmrIdFact) !== "undefined" && tmrIdFact)
				clearTimeout(tmrIdFact);
			tmrIdFact = setTimeout(function() { statusReq(); }, 1000);
		}
		xhr.ontimeout = function(ev) {
			if (typeof(tmrIdFact) !== "undefined" && tmrIdFact)
				clearTimeout(tmrIdFact);
...
function nextRequest(){
	if (forceReload){
		forceReload = false;
		reloadPage();
	}else if(clearBBUCounterAction){
		clearBBUCounterAction = false;
		submitOnClearDeepDischargeButton();
	}else if(submitConf){
		submitConf = false;
		submitConfig();
	}else{
...
function GuiFact() {
	...
	this.showStatus = function(mon) {
		try{
			...
			el.value = mon.paCurrent[0][0].toFixed(0)+'mA';
	...
	this.shMasterOTA = function(fact, b) {
		var el = document.getElementById('masterOTA'+'_'+b);
		el.selectedIndex = (fact.masterOTA? 1 : 0);
	}
	...
	this.rdPwrLim = function(fact, b) {
		var el = document.getElementById('powerLim'+'_'+b);
		fact.powerlimit[b] = parseInt(el.value);
	}
	...
	this.rdMasterOTA = function(fact, b) {
		var el = document.getElementById('masterOTA'+'_'+b);
		fact.masterOTA = (el.selectedIndex != 0);
	}
	...
	this.shADummyThresholdOff = function(fact, b) {
		var el = document.getElementById('previsionThresholdDown'+'_'+b);
		el.value = fact.previsionThresholdDown[b];
	}
	...
	this.rdADummyThresholdOff = function(fact, b) {
		var el = document.getElementById('previsionThresholdDown'+'_'+b);
		var value = parseInt(el.value);
		if (value < 0) {
			value = 0;
		...
		fact.previsionThresholdDown[b] = value;
	}
	...
	this.Map = [
		[
			...
			},
			{
				...
				label:'MASTER&nbsp;TYPE&nbsp;',
				type: 'select',
				options: [["RACK MOUNT", "OVER THE AIR"]],
				show: this.shMasterOTA,
				read: this.rdMasterOTA,
				len: 1
			},
			{
				id:'opfEnable',
			...
			},
			{
				...
				type: 'input',
				inputtype: 'button',
				onclick: clearDeepDischargeButtonClick,
				value: "CLEAR",
		...
		[	
			...
			},
			{
				...
				display: true,
				label:'ANALOG&nbsp;DUMMY&nbsp;THRESHOLD&nbsp;ON&nbsp;',
				type: 'input',
				inputtype: 'text',
				onkey: isKeyDecimalNumber,
			...
			{
				id:'previsionThresholdDown',
				display: true,
				label:'ANALOG&nbsp;DUMMY&nbsp;THRESHOLD&nbsp;OFF&nbsp;',
				type: 'input',
				inputtype: 'text',
				onkey: isKeyDecimalNumber,
				show: this.shADummyThresholdOff,
				read: this.rdADummyThresholdOff,
				len: 2
			}
		],
		[				
		...
		[
			...
			},
			{
				id:'calPaCurr',
				display: true,
				label:'PA&nbsp;CURRENT&nbsp;DETECTOR&nbsp;(UL BAND 0)',
				type: 'input',
				inputtype: 'text',
				onkey: isKeyDecimalNumber,
