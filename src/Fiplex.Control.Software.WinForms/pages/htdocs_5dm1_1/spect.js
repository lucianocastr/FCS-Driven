...
var postGetTimeout = 100;
var forceReload = false;
var spectSettingsRequested;
var sizeOfSpectSettings = 24;
...
function parseGlobalConfig(){
	...
	if (typeof(window.parent.navi.confFrame) !== 'undefined') {
		var str = window.parent.navi.confFrame;
		var deviceStr = str.split('\t\t\t');
		if (parseGlobalConfigUnit(deviceStr[0].substr(remoteHeaderLength))){
			for (var k=1;k<deviceStr.length;k++){
				var indexRemote = parseInt(deviceStr[k].substring(0,4),16); if (indexRemote<0) indexRemote+=65536;
				if ((indexRemote & 0xff)==0) continue;//discarding expansion
				if (this.fibInfo.FOmoduleConnected[indexRemote]){
					if (deviceStr[k].length < remoteBasicGlobalConfigLength+remoteHeaderLength) {
						validCfg[indexRemote] = false;
					}else{
						validCfg[indexRemote] = (1 === parseInt(deviceStr[k].substring(4,6),16));
						if (validCfg[indexRemote]){
							basicCfg[indexRemote] = new BasicConf(indexRemote);
							basicCfg[indexRemote].parse(deviceStr[k].substr(remoteHeaderLength));
						}
					}
				}
			}
		...
		alert("Error retrieving DAS config info");
		window.parent.navi.document.getElementById("start").click();
		return;
	}
}
...
function navigateToOverview(){
	alert("DAS is being re-configured by another instance. This operation cannot be done while this GUI is in Spectrum section");
	setTimeout(function() {
		window.parent.navi.document.getElementById("start").click();
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState == 4 && this.status == 200) {
				...
				if (this.responseText.length<remoteHeaderLength){ //fcs returns string "WAIT" if USB response is not ready
					...
					window.parent.navi.confFrame = this.responseText;
					parseGlobalConfig();
					spectrum.createDeviceSelect();
					nextRequest();
				}
...
function parseGlobalConfigUnit(str){
	var srarr = str.split('\t');
	if (srarr.length < 7) return false;
	factory.parse(srarr[1]);
	config.parse(srarr[0],factory);
...
function request_spectrum() {
	try {
		...
		xhr = null;
		spectSettingsRequested = spectSettings;
	} catch (err) {}
}
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState==4) {
				if (this.status === 200) {
					...
					if (serverResponse.indexOf("RCFG") >= 0) { //Load file operation: GUI goes to DAS Overview
						navigateToOverview();
						return;
					}
					if (serverResponse == "NACK") {
						var dummyResponse = strRepeat("80", spectrum.nchannels);
						spectrum.parse(dummyResponse);
	 					showResultIcon(ERR_NONE);
	 					guiBlocked(false);
						showRefresh();
					} else {
						var expectedLength = spectrum.nchannels * 2;
						if (window.parent.navi.isEthernetMode) expectedLength += sizeOfSpectSettings;
						if (serverResponse.length < expectedLength) {
							...
							if (window.parent.navi.isEthernetMode) {
								var spectSettingsReceived = serverResponse.substr(0, sizeOfSpectSettings);
								var isError = (spectSettingsReceived != spectSettingsRequested);
							}
							if (!isError) {
								if (window.parent.navi.isEthernetMode) {
									spectrum.parse(serverResponse.substr(sizeOfSpectSettings));
								} else {
				...
				tmrId = setTimeout(function() { nextRequest(); }, Math.floor(100+Math.random()*900));
			}
		}
		xhr.onerror = function(ev) {
			if (typeof(tmrId) !== "undefined" && tmrId)
...
function Spectrum() {
	this.parse = function(sr) {
		if (!this.spectBox || this.bandChanged || this.resBwChanged 
		    ...
		    || this.bandSelectorChanged || this.deviceSelectorChanged) {
			var dnXscale = {
				min: this.FreqMinHz,
				max: this.FreqMaxHz,
		...
		if (this.chainChanged || this.bandSelectorChanged || this.deviceSelectorChanged) {
			this.inputSpectrum = [];
			this.outputSpectrum = [];
			//para evitar pintar espectros en la transición
			this.deviceSelectorChanged = false;
			this.bandSelectorChanged = false;
		...
		// if (this.previousIo == -1) {
		// 	this.previousIo = this.io;
		// 	this.inputSpectrum = [];
		// 	this.outputSpectrum = [];
		// 	this.render(this.spectBox, null, null);
		// 	return;
		// }
		if (this.io == 0) {
			this.inputSpectrum = [];
	...
	this.createChannelBars = function(box) {
		var b = this.chain;
		for (var c = 0; c < config.CHNR; ++c) {
			if (config.filterEnabled[0][0][b][c]) {
				continue;
			}
			var fr = config.freqHz[0][b][c];
	...
	this.computeSpectBandLimits = function(b, band) {
		var fm = [];
		var fmin, fmax; 
		fmin = factory.fstart[2*band+b];
		fmax = factory.fstop[2*band+b];
		fm.push(fmin);
		fm.push(fmax);
		return fm;
	}
	this.createBandSelect = function() {
		var box = document.getElementById("bandSelect");
		if (!box) {
			return;
		}
		...
		for (var band=0;band<2;band++){
			...
			el.options.add(opt);
			opt.text = factory.bandNames[band];
			opt.value = band;
		}
		el.value = this.band;
		for (var band=0;band<2;band++){
		...
		el.onchange = function(ev) {
			self.band = this.value;
			localStorage.setItem("spect_band"+Prjstr+window.location.host, self.band);
			self.initSettings();
			self.setBandLimits(self.FreqMinHz, self.FreqMaxHz);
			self.bandSelectorChanged = true;
		}
	}
	this.createDeviceSelect = function() {
		var box = document.getElementById("deviceSelect");
		if (!box) {
			return;
		}
		var el = document.getElementById("selDevice");
		if (el!=null) remove_element(el);
		var el = document.createElement("select");
		box.appendChild(el);
		el.id = "selDevice";
		el.name = el.id;
		for (var deviceNr in fibInfo.FOmoduleConnected){
			var n1 = deviceNr & 0xff; var n2 = deviceNr>>8 & 0xff;
			if (n1==0) continue; //discard master and expansions
			if (!fibInfo.FOmoduleConnected[deviceNr] || !validCfg[deviceNr]) continue;
			var opt = document.createElement("option");
			el.options.add(opt);
			opt.text = n2+"."+n1+".- "+basicCfg[deviceNr].tag;
			opt.value = deviceNr;
		}
		var nval = localStorage.getItem("spect_remote"+Prjstr+window.location.host);
		if (nval==null || typeof(nval)==="undefined") nval = 1;
		el.value = nval;
		if (el.value==""){
			el.selectedIndex = 0;
			nval = el.value;
		...
		self.deviceNr = nval;
		localStorage.setItem("spect_remote"+Prjstr+window.location.host,nval);
		el.onchange = function(ev) {
			localStorage.setItem("spect_remote"+Prjstr+window.location.host,this.value);
			self.deviceNr = this.value;
			self.initSettings();
			self.deviceSelectorChanged = true;
		}
	}
	this.createBandLimitsForm = function() {
		...
		var b = this.chain;
		var band = this.band;
		var fm = this.computeSpectBandLimits(b, band);
		...
		for (var i = 0; i < 2; ++i) {
			...
			el.onchange = function(ev) {
				...
				localStorage.setItem("spect_fs"+Prjstr+self.band+self.chain+window.location.host, self.newFreqMinHz);
				localStorage.setItem("spect_fp"+Prjstr+self.band+self.chain+window.location.host, self.newFreqMaxHz);
			}
	...
	this.reSetBandLimits = function(ev) {
		...
		if (self.zSpanSt) {
			...
			var n = ~~Math.round((fi - factory.fref[2*band+b])/this.FREQ_STEP_HZ);
			fi = n*this.FREQ_STEP_HZ + factory.fref[2*band+b];
		} else {
	...
	this.getSettings = function() {
		...
		*/
		...
		s |= this.band == 1 ? this.settingsBits.band : 0;
		r += hexformat(s, 2);
		s = ((this.avg - 1) & this.settingsBits.avg);
		r += hexformat(s, 2);
		var dev = ((this.io == 1) ^ (this.chain == 1))?0:this.deviceNr;
		r += hexformat(dev, 4);
		return r;
	}
	this.initSettings = function() {
		...
		if (isNaN(band) || band < 0 || band > 1) {
			band = 0;
			localStorage.setItem("spect_band"+Prjstr+window.location.host, band);
		}
		this.band = band;		
		...
		if (!isNaN(fi) && !isNaN(fp) && fp >= fi
			...
			localStorage.setItem("spect_fs"+Prjstr+band+b+window.location.host, this.newFreqMinHz);
			localStorage.setItem("spect_fp"+Prjstr+band+b+window.location.host, this.newFreqMaxHz);
		}
		...
		this.ioSet = [];
		for (var i = 0; i < 2; ++i) {
			var io = parseInt(localStorage.getItem("spect_io_"+i+Prjstr+window.location.host));
			if (isNaN(io) || (io != 0 && io != 1)) {
				io = 1;
				localStorage.setItem("spect_io_"+i+Prjstr+window.location.host, io);
			}
			this.ioSet.push(io);
		}
	}
	...
	this.MAXRES = factory.commonUl ? 5:6; //RBWmax de 100KHz en V/U
	this.MAXAVG = 32;
