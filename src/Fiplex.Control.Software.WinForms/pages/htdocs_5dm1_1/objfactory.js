 function Factory(factoryFrame) {
	...
	this.ULlowGainMode			= false; //sólo se usará en los remotos, no en master
	this.testPowerLoopMode		= false;
	this.FASrelayTestMode		= false;
	this.masterOTA				= false; //true-master OTA, false-master rack mount
	...
	this.previsionThresholdDown	= [0, 0];
	...
	this.deepDischargeVolt_mV		= 20.4;
	this.unusedCoefPaCurrentDetector 	= 0;
	this.unusedCoefBoardCurrentDetector 	= 0;
	this.clearDeepDischargeCounter;
	
	this.relayInstalled				= 0;
	this.relayModeConfiguration 	= 0;
	
	//new flex2.0 parameters (all belonging to remotes)
	this.plaAvailable 				= false;
	this.plaBand 					= 0; //parameter to define path loss an. measurement band: 0/1
	this.plaMeasTimeMs 				= 0; //path loss an. measurement time in milliseconds
	this.agcDeltaHybridMode 		= 0;
	...
	this.parse = function(s) {
		...
		for (b=0;b<2;b++){
			...
			if (b==0) {
				...
				temp= parseInt(s.substring(ind,ind+2), 16); ind+=2;
				this.masterOTA = (temp & 0x1)==0;
			}
			if (b==1) {
				this.FASrelayTestMode = (temp & 0x04)!=0;
				this.clearDeepDischargeCounter = false;
				this.plaAvailable = (temp & 0x20)!=0;
				this.plaBand = (temp>>6) & 0x1;
				this.plaMeasTimeMs = parseInt(s.substring(ind,ind+2), 16); ind+=2;
			}
		...
		for (var i=0;i<2;i++){
			...
			this.previsionThresholdDown[i] = parseInt(s.substring(ind, ind+2), 16); ind += 2;
		}
		...
		this.deepDischargeVolt_mV = parseInt(s.substr(ind,4), 16); ind+=4;
		this.calPaCurrentDetector.parse(s.substr(ind,16)); ind += 16;
		this.agcDeltaHybridMode = to_ufloat(parseInt(s.substr(ind,4), 16)); ind+=4;
	...
	this.getFrm = function(){
		...
		for (var b=0;b<2;b++){
			...
			if (b==0) {
				...
				cal+=hexformat(mask,2);
				mask = 0;
				if (!this.masterOTA) mask=0x1;
				cal+=hexformat(mask,2);
			}
			if (b==1) {
				if (this.FASrelayTestMode) mask|=0x04;
				if (this.clearDeepDischargeCounter) mask|=0x10;
				if (this.plaAvailable) mask|=0x20;
				mask |= (this.plaBand & 0x1)<<6;
				cal+=hexformat(mask,2);
				cal+=hexformat(this.plaMeasTimeMs,2);	
			}
		...
		for (var i=0;i<2;i++) {
			...
			cal += hexformat(this.previsionThresholdDown[i],2);
		}
		for (var b = 0; b < 2; b++) {
		...
		cal += hexformat(this.deepDischargeVolt_mV,4);
		cal += this.calPaCurrentDetector.getFrm();
		cal += hexformat(double_to_uint(this.agcDeltaHybridMode),4);
