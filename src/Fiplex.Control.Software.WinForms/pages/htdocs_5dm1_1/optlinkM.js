...
function optOnLoadFunc() {
	...
	var n1=nr & 0xff; var n2=(nr>>8)&0xff;
	var titl = nr==0?"MASTER":(n1==0?"EXPANSION "+n2+".0":("REMOTE "+n2+"."+n1));
	titl += " PORT " + (~~port+1)+" OPTICAL INFO";
	hdr.innerHTML = titl;
	mon = new Monitor(nr);
	mon.parse(localStorage.getItem("Status_"+nr+"_"+Prjstr+window.location.host));
	renderOpticalTable();
	optLoadStatus();
}
...
function showMonitor() {
	...
	num = mon.fibInfo.FOActiveOpticalLink;
	document.getElementById("optActLnk").src = num==0?"/bullet_green.png":"/bullet_green.png";
