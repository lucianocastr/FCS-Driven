...
function Spectrum() {
	...
	this.createBandLimitsForm = function() {
		...
		for (var i = 0; i < 2; ++i) {
			...
			el.onchange = function(ev) {
				...
				if (self.zSpanSt) {
					localStorage.setItem("spect_fz_1dr2_1_"+self.chain+window.location.host, self.newFreqZspanHz);
				} else {
					localStorage.setItem("spect_fs_1dr2_1_"+self.chain+window.location.host, self.newFreqMinHz);
					localStorage.setItem("spect_fp_1dr2_1_"+self.chain+window.location.host, self.newFreqMaxHz);
				}
			}
	...
	this.createRbwForm = function() {
		...
		el.onchange = function(ev) {
			localStorage.setItem("spect_rbw_1dr2_1_"+window.location.host, this.selectedIndex);
		}
		var t = document.createElement("span");
		t.innerHTML = str;
		t.style.fontWeight = "bold";
	...
	this.createAvgForm = function() {
		...
		el.onchange = function(ev) {
			...
			this.value = v;
			localStorage.setItem("spect_avg_1dr2_1_"+window.location.host, v);
		}
		var t = document.createElement("span");
		t.innerHTML = "Avg.";
		t.style.fontWeight = "bold";
	...
	this.createZspanForm = function() {
		...
		el.onclick = function(ev) {
			localStorage.setItem("spect_zspan_1dr2_1_"+window.location.host, this.checked ? 1 : 0);
	...
	this.getSettings = function() {
		...
		if (this.chainChanged) {
			localStorage.setItem("spect_chain_1dr2_1_"+window.location.host, b);
			this.changeChain();
		}
	...
	this.initSettings = function() {
		var b = parseInt(localStorage.getItem("spect_chain_1dr2_1_"+window.location.host));
		if (isNaN(b) || b < 0 || b > 1) {
			b = 1;
			localStorage.setItem("spect_chain_1dr2_1_"+window.location.host, b);
		}
		...
		var fz = parseInt(localStorage.getItem("spect_fz_1dr2_1_"+b+window.location.host));
		if (isNaN(fz) || fz < fStart || fz > fStop) {
			fz = fStart;
			localStorage.setItem("spect_fz_1dr2_1_"+b+window.location.host, fz);
		}
		this.FreqZspanHz = this.newFreqZspanHz = fz;
		var zs = parseInt(localStorage.getItem("spect_zspan_1dr2_1_"+window.location.host));
		if (isNaN(zs) || zs < 0 || zs > 1) {
			zs = 0;
			localStorage.setItem("spect_zspan_1dr2_1_"+window.location.host, zs);
		}
		this.zSpanSt = zs != 0;
		var rbw = parseInt(localStorage.getItem("spect_rbw_1dr2_1_"+window.location.host));
		if (isNaN(rbw) || rbw < 0 || rbw >= this.MAXRES) {
			rbw = this.MAXRES - 1;
			localStorage.setItem("spect_rbw_1dr2_1_"+window.location.host, rbw);
		}
		this.resBw = rbw;
		this.spanStep = this.resBw;
		var v = parseInt(localStorage.getItem("spect_avg_1dr2_1_"+window.location.host));
	...
	this.computeBand = function(b) {
		...
		if (b == 0) {
			...
			var bandDL = factory.computeFrefDl();
			fRef = bandDL.fREFDL;
			var fStart = bandDL.fREFDL - bandDL.bwDL;
			var fStop = bandDL.fREFDL + bandDL.bwDL;
		}
		var fi = parseInt(localStorage.getItem("spect_fs_1dr2_1_"+b+window.location.host));
		var fp = parseInt(localStorage.getItem("spect_fp_1dr2_1_"+b+window.location.host));
		if (!isNaN(fi) && !isNaN(fp) && fp >= (fi+200000) //revisar zspan
			...
			fmin = fStart;
			fmax = fStop;
			localStorage.setItem("spect_fs_1dr2_1_"+b+window.location.host, fmin);
			localStorage.setItem("spect_fp_1dr2_1_"+b+window.location.host, fmax);
		}
...
function Tags() {
	...
	var tagstr = localStorage.getItem("tags_1dr2_1_"+window.location.host);
