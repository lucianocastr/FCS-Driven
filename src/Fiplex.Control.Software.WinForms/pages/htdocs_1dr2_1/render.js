...
function createFilteringTable(nr) {
	...
	filtBox.appendChild(createBackupSel());
...
function createFilterCtlTable(nr) {
	...
	if (version.isFwNewSpect() && factory.data.persistEnable) {
		tdNode = document.createElement("th");
...
function createFiberTable() {
	...
	tdNode.style.textAlign = "right";
	tdNode.style.whiteSpace = "nowrap";
	tdNode = createLedBox("optFrSync1");
	trNode.appendChild(tdNode);
	if (twofibers){
		tdNode = createLedBox("optFrSync2");
		trNode.appendChild(tdNode);
	}
	trNode = document.createElement("tr");
	tbodyNode.appendChild(trNode);
...
function fiberPowerSet(nr, val, color) {
	try {
...
function createClearCounters() {
	...
	var doshow = true;
	if (version.compareSwMajor("02") < 0) {
		doshow = false;
	} else if (version.compareSwMajor("02") == 0) {
		if (version.compareSw("0220") < 0) {
			doshow = false;
		}
	} else if (version.compareSwMajor("03") == 0) {
		if (version.compareSw("0309") < 0) {
			doshow = false;
		}
	}
	if (!doshow) {
