...
function Factory() {
	...
	this.createForm = function(version) {
		var Texts = TextEn;
		var sne;
		sne = (typeof(version) !== "undefined" && version.swGTnpr());
		...
		for (var i = this.factbandmapLen; i < this.frameMap.length; ++i) {
			...
			if (this.frameMap[i].id == 'pmax' && this.showNewElements) {
		...
		try {
			...
			for (var n = this.factbandmapLen; n < this.frameMap.length; n++) {
				...
				if (this.frameMap[n].id == 'fmodulo') 		num = parseInt(el.value);
					...
					num = parseInt(el.value, 16); 
					num &= (this.asmOptionBits.noHpaMonitorBit | this.asmOptionBits.nfpaMonitorBit);
					el = document.getElementById('allowDisFilt');
