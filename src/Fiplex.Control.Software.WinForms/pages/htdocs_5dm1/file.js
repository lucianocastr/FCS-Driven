...
var saveFileCmd = 
[
	{get: "/updt_comm.shtml", post:"nosend" , postReg: "undefined", len:  157},
...
[
	...
	{post:"np_settings_str" , postReg: "", len:  157, type: NORMAL, device: DEVMASTER},
	{post:"ctl_conf_str" , postReg: "conf_req", len: remoteHeaderLength + masterConfigLength, type: CHECKHEADER, device: DEVMASTER},
	{post:"nfpa_str" , postReg: "nfpa_req", len:  remoteHeaderLength + masterNfpaConfigLength, type: CHECKHEADER, device: DEVMASTER},
	{post:"ctl_tags_str" , postReg: "tags_req", len:  tagLength*(1+nrOfRemotes), type: CHECKSUBTAG, device: DEVMASTER}

];
for (var k=0;k<nrOfRemotes;k++){
	loadFileCmd.push({post:"ctl_conf_str" , postReg: "conf_req", len: remoteHeaderLength + remoteConfigLength, type: CHECKHEADER, device: DEVREMOTE});
	loadFileCmd.push({post:"nfpa_str" , postReg: "nfpa_req", len:  remoteHeaderLength + remoteNfpaConfigLength, type: CHECKHEADER, device: DEVREMOTE});
}
...
function checkFile(fdatafr){
	...
	for (var k=0;k<fdatafr.length;k++){
		if (loadFileCmd[k].type==CHECKHEADER){ //config o alarm config
			if (loadFileCmd[k].device == DEVMASTER && loadFileCmd[k].len!=fdatafr[k].length) return false; //exact length is required
			if (loadFileCmd[k].device == DEVREMOTE && loadFileCmd[k].len>fdatafr[k].length) return false; //minimum length is required
