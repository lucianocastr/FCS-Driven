...
function parseGlobalConfigUnit(str, n) {
	...
	if (n == 0) {
		...
		config.parse(srConf,factory,0);
		gui.createForm();
		gui.show(factory);
		localStorage.setItem("Factory_0_"+Prjstr+window.location.host, srFact);
	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState ===4 && this.status === 200) {
				...
				gui.showPaUlCurr(monitor.monitorUnit[0]);
...
function load_NFPAstatus() {
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState ===4 && this.status === 200) {
				var deviceStr = this.responseText.split(frameSeparator);
				monitorNFPA.parse(deviceStr[0], config.bbu_type);
