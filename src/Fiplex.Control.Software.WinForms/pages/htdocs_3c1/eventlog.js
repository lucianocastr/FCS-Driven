...
function EventLog() {
	...
	this.Gui = function() {
		...
		this.getEventName = function(rtype, factory, NFPAcfg) {
			var alarmName = "";
			var NrAlarmsGeneral = NFPAcfg.alarmNames.length;
			...
			if ( index >= 0 && index < NrAlarmsGeneral ) {
				...
				if (NFPAcfg.isIndexBandAlarmB0(index)) {
					alarmName = alarmName.replace("B0", factory.bandNames[0]);
				} else if (NFPAcfg.isIndexBandAlarmB1(index)) {
					alarmName = alarmName.replace("B1", factory.bandNames[1]);
				}
			} else if (index < (NrAlarmsGeneral+NrAlarmsBbu+1)) {
				index -= (NrAlarmsGeneral+1);	// skip PA ON/OFF event
