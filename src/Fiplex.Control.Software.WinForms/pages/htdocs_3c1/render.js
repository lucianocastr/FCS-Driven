...
function Page() {
	...
	this.createAlarmTableBand = function(){
		...
		for (var band=0;band<2;band++){
			...
			row.appendChild(cell);
			for (var k=0;k<self.NFPA.NUMALARM;k++){
				if (self.NFPA.isUndefinedAlarm(k)) {
					continue;
				}
				if (!self.NFPA.isBandAlarm(k)) {
					continue;
				}
				if ( (band == 0 && self.NFPA.isBandAlarmB1(k))
					|| (band == 1 && self.NFPA.isBandAlarmB0(k)) ) 
				{
					continue;
				}
				var n = self.NFPA.naturalOrder[k];
				var alarmName = self.NFPA.alarmNames[n];
				if (band == 0) {
					alarmName = alarmName.replace(" B0", "");
				} else {
					alarmName = alarmName.replace(" B1", "");
				}
				// tb.appendChild(this.createAlarmLed(k,alarmName,show && true));
				tb.appendChild(this.createAlarmLed(k,alarmName,show));
			}
		}	
		return cellb;
	}
	this.createAlarmTable = function(){
		...
		for (var k=0;k<self.NFPA.NUMALARM;k++){
			...
			var n = self.NFPA.naturalOrder[k];
			// tb.appendChild(this.createAlarmLed(k,self.NFPA.alarmNames[n],true));
			var show = k<8 || self.NFPA.alarmInstalled[n];
			tb.appendChild(this.createAlarmLed(k,self.NFPA.alarmNames[n],show));
		}
	...
	this.createShowFiltAdjTable = function(band) {
		...
		// 	var head = document.createElement("th");
		// 	row.appendChild(head);
		// 	head.innerHTML = "Assign 1st ADJBW to FirstNet";
		// 	head.className = "thdrht";
		// 	var cell = document.createElement("td");
	...
	this.createAlarmLed = function(num,txt,show){
		var row = document.createElement("tr");
		var cell = document.createElement("th");
		cell.style.width = "70%";
		...
		cell.innerHTML = txt;
		cell.className = "thdrht";
		cell = createLedBox("LedAlarm_"+num);
		cell.style.paddingLeft = "10px";
		cell.style.paddingRight = "10px";
		row.appendChild(cell);
		row.style.display = show?"table-row":"none";
	...
	this.ledAlarmSet = function(num,alarm) {
		ledSetColor("LedAlarm_"+num, alarm ? "red" : "grey");
	}
	...
	this.BWtable = [
		{ix:   0, include: false, value:   0, txt: "150KHz", khz:  150.0}, //no hay 150KHz
	...
	this.createChBw = function(b, c, band) {
		...
		for (var i = 0; i < this.BWtable.length; i++) {
			...
			var opt = document.createElement("option");
			el.options.add(opt);
			var v = FilterTypes[band][i]['data'][3];
			if (self.factory.fmodulo*self.factory.fstep == 10000000) v+=0.3; //delay incremented by 0.3 for clk = 480MHz
	...
	this.showStatus = function(monitor) {
		...
		for (var k = 0; k < self.NFPA.NUMALARM; k++) {
			var n = self.NFPA.naturalOrder[k];
			self.ledAlarmSet(k, monitor.nfpaAlarm[n]);
		}
		for (var k=0;k<self.config.NR_OF_RELAYS_MAX;k++){
		...
		if ( isBbuSerialMode(self.config, self.factory, self.version) ) {
	...
	this.createAdjFr = function(b, c, s, band) {
		...
		// 	fr.value = "FIRSTNET "+(b==0?"788-798MHz":"758-768MHz");
	...
	this.FilterValidSep = FilterValidSeparation;
...
function WarningBox() {
	...
	this.filterWarnText = function(filtSepKhz,fact) {
		...
		+ "2) The bandwidth setting must be 100 KHz.</br>"
		+ "3) All of them must have the same fine-gain setting.</br>";
		if (fact.chBandEnabled[0] && fact.chBandEnabled[1]){
