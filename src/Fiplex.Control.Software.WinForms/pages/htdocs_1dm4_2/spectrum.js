...
function onloadInit() {
	factory = new Factory();
	var factfrm = localStorage.getItem("factory_"+prjname+window.location.host);
	factory.parse(factfrm);
	config = new Config(maxChNr);
	var cfgfrm = localStorage.getItem("cfg_"+prjname+window.location.host);
	config.parse(cfgfrm);
...
function Spectrum(maxChNr) {
	...
	this.createBandLimitsForm = function() {
		...
		if (fstartspec==null)
			fstartspec=factory.data.band[0].fStartDL;
		else{
			fstartspec = fstartspec<factory.data.band[0].fStartDL?factory.data.band[0].fStartDL:fstartspec;
			fstartspec = fstartspec>factory.data.band[0].fStopDL?factory.data.band[0].fStartDL:fstartspec;
		}
		if (fstopspec==null)
			fstopspec=factory.data.band[0].fStopDL;
		else{
			fstopspec = fstopspec>factory.data.band[0].fStopDL?factory.data.band[0].fStopDL:fstopspec;	
			fstopspec = fstopspec<factory.data.band[0].fStartDL?factory.data.band[0].fStopDL:fstopspec;	
		}
	...
	this.reSetBandLimits = function(ev) {
		...
		if (!target || (target && target.id == "fstart")) {
			...
			if (fs < factory.data.band[0].fStartDL) {
				fs = factory.data.band[0].fStartDL;
			} else if (fs >= factory.data.band[0].fStopDL) {
				fs = this.FreqMinHz;
			}
			...
			fp = fs + diff*1e6;
			if (fp > factory.data.band[0].fStopDL) {
				fp = factory.data.band[0].fStopDL;
				fs = fp - diff*1e6;
			}
		} else {
			...
			if (fp > factory.data.band[0].fStopDL) {
				fp = factory.data.band[0].fStopDL;
			} else if (fp <= factory.data.band[0].fStartDL) {
				fp = this.FreqMaxHz;
			}
	...
	this.inYscale = {min: -90.0, max: 0.0 };
