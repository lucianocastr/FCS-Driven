...
function Factory() {
	...
	this.devConfig = [
		...
		{id: 'uplinkCoupling',	shift: 1, label: 'UPLINK&nbsp;COUPLING&nbsp;',		options: ["ANSALDO", "ANTENNA"] },
		{id: 'showGainTitles',shift: 2, label: 'SHOW&nbsp;GAIN&nbsp;TITLES&nbsp;', options: ['NO', 'YES'] },
	...
	this.hasPwrReductionPmax = false;
	this.hasPaCurrent = false;
	this.parse = function(sr) {
		...
		this.data.showGainTitles = 	(mask >> this.devConfig[2].shift) & 0x01;
	...
	this.createForm = function(version) {
		var Texts = TextEn;
		var sne = typeof(version) != "undefined" && version.swGTnpr();
		...
		if (this.showNewElements) {
			mainTb.appendChild(this.createPaCurrent());
	...
	this.createPaCurrent = function() {
		var row = document.createElement("tr");
		var cell = document.createElement("th");
		row.appendChild(cell);
		cell.innerHTML = "PA&nbsp;CURRENT&nbsp;ALARM&nbsp;(mA)";
		cell = document.createElement("td");
		row.appendChild(cell);
		var el = document.createElement("input");
		el.type = "text";
	...
	this.render = function() {
		...
		try {
			...
			if (typeof(el) != "undefined" && el) {
				if (this.data.band[i].pwrReductionCap) {
					el.disabled = false;
			...
			el = document.getElementById(this.devConfig[i++].id);
			el.selectedIndex = this.data.showGainTitles;
			el = document.getElementById(this.devConfig[i++].id);
