<!--
var LIMIT_GAIN_UL = [45, 80];
var LIMIT_GAIN_DL = [75, 80];
var sqThrDLLims = {min: -90, max: -40};
...
function parse_status(serverResponse, autoUpdt) {
	...
	var idx;
	var device;
	var bsAlarmCapable, bsSignalIn, powerInput;
	...
	for (var n = 1; n <= remotesNr+expansorNr; n++) { //n de 1 a remotesNr --> remotes, n de remotesNr a remotesNr+expansorNr -->expansores
		...
		for (var k = 0; k < kmax; k++) {
			...
			if (!isRemote && !isExpansor) {
				...
				fiberLinkLedSet(n, (alarm || rxalarm) ? "red" : (warning? "yellow" : "green"));
			...
			if (!isExpansor)
			{
				optRxLedSet(r, k, (alarm || rxalarm)? "red": (warning? "yellow" : "green"));
				if (isRemote) {
	...
	for (var n = 1; n <= remotesNr; n++) {
		...
		if (version.compareSw("0300") >= 0) {
			statPAFailLedSet(r, (bitmask & 0x80)?  "red" : "grey");
		}
...
function computeDataSync(n, gtpalarm, sfpalarm, rxpow) {
	...
	// if (!(gtpalarm & 0x02) && !(sfpalarm & 0x02) && (rxpow > -29.5)) {
	if (!(gtpalarm & 0x02)) {
		dataLockAlarm = false;
	} else {
		dataLockAlarm = true;
...
function parse_config(serverResponse) {
	...
	if (typeof(autoModeS) !== "undefined" && version.swGE4_00()) {
	...
	for (var n = 1; n <= remotesNr && n < confstrs.length; ++n) {
		...
		if (!window.top.master_rx_loss_alarm[n] && valid) {
			hpaSwSet(n, bitmask & 0x01);
			var standardRemote = ((bitmask & 0x20) == 0);
			isRemoteStandard[n] = standardRemote;
			var show = !standardRemote;
			var mb = (bitmask & 0x40) != 0;
			var forceBackup = (bitmask & 0x80) != 0;
			showRemoteMode(n, show);
			setRemoteMode(n, mb);
			setForceBackup(n, forceBackup);
		}
		shift += 2;
		bitmask = parseInt(confstrs[n].substr(shift,2),16);
		...
		var limit_gainUL = LIMIT_GAIN_UL[k];
		var limit_gainDL = LIMIT_GAIN_DL[k];
		if (!window.top.master_rx_loss_alarm[n] && valid) gainULSet(n, limit_gainUL - attout_up);
		if (!window.top.master_rx_loss_alarm[n] && valid) gainDLSet(n, limit_gainDL - attin_down - attout_down);
		shift += 2;
...
function format_unit(currentConfigFrm, channel, keepCh, n) {
	...
	if (n > 0) {
		if (!isRemoteStandard[n]) {
			bitmask &= 0x3E;
		}
		if (hpaIsOn(n))
			bitmask |= 0x01;
		if (!isRemoteStandard[n]) {
			var mbCtrl = readRemoteMode(n);
			if (mbCtrl == 0) {
				bitmask &= ~0x40;
			} else if (mbCtrl == 1) {
				bitmask |= 0x40;
			}
			var fbak = readForceBackup(n);
			if (fbak == 0) {
				bitmask &= ~0x80;
			} else if (fbak == 1) {
				bitmask |= 0x80;
	...
	var rpr = remotePwrReductionGet();
	if (rpr < remPwrRedLims.min) {
		rpr = remPwrRedLims.min;
		remotePwrReductionSet(rpr);
	} else if (rpr > remPwrRedLims.max) {
		rpr = remPwrRedLims.max;
		remotePwrReductionSet(rpr);
	}
	if (n == 0)
	{
	...
	if (n == 0) {
		if (num > sqThrDLLims.max)
			num = sqThrDLLims.max;
		else if (num < sqThrDLLims.min)
			num = sqThrDLLims.min;
	}
	else{
