...
var tbs_settings = {min: -80, low_alarm: -128, low_warn: -128, high_warn: -6, high_alarm: 0, max: 0 };
var rfin_settings = {min:  -80, low_alarm: -128, low_warn: -128, high_warn: 0, high_alarm: 0, max: -20 };
var chPowMaster_settings = {min: -80, low_alarm: -80, low_warn: -80, high_warn: 0, high_alarm: 0, max: -20 };
var chPowRemote_settings = {min: -100, low_alarm: -128, low_warn: -128, high_warn: 0, high_alarm: 0, max: 0 };
...
function renderUnit(nr) {
	...
	if (nr==0){
		...
		cell = document.createElement("td");
		cell.id = "remoteTypeTitle_"+nr;
		cell.innerHTML = "Remote&nbsp;Type";
		cell.style.paddingRight = "10px";
		cell.style.fontWeight = "bold";
		cell.style.visibility = "hidden";
		headDiv.appendChild(cell);
		cell = document.createElement("td");
		cell.id = "remoteTypeCtl_"+nr;
		cell.style.minWidth = "100px";
		cell.appendChild(createRemoteModeCtl(nr));
		headDiv.appendChild(cell);
		cell.style.textAlign = "center";
		cell.style.paddingRight = "10px";
		cell.style.visibility = "hidden";
...
function renderUnitContent(nr, parent) {
	...
	if (isMaster && version.swGE4_00()) {
		...
		cell.colSpan = version.swGTnpr() ? 4 : 3;
	...
	if (!isMaster || isMaster && (version.swGTnpr())) {
		cell = document.createElement("td");
		cell.className = "contentcell";
...
function createFilteringTable(nr) {
	...
	if (nr == 0) {
		...
		if (factory.data.band[0].simplex) {
			createSimplexCtrl(0, trNode);
		}
		box.appendChild(trNode);
		filtBox.appendChild(box);
	}
	if (isMaster) {
		...
		filtBox.appendChild(createFilterCtlTable(nr));
		var chTbl = createChannelTable(nr,1,24,"");
		filtBox.appendChild(chTbl);
	}
...
function createFilterCtlTable(nr) {
	...
	trNode = document.createElement("tr");
	tbodyNode.appendChild(trNode);
	if (nr > 0 && window.top.allowDisFilt[nr-1]) {
		tdNode = document.createElement("td");
		tdNode.innerHTML = Texts['FENABLE'];
		tdNode.style.textAlign = "right";
	...
	if (nr != 0) {
		...
		tdNode.appendChild(createBackupSel(nr));
		trNode.appendChild(tdNode);
	}
	else{
		trNode = document.createElement("tr");
...
function createSqThrControl(nr,id,trNode,lname){
	...
	if (nr == 0) {
		var tooltip = "Min: "+sqThrDLLims.min +", Max: "+sqThrDLLims.max+" dBm";
	}else{
...
function createMasterModeCtl(){
	...
	if (version.swGE4_00()) {
		...
		texts = ["PRIMARY", "SECONDARY", "AUTO (SQ. BASED)"];
	}
...
function createFilterChannel(chFiltRow, ch, nr) {
	...
	var s;
	if (isMaster) {
		s = chPowMaster_settings;
	} else {
		s = chPowRemote_settings;
	}
...
function createRfInTable(nr) {
	...
	if (isMaster && (version.compareSw("0221") >= 0)) {
		trNode = createRemotePowerReduction();
		tbodyNode.appendChild(trNode);
	}
	if (!isMaster) {
...
function createRemotePowerReduction() {
	var trNode = document.createElement("tr");
	var tdNode = document.createElement("td");
	var str = "DL&nbsp;Output</br>";
	str += "Power&nbsp;Reduction";
	tdNode.innerHTML = str;
	tdNode.style.fontWeight = "bold";
	tdNode.colSpan = 2;
	trNode.appendChild(tdNode);
	tdNode = document.createElement("td");
...
function createAttenuator(id,inout,nr) {
	...
	var tdNode = document.createElement("td");
	if (nr == 0 && inout == 0) {
		tdNode.innerHTML = "Input&nbsp;"+Texts['ATTENUATOR'];
		tdNode.colSpan = 2;
	} else {
...
function createRfGain(id) {
	...
	tdNode.style.textAlign = "left";
	trNode.appendChild(tdNode);
	trNode.style.display = factory.data.showGainTitles?"table-row":"none";
	return trNode;
}
...
function createHpaTable(nr) {
	...
	if (window.top.NFPAMonitor[nr]!=0){
		...
		trNode.appendChild(tdNode);			
		// solamente se mostrará con micros con ethernet, que no pueden
		// tener emparejamiento de rf, porque se reusa el bit que lo indica
		if (typeof(version) == 'undefined' || !version || version.isUndefined()) {
			var sr = version.retrieve();
			version.parse(sr);
		}
		if (version.compareSw("0300") < 0) {
...
function remoteInputsDisableStateSet(n, disabled) {
	...
	forceBackupSetDisable(n, disable);
}
...
function createClearCounters() {
	...
	el.style.fontWeight = "bold";
	el.style.marginRight = el.style.marginLeft = "auto";
	if (typeof(version) == 'undefined' || !version || version.isUndefined()) {
		var sr = version.retrieve();
		version.parse(sr);
	}
	if (version.compareSw("0224") < 0) {
...
function createRemoteModeCtl(nr) {
	el = document.createElement("select"); 
	el.id = "remoteMode_"+nr;
	el.name = el.id;
	el.type = "select";
	el.nr = nr;
	el.style.textAlign = "left";
	var texts = ["MAIN", "BACKUP"];
	for (var i = 0; i < texts.length; ++i) {
		var opt = document.createElement("option");
		el.options.add(opt);
		opt.text = texts[i];
	...
	el.onchange = function(ev) {
		var show = false;
		if (this.selectedIndex == 0 && !isRemoteStandard[this.nr]) {
			show = true;
		}
		showForceBackup(this.nr, show);
	}
	return el;	
}

function showRemoteMode(nr, show) {
	var ids = [];
	ids.push("remoteTypeTitle_"+nr);
	ids.push("remoteTypeCtl_"+nr);
...
function setRemoteMode(nr, bak) {
	try {
		var el = document.getElementById("remoteMode_"+nr);
		if (!el) {
			return;
		}
...
function createBackupSel(nr) {
	var box = document.createElement("div");
	box.id = "forceBackupBox_"+nr;
	box.style.width = "100%";
	box.style.display = "none";
	...
	var el = document.createElement("input");
	el.type = "checkbox";
	el.id = el.name = "forceBackup_"+nr;
	td.appendChild(el);
	innerBox.appendChild(tab);
	return box;
}

function showForceBackup(nr, show) {
	try {
		var el = document.getElementById("forceBackupBox_"+nr);
		el.style.display = (show ? "block" : "none");
	} catch(err) {}
}

function setForceBackup(nr, on) {
	try {
		var el = document.getElementById("forceBackup_"+nr);
		el.checked = (on ? true : false);
	} catch(err) {}
}

function readForceBackup(nr) {
	try {
		var el = document.getElementById("forceBackup_"+nr);
...
function forceBackupSetDisable(nr, disable) {
	try {
		var el = document.getElementById("forceBackup_"+nr);
		el.disabled = disable? true : false;
		el.style.backgroundColor = disable ? "#CCCCCC" : "white";
