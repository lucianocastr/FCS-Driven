function Factory(factoryFrame) {
	...
	this.psFstartAdj = function(b, v) {
		self.data.bandAdj[b].fnStart = v;
	}
	this.psFstopAdj = function(b, v) {
		self.data.bandAdj[b].fnStop = v;
	}
	...
	this.ftPwrLim = function(b) {
		var v = self.data.band[b].powerLim & 0x7F;
		if (self.data.pwrReductionEn && b == 0) {
			v |= 0x80;
		}
		return v;
	}
	...
	this.ftFstartAdj = function(b) {
		return self.data.bandAdj[b].fnStart;
	}
	this.ftFstopAdj = function(b) {
		return self.data.bandAdj[b].fnStop;
	}
	...
	this.Map = [ 
		...
		},
		{
			...
			len: 8,
			dataLen: 2,
			fmt: this.ftFstartAdj,
			prs: this.psFstartAdj
		},
		{
			id:'fnStop',
			len: 8,
	...
	this.getOsc = function() {
		if (this.data.fmodulo % 150 == 0) {
			return 150;
		} else if (this.data.fmodulo % 160 == 0) {
			return 160;
		}
		return 150;
	}
