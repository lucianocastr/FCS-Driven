function ftoolGui(clk) {
	...
	this.carriersAreaSetTitle = function(b, el) {
		var fstart = (self.factory.data.band[b].fStart / 1e6).toFixed(6);
		var fstop = (self.factory.data.band[b].fStop / 1e6).toFixed(6);
	...
	this.computeFilters = function() {
		...
		var fNoverlap = self.checkFnOverlap(inputCarr, b);
		if (fNoverlap.length > 0) {
			alert("Some carriers overlap FirstNet band "+ 
				(b == 0 ? "uplink":"downlink"+":\n"+
				fNoverlap));
			return null;
		}
		...
		if (n > self.confs.CHFPGANR) {
			alert("Too many filters required: "+n+":\n"+"Maximum nr. of filters is "+self.confs.CHFPGANR); 
			return null;
		}
		return gfilts;
	}
	...
	this.checkFnOverlap = function(cl, b) {
		var ovlp = [];
		var fNon = self.confs.getChActive(self.confs.FNCH);
		if (!fNon) {
			return ovlp;
		}
	...
	this.computeNrFiltBw3dB = function(bw) {
		...
		for (var i = 8; i < self.filterTypes[k]['filt'].length; ++i) {
