	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState == 4 && this.status == 200) {
				timerId = setTimeout(function() { loadConfs(); }, 1000);
			}
		}
		xhr.onerror = function(ev) {
			if (typeof(timerId) !== "undefined" && timerId)
...
function freqsReq() {
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState == 4 && this.status == 200) {
				timerId = setTimeout(function() { load_freqs(); }, 1000);
			}
		}
		xhr.onerror = function(ev) {
			if (typeof(timerId) !== "undefined" && timerId)
...
function Spectrum() {
	...
	this.computeSpectBandLimits = function(b) {
		var fm = [];
		var fmin, fmax; 
		if (factory.data.band[b].fStart > factory.data.bandAdj[b].fnStart) {
			fmin = factory.data.bandAdj[b].fnStart;
		} else {
			fmin = factory.data.band[b].fStart;
		}
		fmin -= this.FREQ_MARGIN_HZ
		if (factory.data.band[b].fStop > factory.data.bandAdj[b].fnStop) {
			fmax = factory.data.band[b].fStop;
		} else {
			fmax = factory.data.bandAdj[b].fnStop;
		}
		fmax += this.FREQ_MARGIN_HZ;
		fm.push(fmin);
		fm.push(fmax);
		return fm;
	}
