<!--
ch_Bw_Txt = ["180", "90", "45", "30", "20", "15"];
var hpa_settings = [{min:   -10, low_alarm:   -10, low_warn:  -10, high_warn: 30, high_alarm: 30, max: 30 },
					{min:   0, low_alarm:   0, low_warn:  20, high_warn: 42, high_alarm: 43, max: 45 }];
...
var chRfOut_settings = [{min: -30, low_alarm: -30, low_warn: -30, high_warn: 30, high_alarm: 30, max: 30 },
			{min: -15, low_alarm: -15, low_warn: -15, high_warn: 45, high_alarm: 45, max: 45 }];
...
function setLimits(){
	hpa_settings[1].high_warn = factory.data.band[1].powLim + 2;
	hpa_settings[1].high_alarm = factory.data.band[1].powLim + 3;
	mainGainLims[0].max = factory.data.band[0].gainLim;
	mainGainLims[1].max = factory.data.band[1].gainLim;
	mainPowerLims[0].min = factory.data.band[0].powLim -20;
	mainPowerLims[0].max = factory.data.band[0].powLim;
	mainPowerLims[1].min = factory.data.band[1].powLim -20;
	mainPowerLims[1].max = factory.data.band[1].powLim;	
}
...
function renderUnit(nr) {
	...
	if (nr==0){
		...
		cell.innerHTML = "Master&nbspType";
...
function rfRedunded() {
	...
	this.readStream = function() {
		try {
			...
			var srs = localStorage.getItem("serial_1cm"+window.location.host).split('\t');
...
function renderUnitContent(nr, parent) {
	...
	el = createRFMainTable(1);
	//el = createRfInTable(1);
	cell.appendChild(el);
	/*
	cell = document.createElement("td");
	cell.className = "contentcell";
	row1.appendChild(cell);
	el = createRfOutTable(1);
	cell.appendChild(el);*/
	cell = document.createElement("td");
	cell.className = "contentcell";
	row1.appendChild(cell);
	
	el = createRFMainTable(0);
	//el = createRfInTable(0);
	cell.appendChild(el);
	/*
	cell = document.createElement("td");
	cell.className = "contentcell";
	row1.appendChild(cell);
	el = createRfOutTable(0);
	cell.appendChild(el);	*/
	/*if (!isMaster) {
		cell = document.createElement("td");
		cell.className = "contentcell";
	...
	cell = document.createElement("td");
	cell.className = "contentcell";
	cell.colSpan = 1;//isMaster ? 3 : 4;
	row3.appendChild(cell);
	el = createFilteringTable(1);
	cell.appendChild(el);
	cell = document.createElement("td");
	cell.className = "contentcell";
	cell.colSpan = 1;//isMaster ? 3 : 4;
	row3.appendChild(cell);
	el = createFilteringTable(0);
	cell.appendChild(el);	
}
...
function createFilterCtlTable(nr) {
	...
	trNode = document.createElement("tr");
	tbodyNode.appendChild(trNode);
	factory.data.allowDisableFilt = false;
	window.top.allowDisFilt[0] = false;
	if (nr == 0 && factory.data.allowDisableFilt
	    || nr > 0 && window.top.allowDisFilt[nr-1]) {
		tdNode = document.createElement("td");
	...
	if (nr!=0)
	{
		...
		{
			tdNode = document.createElement("td");
			tdNode.innerHTML = Texts['RMODE'];
			tdNode.style.textAlign = "right";
			trNode.appendChild(tdNode);
			var modeCtl = createModeCtl(nr);
			trNode.appendChild(modeCtl);
		}
		if ((window.top.mode[nr]==0x08)&&isFiberRedunded(nr)){
			tdNode = document.createElement("td");
			trNode.appendChild(tdNode);
			tdNode.innerHTML = '&nbsp;CONFIGURATION&nbsp;ERROR';
			tdNode.title = "Addition mode is not allowed for fiber redunded remotes. Please, change mode or re-identify devices in Device List section";
		...
		tbodyNode.appendChild(trNode);
		tdNode = document.createElement("td");
		tdNode.innerHTML = Texts['LINKF'];
		tdNode.colSpan = 3;
		trNode.appendChild(tdNode);
		tdNode = createLinkedFreqCtrl();
		tdNode.colSpan = 3;
		trNode.appendChild(tdNode);*/
	}
	return box;
...
function createModeCtl(nr){
	tdNode = document.createElement("td");
	el = document.createElement("select"); 
	el.id = "mode_remote"+nr;
	el.name = el.id;
	el.type = "select-one";
	el.style.textAlign = "left";
	el.onchange = function(ev) {
		ev = ev || window.event;
		var target = ev.target;
		window.top.mode[nr]=target.selectedIndex<<3;
		setRemoteMode(nr,window.top.mode[nr]);
		redrawAll(false);
	}
	var opt = document.createElement("option");
	el.options.add(opt);
	...
	opt.text = "ADDITION";
	opt.value = 1;
	el.selectedIndex = (window.top.mode[nr] == 0x8) ? 1:0;
	tdNode.appendChild(el);
	tdNode.style.textAlign = "left";
	return tdNode;
}
function getRemoteMode(nr){
	try {
	var selmode = document.getElementById("mode_remote"+nr);
...
function setNFPALed(state){
	try {
		ledSetColor("nfpaStatusLed",state?"red":"grey");
	} catch (err) {}	
}
...
function setRemoteMode(nr,mode){
	var selmode = document.getElementById("mode_remote"+nr);
	if (selmode == null) return;
	selmode.selectedIndex = (mode==0)? 0 : 1;
}
...
function createChannelTable(nr,start,stop,title) {
	...
	//show_ch_nr = window.top.MaxChNr;
	var nr_rows = Math.ceil(show_ch_nr);
	var mask = 1<<(start-1);
	var ch = start;		
	if (title.length>0){
...
function createFilterChannel(chFiltRow, ch, nr) {
	...
	chFr.name = chFr.id;
	if (isMaster){
		var tooltip = "Min: "+(factory.data.band[1].fStart/1e6) +", Max: "+(factory.data.band[1].fStop/1e6)+" MHz";
		chFr.title = tooltip;
	}else{
	...
	td = document.createElement("td");
	td.id = "chPoutMet"+"_"+ch+"_"+nr;
	chFiltRow.appendChild(td);
	s = chRfOut_settings[1-nr];
	var chPoutMet = new mMeter(s.min, s.max, s.low_alarm, s.high_alarm, s.low_warn, s.high_warn);
	chPoutMet.attachTo(td);
	chPoutMet.valueSet(s.min);
	td = document.createElement("td");
	td.id = "chPoutText"+"_"+ch+"_"+nr;
	td.style.minWidth = "40px";
	td.style.textAlign = "right";
...
function chPoutSet(ch, nr, val, color) {
	try {
		var el = document.getElementById("chPoutMet"+"_"+ch+"_"+nr);
		if (typeof(el) != "undefined" && el) {
			var elmet = el.mMeter;
			var valMet = val;
			if (isNaN(valMet)) valMet = elmet.mMin;
			if (typeof(color) === 'undefined')
				elmet.valueSet(valMet);
			else
				elmet.valueSet(valMet, color);
		}
		var eltxt = document.getElementById("chPoutText"+"_"+ch+"_"+nr);
		if (typeof(val) === 'undefined' || isNaN(val))
			eltxt.innerHTML = val;
		else
			eltxt.innerHTML = val.toFixed(1);
...
function createRFMainTable(nr){
	...
	row.appendChild(cell);
	cell.innerHTML = nr==0 ? Texts['DNLINK'] : Texts['UPLINK'];
	cell.colSpan = 3;
	cell.className = 'nh';
...
function createRfInTable(nr) {
	...
	sqThr.style.display = "inline";
	if (nr == 0) {
		var k = 1;//factory.data.uplinkCoupling == 1 ? 1 : 0;
		var tooltip = "Min: "+sqThrDLLims[k].min +", Max: "+sqThrDLLims[k].max+" dBm";
		sqThr.title = tooltip;
	}
	else
	...
	trNode = createMainGainCtrl("maxGain"+nr,nr);
	tbodyNode.appendChild(trNode);
	
	trNode = createMainPowerCtrl("maxPower"+nr,nr);
	tbodyNode.appendChild(trNode);	
...
function mainGainSet(nr, value) {
	try {
		var el = document.getElementById("maxGain"+nr);
		if (!isNaN(value))
			el.value = value;
	} catch (err) {}
}
function mainGainGet(nr) {
	try {
		var el = document.getElementById("maxGain"+nr);
		return parseInt(el.value);
	} catch (err) {
...
function mainPowerSet(nr, value) {
	try {
		var el = document.getElementById("maxPower"+nr);
		if (!isNaN(value))
			el.value = value;
	} catch (err) {}
}
function mainPowerGet(nr) {
	try {
		var el = document.getElementById("maxPower"+nr);
		return parseInt(el.value);
	} catch (err) {
...
function createRfOutTable(nr) {
	...
	trNode.appendChild(tdNode);*/
	tdNode = createHpaSwitch(1-nr);
	trNode.appendChild(tdNode);
	
	//var s= (isMaster)? master_settings:hpa_settings;
	s = hpa_settings[1-nr];
	trNode = createMetRow("rfOutPow"+(1-nr), s, "dBm", "Power", "right");
	tbodyNode.appendChild(trNode);
	if ((nr==1)&&(window.top.NFPAMonitor[0]!=0)){
		trNode = document.createElement("tr");
		tbodyNode.appendChild(trNode);
		tdNode = document.createElement("th");
		tdNode.innerHTML = TextEn['COMERR'];
		tdNode.style.textAlign = "left";
		trNode.appendChild(tdNode);
		tdNode = createLedBox("nfpaStatusLed");
		trNode.appendChild(tdNode);
		tdNode = document.createElement("td");
		tdNode.id = "nfpaStatusText"
		trNode.appendChild(tdNode);
		trNode = document.createElement("tr");
		tbodyNode.appendChild(trNode);
		tdNode = document.createElement("th");
	...
	if (nr==0)
	{
		trNode = document.createElement("tr");
		tbodyNode.appendChild(trNode);
		if (window.top.NFPAMonitor[1]!=0){
			tdNode = document.createElement("td");
			tdNode.innerHTML = Texts['COMERR'];
			tdNode.style.textAlign = "left";
			tdNode.style.fontWeight = "bold";
			trNode.appendChild(tdNode);
			tdNode = createLedBox("hpaStatLed1");
			trNode.appendChild(tdNode);
		}	
		trNode = document.createElement("tr");
		tbodyNode.appendChild(trNode);		
		tdNode = document.createElement("td");
		tdNode.innerHTML = (window.top.NFPAMonitor[1] != 0 ? Texts['AGCFAIL'] : Texts['PAOVF']);
		tdNode.style.textAlign = "left";
		tdNode.style.fontWeight = "bold";
		trNode.appendChild(tdNode);
		tdNode = createLedBox("hpaOvfLed"+1);
		trNode.appendChild(tdNode);
		tbodyNode.appendChild(trNode);
		if (window.top.NFPAMonitor[1]!=0){
			trNode = document.createElement("tr");
			...
			tdNode.style.fontWeight = "bold";
			trNode.appendChild(tdNode);
			tdNode = createLedBox("hpaVswrLed1");
			trNode.appendChild(tdNode);
			trNode = document.createElement("tr");
			tbodyNode.appendChild(trNode);			
			tdNode = document.createElement("td");
			...
			tdNode.style.fontWeight = "bold";
			trNode.appendChild(tdNode);	
			tdNode = createLedBox("hpaTxLowLed1");
			trNode.appendChild(tdNode);	
			
		}		
	}else{
		if (factory.data.PACurrMonitor!=0){
			trNode = document.createElement("tr");
			tbodyNode.appendChild(trNode);
			tdNode = document.createElement("td");
	...
	tbodyNode.appendChild(trNode);
	trNode = createMainGainCtrl("attOut"+nr,1,nr);
	tbodyNode.appendChild(trNode);
	*/
	return box;
}
...
function createMainGainCtrl(id,nr) {
	var trNode = document.createElement("tr");
	var tdNode = document.createElement("td");
	tdNode.innerHTML = Texts['MGAIN'];
	tdNode.style.fontWeight = "bold";
	tdNode.style.textAlign = "right";
	trNode.appendChild(tdNode);
	tdNode = document.createElement("td");
	...
	var tooltip = "Min: "+mainGainLims[1-nr].min +", Max: "+mainGainLims[1-nr].max+" dB";
	att.title = tooltip;
	att.id = id;
	att.name = id;
	att.type = "text";
...
function createMainPowerCtrl(id,nr) {
	var trNode = document.createElement("tr");
	var tdNode = document.createElement("td");
	tdNode.innerHTML = Texts['MPOWER'];
	tdNode.style.fontWeight = "bold";
	tdNode.style.textAlign = "right";
	trNode.appendChild(tdNode);
	tdNode = document.createElement("td");
	...
	var att = document.createElement("input");
	var k = (nr==0) ? 1:0;
	var tooltip = "Min: "+mainPowerLims[k].min +", Max: "+mainPowerLims[k].max+" dBm";
	att.title = tooltip;
	att.id = id;
	att.name = id;
	att.type = "text";
...
function createStatusTable(nr) {
	...
	{
		td = document.createElement("td");
		td.innerHTML = Texts['LINKF'];
		td.style.textAlign = "right";
		td.style.fontWeight = "bold";
		tr.appendChild(td);
		td = createLinkedFreqCtrl();
		tr.appendChild(td);
	}
	td = document.createElement("td");
...
function createFiberTable(nr) {
	...
	createGdCtrl(tbodyNode, window.top.FiberMask[nr]==0x03, nr);
	return box;
}

function createGdCtrl(tbodyNode, twofibers, nr) {
	var trNode, tdNode;
	trNode = document.createElement("tr");
	tdNode = document.createElement("th");
...
function optPopup(nr,ne,url) {
	...
	var fmask = localStorage.getItem("fibermask_1cm"+window.location.host);
...
function fiberPowerSet(nr, nfiber, val, color) {
	try {
		/*var el = document.getElementById("fiberRxPowMet_"+nfiber+"_"+nr);
		if (typeof(el) != "undefined" && el) {
			var elmet = el.mMeter;
