...
var sqThrULLims = {min: -110, max: -70};
var attInLims = {min: 0, max: 30};
var attOutLims = [{min: 0, max: 50} , {min: 0, max: 30}];
var fineGainLims = {min: -40, max: 0};
var mainGainLims = [ {min: 60, max: 80}, {min: 60, max: 80}];							
var mainPowerLims = [ {min: 10, max: 30}, {min: 20, max: 40}];	
var delayLims = {min: 0, max: 200};
var masterMode;
var deviceconfig = new Config();
...
function Config() {
	...
	for (var i = 0; i < 2; ++i) {
		this.data.mainGain.push(0);
		this.data.sqEnabled.push(0);
		this.data.fineGain[i] = [];
		for (var j = 0; j < window.top.MaxChNr; ++j) {
			this.data.fineGain[i].push(0);
		}			
	}	
	this.parse = function(sr) {
		try {
			...
			if (cfgstrs.length>=2){
				this.data.sqEnabled[0] = (parseInt(cfgstrs[1].substr(8,2),16) & 0x10)?1:0;
				this.data.sqEnabled[1] = (parseInt(cfgstrs[0].substr(14,2),16) & 0x10)?1:0;
				this.data.mainGain[1]=parseInt(cfgstrs[0].substr(16,2),16);
				this.data.mainGain[0]=parseInt(cfgstrs[1].substr(10,2),16);
				for (var j = 0; j < window.top.MaxChNr; ++j) {
					this.data.fineGain[1][j]=cSignedByte(parseInt(cfgstrs[0].substr(28+6*j,2),16));
					this.data.fineGain[0][j]=cSignedByte(parseInt(cfgstrs[1].substr(20+6*j,2),16));
				}	
			}
...
function parse_status(serverResponse) {
	var shift, num, agc, volt, bias, txpow, rxpow, warning, alarm, temp;
	var signal_downlink, signal_in, signal_ovf, ch_disabled, fpgaAlarm, tempAlarm, tempAlarmR;
	...
	for (var n = 1; n <= 1; n++) {
		...
		ovfLedSet(n, color);
		fpgaAlarm = bitmask & 0x02;
		tempAlarmR = (bitmask & 0x04)>>2;
		boardTempLedSet(n, tempAlarmR ?  "red" : "grey");
	...
	for (var ch = 1, shift = 20, mask = 1; ch <= ch_nr; ++ch) {
		...
		agc = cSignedByte(parseInt(statstrs[0].substr(shift,2),16));
		chAgcSet(ch,0,agc);
		if ((ch_disabled)||!hpaIsOn(1))
			chPoutSet(ch,0,"OFF");
		else{
			if ((deviceconfig.data.sqEnabled[1]!=0) && (!signal_in))
				chPoutSet(ch,0,"OFF");
			else
				chPoutSet(ch,0,num+deviceconfig.data.mainGain[1]+deviceconfig.data.fineGain[1][ch-1]-agc);
		}
		shift += 2;
	}
	bitmask = parseInt(statstrs[0].substr(6,2),16);
	...
	ovfLedSet(0, color);
	fpgaAlarm = bitmask & 0x02;
	tempAlarm = (bitmask & 0x04)>>2;
	boardTempLedSet(0, (tempAlarm | tempAlarmR) ?  "red" : "grey");
	...
	rfOutPowSet(0, hpaIsOn(0)?num:"OFF");
	num = parseInt(statstrs[0].substr(392,2),16) & 0x80;
	statHpaLedSet(0,num!=0?"red":"grey");
	...
	for (var n = 1; n <= remotesNr; n++) {
		...
		for (var ch = 1, shift = 80,mask = 1; ch <= ch_nr; ch++) {
			...
			agc = cSignedByte(parseInt(statstrs[n+expansorNr].substr(shift,2),16));
			chAgcSet(ch,n,agc);
			if ((ch_disabled)||!hpaIsOn(0))
				chPoutSet(ch,n,"OFF");
			else{
				if ((deviceconfig.data.sqEnabled[0]!=0) && (!signal_in))
					chPoutSet(ch,n,"OFF");
				else
					chPoutSet(ch,n,num+deviceconfig.data.mainGain[0]+deviceconfig.data.fineGain[0][ch-1]-agc);
			}
			shift += 2;
		}
	}
	...
	for (var n = 1; n <= remotesNr+expansorNr; n++) { //n de 1 a remotesNr --> remotes, n de remotesNr a remotesNr+expansorNr -->expansores
		...
		for (var k = 0; k < kmax; k++) {
			...
			if (!isExpansor){
				optStatusLedSet(n, k, alarm? "red" : (warning? "yellow" : "green"));
			...
			if (!isRemote && !isExpansor) {
					...
					fpgaLedSet(0, masterFpgaAlarm!=0 | alarm ?  "red" : "grey");
			...
			if (!isExpansor)
			{
				optRxLedSet(n, k, alarm? "red": (warning? "yellow" : "green"));
				if (isRemote) {
					...
					if (bitmask==0) optActiveLinkSet(n,"green","grey");
					else optActiveLinkSet(n,"grey","green");
				}
				shift += 2;
				num  =  parseInt(statstrs[device].substr(shift,4),16);
				if (isRemote && window.top.master_rx_loss_alarm[n]) {
					optErrorsSet(n, k, "invalid");
				} else {
					optErrorsSet(n, k, num);
				}
			}
			else
	...
	for (var n = 1; n <= 1; n++) {
		...
		ovfHpaLedSet(n, (bitmask & 0x01)?  "red" : "grey");
		
		vswrHpaLedSet(n, (bitmask & 0x04)?  "red" : "grey");
		statHpaLedSet(n, (bitmask & 0x10)?  "red" : "grey");
		txLowHpaLedSet(n, (bitmask & 0x08)?  "red" : "grey");
		
		shift += 2;
		boardtempR = to_float(parseInt(statstrs[n+expansorNr].substr(shift,4),16));
		//boardTempSet(n, num)
		shift += 4;
		num = to_float(parseInt(statstrs[n+expansorNr].substr(shift,4),16));
		rfOutPowSet(n, hpaIsOn(n)?num:"OFF");
	}
	for (var n = 1; n <= 1; n++) {
		...
		if (window.top.master_rx_loss_alarm[n]) {
			...
			ovfHpaLedSet(n, "grey");
			vswrHpaLedSet(n, "grey");
			statHpaLedSet(n, "grey");
			txLowHpaLedSet(n, "grey");
			enHpaLedSet(n, "grey");
			rfOutPowSet(n, '');
			optStatusLedSet(n, 1, "grey");optStatusLedSet(n, 2, "grey");
			optRxLedSet(n, 1, "grey");optRxLedSet(n, 2, "grey");
			fiberPowerSet(n, 1, '---', "#bbbbbb");fiberPowerSet(n, 2, '---', "#bbbbbb");
			optFrSyncLedSet(n, "grey");
			boardTempSet(n, "---");
			if (n==1) boardtempR=-100;
			remoteInputsDisableStateSet(n, true);
		} else {
			setTag(n, tags[k+window.top.MaxExpansorNr]);
		}
	}
	boardTempSet(0, boardtempR > boardtemp?boardtempR:boardtemp);
...
function parse_config(serverResponse) {
	...
	deviceconfig.parse(serverResponse);
	//Master
	shift = 14;
	...
	setMasterMode(bitmask & 0x20);
	setLinkedFreq(window.top.LinkedFreq);
	masterMode = (bitmask & 0x20) != 0;
	shift += 2;
	...
	bitmask = parseInt(confstrs[0].substr(shift,2),16); //PA UL OFF/ON
	hpaSwSet(0, !(bitmask & 0x1));
	shift += 2;
	attout = parseInt(confstrs[0].substr(shift,2),16);
	mainPowerSet(0,attout);
	...
	for (var n = 1; n <= remotesNr && n < confstrs.length; ++n) {
		...
		if (!window.top.master_rx_loss_alarm[n] && valid) setRemoteMode(n, window.top.mode[n]);
	...
	for (n = 1; n <= remotesNr && n < confstrs.length; ++n) {
		...
		for (var ch = 1; ch <= ch_nr; ++ch) {
			...
			if ((fr - factory.data.band[0].fStart) != (channel[ch] - factory.data.band[1].fStart) && filtEnIsSet(n)) {
...
function parseChFreq(data, isUL) {
	...
	var fo = isUL ? factory.data.band[0].fo : factory.data.band[1].fo;
...
function parseChannelsEnabled(serverResponse, nr) {
	...
	if (nr == 0) {
		num = parseInt(confstrs[0].substr(-8,6).substr(0,6),16);
...
function format_config(currentConfigFrm, reset_nr) {
	...
	if (reset_nr >= 0 && reset_nr <= window.top.remotesNr) {
		var subframe = format_reset(currentConfigFrm, 1);
		confFrames.push(subframe);
		subframe = format_reset(currentConfigFrm, 0);
		confFrames.push(subframe);
		return;
	}
	var channel = new Array();
...
function format_reset(currentConfigFrm, n) {
	...
	if (n==0)
		subframe = confstrs[0].substr(0,12) + "01" + confstrs[0].substr(-162,162);
	else
...
function format_unit(currentConfigFrm, channel, keepCh, n) {
	...
	/* CÓDIGO ANTIGUO DE PROGRAMACIÓN MAIN GAIN + ATT_IN + ATT_OUT EN EL MASTER Y
	MAIN GAIN + ATT_OUT EN EL REMOTO
	...
	var num =  mainGainGet(n);
	if (num > mainGainLims[1-n].max)
		num = mainGainLims[1-n].max;
	else if (num < mainGainLims[1-n].min)
		num = mainGainLims[1-n].min;
	configFrm += hexformat(num, 2);
	
	if (n==0){
		bitmask = 0;
		if (!hpaIsOn(n)) bitmask |= 0x1;
		configFrm += hexformat(bitmask, 2); //Atenuador de entrada, ahora PA OFF/ON
	}
	k = (n==0) ? 1:0;
	num =  mainPowerGet(n);
	if (num > mainPowerLims[k].max)
		num = mainPowerLims[k].max;
	else if (num < mainPowerLims[k].min)
		num = mainPowerLims[k].min;
	if (num < 0)
		num += 256;	
	configFrm += hexformat(num, 2);
	
	k = 1;//(n==0) ? (factory.data.uplinkCoupling == 0 ? 0 : 1) : 1;
	num =  filtSqThrGet(n);
	if (n == 0) {
	...
	for (var ch = 1; ch <= ch_nr; ++ch) {
		...
		if (n == 0) {
			...
			fr = chFrGet(ch, 0);
			if (isNaN(fr) || fr < factory.data.band[1].fStart) {
				fr = factory.data.band[1].fStart;
				chFrSet(ch, 0, fr);
			}
			if (isNaN(fr) || fr > factory.data.band[1].fStop) {
			...
			if (!window.top.LinkedFreq){
				...
				var nmax = ~~Math.round((factory.data.band[0].fStop - factory.data.band[0].fo) / factory.data.fstep);
				var nmin = ~~Math.round((factory.data.band[0].fStart - factory.data.band[0].fo) / factory.data.fstep);
	...
	if (n > 0){
		...
		configFrm += currSubfrm.substr(configFrm.length, 16); //debug channels modo add
	}
...
function computeChNrOtherBand(chNr, isUL) {
	var fo = isUL ? factory.data.band[0].fo : factory.data.band[1].fo;
	var fstart = isUL ? factory.data.band[0].fStart : factory.data.band[1].fStart;
	var fr = chNr * factory.data.fstep + fo;
	var diff = fr - fstart;
	fstart = isUL ? factory.data.band[1].fStart : factory.data.band[0].fStart;
	fr = fstart + diff;
...
function linkActiveRedunded(nr){
	...
	if (isFiberRedunded(nr)){
		if ((rxloss[nremote[0]]) && (!rxloss[nremote[1]]))
			...
			return nremote[0];
		//return (parseInt(statstrs[nr+ne].substr(16,2),16)==0)?nremote[0]:nremote[1];
	}
	else
		return nr;
