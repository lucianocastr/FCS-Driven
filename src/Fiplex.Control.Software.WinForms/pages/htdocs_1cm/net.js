...
function onloadInit() {
	...
	for (var i = 0; i <= window.top.MaxRemotesNr+window.top.MaxExpansorNr; ++i)
		tags.push("");
	window.top.fiplexTagsStr = localStorage.getItem("tags_1cm"+window.location.host);
...
function doStart() {
	...
	window.top.fiplexSerialStr=localStorage.getItem("serial_1cm"+window.location.host);
	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState == 4 && this.status == 200) {
				...
				window.parent.navi.factoryFrame = window.top.fiplexFactStr;
				factory.parse(this.responseText);
				localStorage.setItem("factory_1cm"+window.location.host, window.top.fiplexFactStr);
				tagsReq();
			}
		}
		xhr.onerror = function(ev) {
	...
	try {
		...
		xhr.onreadystatechange = function(ev) {
			if (this.readyState == 4 && this.status == 200) {
				...
				localStorage.setItem("tags_1cm"+window.location.host, window.top.fiplexTagsStr);
				confReq();
			}
		}
		Date.now = Date.now || function() { return +new Date; };
	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState == 4 && this.status == 200) {
				...
				localStorage.setItem("fibermask_1cm"+window.location.host, window.top.FiberMask);
				window.top.mode = parseMode(window.top.fiplexConfStr);
				localStorage.setItem("mode_1cm"+window.location.host, window.top.mode);
				...
				localStorage.setItem("cfg_1cm"+window.location.host, cfgarr[0]);
				window.parent.navi.configFrame = cfgarr[0];
				if (cfgarr.length>1) window.parent.navi.configRemFrame = cfgarr[1];
	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState == 4 && this.status == 200) {
				...
				localStorage.setItem("stat_1cm"+window.location.host, serverResponse);
