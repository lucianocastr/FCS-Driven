...
function optOnLoadFunc() {
	...
	fmask = localStorage.getItem("fibermask_1cm"+window.location.host);
	fibermask = fmask.split(",");
	mmask = localStorage.getItem("mode_1cm"+window.location.host);
	mode = mmask.split(",");
...
function optLoadStatus() {
	var serverResponse = localStorage.getItem("stat_1cm"+window.location.host);
...
function getRxLoss(idx)
{
	...
	var statstrs = (localStorage.getItem("stat_1cm"+window.location.host)).split("\t");
	if (idx<=6)
	{
		device=0;
		shift = 82*2 + (idx-1)*28;
