function Frequency(obj) {
	...
	this.setFrameStr = function(str) {
		...
		var fnSqEn = this.parseFnSqEn(str);
		var fnSqTh = this.parseFnSqTh(str);
		if (fnSqEn === null || fnSqTh === null) {
			return null;
		}
		this.frm[b] = str;
	}
	...
	this.parseFnSqTh = function(str) {
		try {
			var s = 2 + this.FNCH*4 + 2;
	...
	this.getFnSqEn = function(b) {
		return this.parseFnSqEn(this.frm[b]);
	}
	this.getFnSqTh = function(b) {
		return this.parseFnSqTh(this.frm[b]);
	}
	...
	this.setFnSqEn = function(b, sqen) {
		try {
			var s = 2 + this.FNCH*4;
	...
	this.setFnSqTh = function(b, sqth) {
		try {
			var s = 2 + this.FNCH*4 + 2;
			if (sqth > this.limitSqTh[b].MAX) {
				sqth = this.limitSqTh[b].MAX;
			} else if (sqth < this.limitSqTh[b].MIN) {
				sqth = this.limitSqTh[b].MIN;
			}
			var v = sqth;
			if (v < 0) {
				v += 0x100;
...
function Conf(obj) {
	...
	this.setFrameStr = function(str) {
		...
		var fnStby = this.parseChStby(str, this.FNCH);
		var fnGfine = this.parseChGfine(str, this.FNCH);
		if (fnStby === null || fnGfine === null) {
			return null;
		}
		this.frm[b] = str;
	}
	...
	this.getChannelsEnabledMask = function() {
		...
		if (this.getChActive(this.FNCH)) {
			activeMask |= (1 << this.FNCH);
		}
		return activeMask;
	}
