...
function Equ() {
	...
	this.render = function() {
		...
		for (var i = 0; i < 2; ++i) {
			for (var j = 0; j < 4; ++j) {
					try {
						var el = document.getElementById('coeffFnet'+i.toString()+j.toString());
						el.value = (this.fnetdata[i][j]);
					} catch (err) {}				
			}
		}
	}
	...
	this.createForm = function() {
		...
		cell = document.createElement("th");
		cell.style.textAlign = "right";
		cell.innerHTML = "FirstNet&nbsp;Coeff&nbsp;(real)";
		row.appendChild(cell);
		for (var j = 0; j < 2; ++j){
			for (var k = 0; k < 2; ++k) {
				cell = document.createElement("td");
				...
				el.size = 10;
				el.className = "number";					
				el.id = 'coeffFnet'+j.toString()+(2*k).toString();
				el.name = el.id;
				el.onkeypress = function(ev) {
					ev = ev || window.event;
					return isKeyDecimalNumber(ev);
		...
		cell = document.createElement("th");
		cell.style.textAlign = "right";
		cell.innerHTML = "FirstNet&nbsp;Coeff&nbsp;(imag)";
		row.appendChild(cell);
		for (var j = 0; j < 2; ++j){
			for (var k = 0; k < 2; ++k) {
				cell = document.createElement("td");
				...
				el.size = 10;
				el.className = "number";					
				el.id = 'coeffFnet'+j.toString()+(2*k+1).toString();
				el.name = el.id;
				el.onkeypress = function(ev) {
					ev = ev || window.event;
					return isKeyDecimalNumber(ev);
