...
function Page() {
	...
	this.createAlarmTable = function(nr){
		...
		for (var k=0;k<self.NFPA[nr].globalAlarmsInstalled.length;k++){
			...
			// if (k==12 && nr==0) show=false;
			if (k==12) {
				// si la versión de micro de master es < 2.00, no mostrar led de system alarm
				if (self.version.compareSw(2,0) < 0) {
					show = false;
				}
			}
			tb.appendChild(this.createGralAlarm(k,nr,self.NFPA[nr].alarmNames[0][k],show));
		}
		return cellb;
	}
	this.createRelayTable = function(nr){
		...
		row.appendChild(cell);		
		for (var k=0;k<((nr==0 && use7relaysInMaster(self.version))?7:4)*2;k++){
			var row = document.createElement("tr");
			tb.appendChild(row);
			if ((k % 2)==0){
				var cell = document.createElement("th");
	...
	this.showConfs = function(onlyFilterFields) {
		...
		self.setForceSystemAlarm(self.config[0].forceSystemAlarm);
	}
	...
	this.readTotalConfsFrm = function(ndev, isReset, isIsolVerif, isIsolClear, isClearErrors, isDelayMeas, forceSend, isForcePaOn, isForcePaOff) {
		var remoteFrame = "";
		var totalConfFrame = [];
		var cnfs = [];
		for (var i=0;i<=nrOfRemotes;i++)
			cnfs.push(new Config());
		cnfs[0] = self.readConfsFrm(0, ndev==0, isReset, isIsolVerif, isIsolClear, isClearErrors, isDelayMeas, forceSend, isForcePaOn, isForcePaOff);
		...
		for (var i = 0; i < nrOfRemotes; i++) {
			...
			if ( monitor.monitorUnit[0].FOmoduleConnected[i] && remoteGlobalConfResponseValid[i] ){
				cnfs[i+1] = self.readConfsFrm(i+1,ndev==(i+1), isReset, isIsolVerif, isIsolClear, isClearErrors, isDelayMeas, forceSend, isForcePaOn, isForcePaOff);
				remoteHeader += hexformat(1, 2);
				//setting read-only parameters
				cnfs[i+1].remoteIsDonnorAntennaCapable = self.config[i+1].remoteIsDonnorAntennaCapable;
				remoteFrame = cnfs[i+1].getFrm(self.factory[i+1],i+1);
				if(remoteFrame!=config[i+1].frm){
					console.log(remoteFrame);
					console.log(config[i+1].frm);
					totalConfFrame.push(remoteHeader+remoteFrame);
				}
			}
		}
		if (!isReset && !isIsolVerif && !isIsolClear && !isClearErrors && !isDelayMeas){ //si son acciones no se chequea overlap ni clase B
	...
	this.readConfsFrm = function(nr, validArgs, isReset, isIsolVerif, isIsolClear, isClearErrors, isDelayMeas, forceSend, isForcePaOn, isForcePaOff) {
		var cnf = new Config();
		...
		if (nr==0) cnf.forceSystemAlarm = self.getForceSystemAlarm();
	...
	this.showStatus = function(monitor) {
		...
		for (var nr=0;nr<=nrOfRemotes;nr++){
			...
			for (var k=0;k<((nr==0 && use7relaysInMaster(self.version))?7:4);k++){
	...
	this.getAdjChConf = function(cnf, c, band, ncfg, nr) {
		...
		var br = (nr==0 || self.isFreqSplitFixed(band,0))?1:0;
		if (on !== null) {
			cnf.filterEnabled[1][band][0][c] = on;
			cnf.filterEnabled[1][band][1][c] = on;
		}
	...
	this.setForceSystemAlarm = function(on){
		try {
			document.getElementById("forceSystemAlarm").checked = !!on;
			document.getElementById("forceSystemAlarmLabel").innerHTML = (on?"ON":"OFF");
			document.getElementById("forceSystemAlarmLabel").style.color = (on?"white":"black");
			document.getElementById("forceSystemAlarmDiv").style.backgroundColor = (on?"red":"#bbbbbb");
		} catch(e){}
	}
	this.getForceSystemAlarm = function(){
		try { return document.getElementById("forceSystemAlarm").checked;} catch(e){return false;}
	}
