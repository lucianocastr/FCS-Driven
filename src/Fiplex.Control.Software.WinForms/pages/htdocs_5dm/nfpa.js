		...
		for (var mode=0;mode<2;mode++){
			...
			for (var num=0;num < numAlarm;num++){
				...
				if ( (nr==0 && mode==1 && (num==0 || num==6  || num==7 || (num>=2 && num<=4))) ||
					...
					(mode==0 && num==12 && nr==0))
				{
					//checkbox system alarm deshabilitado en las alarmas inexistentes y la propia system alarm en master
					try {
						var el = document.getElementById("systemAlarm_"+num+"_"+mode+"_0_"+nr);
						el.checked = false;
						el.disabled = true;
					}catch(e){}
				}
...
function GUI_NFPA() {
	...
	this.readConf = function(nfpa,nr){
		...
		for (var k=0;k<24;k++){
			...
			nfpa.generalSystemAlarm[0][k] = document.getElementById("systemAlarm_"+k+"_0_0_"+nr).checked;
			for (var j=0;j<((nr==0 && use7relaysInMaster(version[0]))?7:4);j++) {
		...
		for (var k=0;k<16;k++){
			...
			nfpa.generalSystemAlarm[1][k] = document.getElementById("systemAlarm_"+k+"_1_0_"+nr).checked;
			for (var j=0;j<((nr==0 && use7relaysInMaster(version[0]))?7:4);j++)
		...
		for (var nr=0;nr<=nrOfRemotes;nr++){
			for (var band=0;band<2;band++){
				...
				for (var k=0;k<24;k++){
					...
					for (var j=0;j<((nr==0 && use7relaysInMaster(version[0]))?7:4);j++) document.getElementById("relay_enabled_"+k+"_0_"+j+"_"+nr).checked = nfpa[nr].relayAssignGlobalAlarm[k][j];
					if ( k >= 8 && k <= 11 ) {
						var externalAlarmNr = k - 8;
						document.getElementById("extAlarmPolarity_"+externalAlarmNr+"_"+nr).checked = nfpa[nr].externalAlarmPolarity[externalAlarmNr];
					}
					document.getElementById("systemAlarm_"+k+"_0_0_"+nr).checked = nfpa[nr].generalSystemAlarm[0][k];
				...
				for (var k=0;k<16;k++){
					...
					for (var j=0;j<((nr==0 && use7relaysInMaster(version[0]))?7:4);j++) document.getElementById("relay_enabled_"+k+"_1_"+j+"_"+nr).checked = nfpa[nr].relayAssignBandAlarm[k][j];
					document.getElementById("systemAlarm_"+k+"_1_0_"+nr).checked = nfpa[nr].generalSystemAlarm[1][k];
				}
	...
	this.createUnit = function(cont,nr) {
		...
		var headDiv = this.createUnitHead("ALARM&nbsp;THRESHOLDS","","","althres_"+nr,nr,true,id);
		unitDiv.appendChild(headDiv);
		var contentDiv = document.createElement("div");
		contentDiv.id = id;
		unitDiv.appendChild(contentDiv);
	...
	this.createGeneralRelayConfig = function(tb, mode,nr){
		...
		cell.colSpan = ((nr==0 && use7relaysInMaster(version[0]))?24:18);
		row.appendChild(cell);
		var row = document.createElement("tr");
		tb.appendChild(row);
		for (var k=0;k<2;k++){
			...
			var cell = document.createElement("th");
			cell.className = "alarmTable";
			cell.innerHTML = "System<br/>Alarm";
			cell.rowSpan = 2;
			row.appendChild(cell);
			var cell = document.createElement("th");
			cell.className = "alarmTable";
			cell.innerHTML = "Relay";
			cell.colSpan = ((nr==0 && use7relaysInMaster(version[0]))?7:4);
			row.appendChild(cell);	
			var cell = document.createElement("th");
			cell.className = "alarmTable";
		...
		for (var j=0;j<nrows;j++) {
			...
			hideCond = hideCond || (nr>0 && j>6); //en los remotos se oculta a partir de séptima fila
			hideCond = hideCond || (nr==0 && mode==0 && (j==3 || j>10)); //en los master se oculta fila 3(antigua alarma fiber 1 y 2) ylas 1 últimas filas
			row.id = "alarmRow_"+nr+"_"+mode+"_"+j;
			if (hideCond) { 
				row.style.display = "none";
				increaseAlarmNr = false;
			} else if (nr>0 && mode==0 && (j == 5 || j == 6)) {
				isUserAlarm = true;
				increaseAlarmNr = false;
			...
			for (var k=0;k<2;k++){
				...
				hideCell = hideCell || (nr>0 && j==4 && k==1); //en remotos general, se quita la celda derecha a other remotes alarms
				hideCell = hideCell || (nr==0 && j==10 && k==1); //en master general, se quita la celda derecha a other remotes alarms
				// si la versión de micro del master es <2.00 ocultar la celda de general system alarm
				if ((mode==0 && nr==0 && j==10 && k==0) || 	// en master
					(mode==0 && nr>0 && j==4 && k==0))	// y en remoto
				{
					hideCell = hideCell || (version[0].compareSw(2,0) < 0);
				}
				var cell = document.createElement("th");
				if ( isUserAlarm ) {
					var userAlarmNr = 2*(j-(nr==0?8:5)) + k + 1;
				...
				if (mode==0 && nr>0){
					var ord = [0,1,2,3,4,5,6,7,12,13,8,9,10,11,14,15];
					ix = ix>15?ix:ord[ix];
				}
				if (mode==1 && nr>0){
				...
				row.appendChild(cell);
				var cell = this.createInputText("AlName",ix,mode,nr); //Se cambia ordenación (*)
				row.appendChild(cell);
				cell.className = "alarmTable";
				if (hideCell) {
					cell.style.display = "none";
				}
				var cell = this.createEnabledCheckBox(ix,mode,0,nr); //Se cambia ordenación (*)
				row.appendChild(cell);
				cell.className = "alarmTable";
				...
				var cell = this.createSystemCheckBox(ix,mode,0,nr); //Se cambia ordenación (*)
				row.appendChild(cell);
				cell.className = "alarmTable";
				cell.style.textAlign = "center";
				cell.style.minWidth = "40px";
				...
				for (var i=0;i<((nr==0 && use7relaysInMaster(version[0]))?7:4);i++){
	...
	this.createSystemCheckBox = function(num,mode,enav,nr){
		var cell = document.createElement("td");
		var el = document.createElement("input");
		el.id = "systemAlarm_"+num+"_"+mode+"_"+enav+"_"+nr;
		el.name = el.id;
		el.type = "checkbox";
		el.enav = enav;
		cell.appendChild(el);
