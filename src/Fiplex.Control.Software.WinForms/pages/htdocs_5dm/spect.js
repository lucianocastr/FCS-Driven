...
function onloadInit() {
	...
	if (typeof(window.parent.navi.confFrame) !== 'undefined') {
		config.parse(window.parent.navi.confFrame,factory);
	} else {
		reloadData();
		return;
	}
				...
				if (this.status === 200) {
					...
					if (serverResponse == "NACK") {
						...
						spectrum.parse(dummyResponse);
	 					...
	 					guiBlocked(false);
						showRefresh();
					} else if (serverResponse.length < spectrum.nchannels * 2) {
...
function Spectrum() {
	...
	this.createDeviceSelect = function() {
		...
		el.id = "selDevice";
		el.name = el.id;
		for (var deviceNr=1; deviceNr <= nrOfRemotes; deviceNr++){
			var opt = document.createElement("option");
			el.options.add(opt);
			var tag = localStorage.getItem("Tag_"+deviceNr+"_"+Prjstr+window.location.host);
			if (tag==null || typeof(tag)==="undefined") tag = "";
			opt.text = deviceNr+".- "+tag;
			opt.value = deviceNr;
		}
