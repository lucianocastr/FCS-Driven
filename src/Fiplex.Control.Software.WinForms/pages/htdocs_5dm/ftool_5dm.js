function ftoolGui() {
	...
	this.initialize = function() {
		...
		this.factory = [];
		this.config = [];
		for ( var i = 0; i <= this.nrOfRemotes; i++ ) {
			var str = localStorage.getItem("Factory_"+i+"_"+Prjstr+window.location.host);
			if (typeof(str) !== 'undefined' && str !== null) {
				this.factory.push(new Factory(str));
			}else{
				this.factory.push(new Factory());
			}
			var str = localStorage.getItem("Conf_"+i+"_"+Prjstr+window.location.host);
			this.config.push(new Config());
			if (typeof(str) !== 'undefined' && str !== null) {
				this.config[i].parse(str,this.factory[i],i);
			}
		}
		this.maxChannels = this.config[0].CHNR;
		...
		for (var band=0;band<2;band++)
			this.toolband.push(this.factory[0].chBandEnabled[band] && this.config[0].uldlLinkedFreq[band]);

		
		if ((this.factory[0].fstop[1]-this.factory[0].fstart[1])!=(this.factory[0].fstop[0]-this.factory[0].fstart[0])) this.toolband[0] = false;
		if ((this.factory[0].fstop[3]-this.factory[0].fstart[3])!=(this.factory[0].fstop[2]-this.factory[0].fstart[2])) this.toolband[1] = false;
		if (this.toolband[0] && !this.toolband[1]) this.band=0;
		if (this.toolband[1] && !this.toolband[0]) this.band=1;
		this.BWKHZ = this.factory[0].commonUl ? 150:100;
		this.bwIndex = this.factory[0].commonUl ? 0:1;
		this.type = this.factory[0].commonUl ? 1:0;
		this.FilterValidSep = FilterValidSeparation[this.type][this.band];
		if (this.factory[0].singleBandEnabled[this.band]) this.maxChannels = 2*this.config[0].CHNR; 
	...
	this.createBandSelect = function(){
		...
		for (var band=0;band<2;band++){
			...
			el.options.add(opt);
			opt.text = self.factory[0].bandNames[band];
			opt.value = band;
		}
		el.value = this.band;
		...
		el.onchange = function(ev) {
			...
			self.FilterValidSep = FilterValidSeparation[self.type][self.band];
	...
	this.carriersAreaSetTitle = function(b, el) {
		var fstart = (self.factory[0].fstart[2*this.band+b] / 1e6).toFixed(6);
		var fstop = (self.factory[0].fstop[2*this.band+b] / 1e6).toFixed(6);
	...
	this.checkOutOfBand = function(cl, b, band) {
		var fstart = self.factory[0].fstart[2*band+b];
		var fstop = self.factory[0].fstop[2*band+b];
		var outofband = [];
	...
	this.computeNrFiltBw3dB = function(bw, band) {
		var fgr = [];
		fgr.push(self.filterTypes[self.type][band][self.bwIndex]['data'][1]*1e3);
		for (var i = 8; i < self.filterTypes.length; ++i) {
			fgr.push(self.filterTypes[i]['data'][1]*1e3);
		}
	...
	this.showProposal = function(gfilts) {
		var str = "Filter Bandwdith: "+self.BWKHZ+" KHz \n";
	...
	this.applyFreqs = function(band) {
		...
		for (var nr=0;nr<=self.nrOfRemotes;nr++){
			self.config[nr].uldlLinkedFreq[band] = isSplitFixed;
			var b = self.b;
			var a = (b + 1) % 2;
			for (var c = 0; c < chList.length; ++c) {
				var chnr = chList[c];
				var stp = self.factory[0].fstep;
				if (stp<=1.5e3) stp/=2;
				self.config[nr].freqHz[band][b][c]=self.factory[0].fref[2*band+b]+stp*chnr;
				self.config[nr].freqHz[band][a][c]=self.config[nr].freqHz[band][b][c]-self.factory[nr].fstart[2*band+b]+self.factory[nr].fstart[2*band+a];
			}
		}
	}
	this.applyConfs = function() {
		for (var nr=0;nr<=self.nrOfRemotes;nr++){
			for (var b = 0; b < 2; ++b) {
				self.setBandConf(self.config[nr], b, self.proposal, self.band);
			}
			for (var c = 0; c < self.maxChannels; ++c) {
				self.setChConf(self.config[nr], c, self.proposal, self.band);
			}
		}
	}
	...
	this.computeChFreq = function(chnr, b, band) {
		var fo = self.factory[0].fref[2*band+b];
		var stp = self.factory[0].fstep;
		if (stp<=1.5e3) stp/=2;
		var fr = chnr * stp + fo;
		return fr;
	...
	this.makeSubmitFrm = function() {
		...
		for (var nr=0;nr<=self.nrOfRemotes;nr++){
			...
			self.frms.push({type: 'ctl_conf_str=', frame: header+self.config[nr].getFrm(self.factory[nr],nr)});
		}
	...
	this.makeStrFrm = function() {
		for (var nr=0;nr<=self.nrOfRemotes;nr++){
			var toolConfFrame = self.config[nr].getFrm(self.factory[nr],nr);
			self.config[nr].saveFrameStr(toolConfFrame,nr);
		}
	}
	this.saveSettingVals = function() {
		...
		var band = this.band;
		if (this.factory[0].chBandEnabled[0] && this.factory[0].chBandEnabled[1] 
		&& this.config[0].uldlLinkedFreq[0] && this.config[0].uldlLinkedFreq[1]){
