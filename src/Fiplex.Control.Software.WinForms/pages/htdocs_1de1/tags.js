		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState==4 && this.status === 200) {
				...
				try {
					...
					var locs = parse_locs(window.top.fiplexTagsStr);
					tagsRender(tags);
					locsRender(locs);
				} catch(err) {};
				initFormChangeCheck();
...
function parse_tags(serverResponse) {
	...
	if (sr.length>(2*TAGLEN)) sr = sr.substr(0,2*TAGLEN);
	var tagstrs = tag_read(sr);
	return tagstrs;
}
...
function parse_locs(serverResponse) {
	var LOCLEN = window.top.LOCLEN;
	var TAGLEN = window.top.TAGLEN;
	var sr = serverResponse;
	if (sr.length>2*(TAGLEN+LOCLEN)) sr = sr.substr(0,2*(TAGLEN+LOCLEN));
	var loc = loc_read(sr);
	return loc;
}
function locsRender(locs) {
	try {
...
function format() {
	...
	var loc = document.getElementById("locName").value;
	tag = tag.trim();
	loc = loc.trim();
