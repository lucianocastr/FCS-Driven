...
function Tag() {
	...
	this.set = function(hextag) {
		var sr = hextag;
		if (sr.length>2*this.TAGLEN) sr = sr.substr(0,2*this.TAGLEN);
		try {
			var t = '';
...
function fiberStatusTable() {
	...
	this.statusLedEntries = [
		{H: "Loss&nbsp;of&nbsp;Optical&nbsp;Signal",	I: "foLos" 	},
		{H: "Loss&nbsp;of&nbsp;Communications",		I: "lossComm"	},
		{H: "FO&nbsp;Transc.&nbsp;Status",			I: "foStatFail"	},
		{H: "Sync&nbsp;to&nbsp;Master",			I: "optFrSync"	},
		{H: "Data&nbsp;Lock",				I: "dataLock"	},
		{H: "Data&nbsp;Sync", 				I: "dataSync"	},
		{H: "FO&nbsp;Tx&nbsp;Fault", 		I: "txFaultAlarm"	},
		{H: "GTP&nbsp;PLL&nbsp;Lock", 		I: "gtpSyncAlarm"	}
	];	
	...
	this.statusMsrEntries = [
		{H: "Error&nbsp;count",		I: "errors"	},
		{H: "Supply&nbsp;Voltage",	I: "voltage"	},
		{H: "Bias&nbsp;Current",	I: "bias"	},
		{H: "Temperature&nbsp;(&ordm;C)",I: "temperat"	},
	...
	this.showData = function(st, f) {
		...
		for (var i = 0; i < this.statusLedEntries.length; ++i) {
			var alarm;
			switch (i) {
			case 0: alarm = d.foLosAlarm ; break;
			case 1: alarm = d.lossComm; break;
			case 2: alarm = d.foStatusFail; break;
			case 3: alarm = d.freqSyncAlarm; break;
			case 4: alarm = d.dataLock; break;
			case 5: alarm = d.dataSync; break;
			case 6: alarm = d.txFaultAlarm; break;
			case 7: alarm = d.gtpSyncAlarm; break;
			}
			var id = this.statusLedEntries[i].I;
			ledSetColor(id, alarm == 2 ? "red" : alarm ==1 ? "yellow" : "green");			
		}
		for (var i = 0; i < this.statusMsrEntries.length; ++i) {
			var num,alarm;
			switch (i) {
			...
			try {
				document.getElementById(id).innerHTML = num.toFixed(p);
				document.getElementById(id).style.color = (alarm==2) ? "#df4040" : (alarm==1)?"#eeee88":"#802000";
			} catch (err) {}
		}
	}
}
	...
	this.showLeds = function(st) {
		for (var i = 0; i < this.SFPNR; ++i) {
			...
			if (!(st.data.detmod & (1 << i))) {
				...
				var alarm = (d.lossComm | d.foLosAlarm | d.foStatusFail); //Se sustituye or por or bit a bit
				//var warning = d.rxLossWarning;
				c = (alarm & 0x2? "red" : (alarm & 0x1 ? "yellow" : "green"));
			}
			ledSetColor(id, c);
		}
	}
...
function Temperature() {
	...
	this.set = function(val, frmAlarm) {
		try {
			var alarm = frmAlarm || val > this.ALARMTEMP;
			ledSetColor("boardTempLed", alarm ?  "red" : "grey");
			var el = document.getElementById("boardTempMet");
			if (typeof(el) != "undefined" && el) {
				var elmet = el.mMeter;
				if (!isNaN(val))
					elmet.valueSet(val,alarm ?  "#df4040" : "rgb(43,194,83)");
				else
...
function hwFail() {
	...
	this.create = function() {
		...
		td.style.textAlign = "right";
		tr.appendChild(td);
		var c = createLedBox("door");
		tr.appendChild(c);		
		var tr = document.createElement("tr");
		tb.appendChild(tr);		
		var td = document.createElement("th");
		td.innerHTML = "Comm.&nbsp;Error";
		td.style.textAlign = "right";
		tr.appendChild(td);
		var c = createLedBox("commError");
		tr.appendChild(c);		
		return box;
	}
	this.getContainer = function() {
	...
	this.setDoor = function(alarm) {
		ledSetColor("door", alarm ? "red" : "grey");
	}	
	this.setCommError = function(alarm) {
		ledSetColor("commError", alarm ? "red" : "grey");
	}	
	this.hideDoor = function(val) {
		//Nunca se oculta el LED Door Open
		//document.getElementById("doorRow").style.display = val?"table-row":"none";
	}
	this.container = this.create();
}
...
function renderStatus(st) {
	try {
		temperature.set(st.data.temperature, st.data.tempAlarm);
		hwfail.set(st.data.hwfail);
		hwfail.setDoor(st.data.doorAlarm);
		//hwfail.hideDoor(st.data.doorAlarmAvailable);
		hwfail.setCommError(st.data.commError);
		flt.showData(st);
