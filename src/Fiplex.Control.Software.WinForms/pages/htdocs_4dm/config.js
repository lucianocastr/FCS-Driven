...
function overallConfigToTextFile(config, nfpacfg, factory, tagstr, ethstr, serial, version, nr){
	...
	cfg += alarmSettingsToText(nfpacfg,factory, nr) + "\n";
	if (nr==0) cfg += ethConfigToText(ethstr)+ "\n";
...
function alarmSettingsToText(nfpacfg,factory, nr){
	var j,k;
	var exist_el;
	var cfg = "";
...
function rfConfigToText(config, nfpacfg, factory, nr){
	...
	cfg += rfConfigGeneralAlarmToText(config, nfpacfg, nr);
	return cfg;
}
function rfConfigGeneralAlarmToText(config, nfpacfg, nr){
	var cfg = "";
	cfg+="CONFIG GENERAL:\n";
	...
	if (nr==0) cfg+="\tAntenna Disconnection Input Threshold (dBm): " + (nfpacfg.antennaDisconnectionThreshold[0]) + "\n";
