function ftoolGui() {
	...
	this.groupCarriers = function(carrList) {
		...
		for (var i = 1; i < carrList.length; ++i) {
			if ((~~Math.round((carrList[i] - carrList[i-1])*1e6)) <= this.FILTSEP90K*1e3) {
				g.push(carrList[i]);
			} else {
				gList.push(g);
				g = [];
	...
	this.computeNrFiltBw3dB = function(bw) {
		var fgr = [];
		fgr.push(self.filterTypes[0]['data'][1]*1e3);
	...
	this.showProposal = function(gfilts) {
		var str = "Filter Bandwdith: 150 KHz \n";
	...
	this.applyFreqs = function(band) {
		...
		for (var nr=0;nr<=self.nrOfRemotes;nr++){
			...
			for (var c = 0; c < chList.length; ++c) {
				var chnr = chList[c];
				self.config[nr].freqHz[band][b][c]=self.factory[nr].fref[2*band+b]+self.factory[nr].fstep*chnr;
				chnr = self.computeChNrOtherBand(chnr, b, c, band);
				self.config[nr].freqHz[band][a][c]=self.factory[nr].fref[2*band+a]+self.factory[nr].fstep*chnr;
			}
		}
	}
	...
	this.computeChFreq = function(chnr, b, band) {
		...
		var fstep = self.factory[0].fstep;
	...
	this.computeChNrOtherBand = function(chnr, b, c, band) {
		var fr = self.computeChFreq(chnr, b, band);
		var diff = fr - self.factory[0].fstart[2*band+b];
		var a = (b + 1) % 2;
		fr = self.factory[0].fstart[2*band+a] + diff;
