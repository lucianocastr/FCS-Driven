...
function renderConf(renderAgain, channelsEnabledOnly, fact) {
	...
	if (renderAgain || !isRendered) {
		...
		// renderPage();
		view.create(fact);
		view.showTag(tags.tag);
		isRendered = true;
	}
	// config.render();
	// tags.render();
	view.showConf(config);
	initFormChangeCheck();
}
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState==4) {
				if (this.status === 200) {
					...
					// monitor.render();
					view.showStatus(monitor, config);
				}
				tmrIdStat = setTimeout(function() { load_status(); }, 1000);
			}
		}
...
function submitform(isReset) {
	...
	var frm = view.formatConf(window.top.fiplexConfStr);
	if (doReset)
		frm = "07"+frm.substr(2);
	doSubmit(frm);
}
...
function clicksDisable(isDisable) {
	try {
		view.resetDisableStateSet(isDisable);
		view.hpaSwDisableStateSet(i, isDisable);
		// for (var i = 0; i < 2; ++i) {
