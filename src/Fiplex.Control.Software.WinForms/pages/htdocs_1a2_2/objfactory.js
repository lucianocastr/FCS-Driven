...
function Factory() {
	...
	this.frameMap = [
		...
		{id:'spectWarn',label:'PA&nbsp;CURRENT&nbsp;/&nbsp;SPECTRUM&nbsp;WARNING&nbsp;',len: 2, data: [], parse: function(s) {return cSignedByte(s);}, format: function(s) {return rSignedByte(s);} },
		...
		{id:'attIn', 	label:'INPUT&nbsp;ATTENUATOR&nbsp;'		,	len: 2, data: [], parse: function(s) {return s;},		format: function(s) {return s;} },
		...
		{id:'inpAtt',	label:'INPUT&nbsp;DIGITAL&nbsp;ATTENUATOR&nbsp;',	len: 2, data: [], parse: function(s) {return s;}, 		format: function(s) {if (s < 0) return 0; else if (s > 30) return 30; else return s;},  	title: "[0..30]" },
		{id:'trackNoise',label:'TRACK&nbsp;NOISE&nbsp;OFFSET&nbsp;'	,	len: 2, data: [], parse: function(s) {return cSignedByte(s);},	format: function(s) {if (s < -128) s = -128; if (s > 127) s = 127; return rSignedByte(s);},	title: "[-128..127]"},
		{id:'persist',	label:'ABNORMAL&nbsp;SIGNAL&nbsp;PERSISTENCE&nbsp;TIME&nbsp;',len: 2, data: [], parse: function(s) {return s;}, 	format: function(s) {if (s < 0) return 0; else if (s > 255) return 255; else return s;},	title: "[0..255]"}
	];
	this.data = {
		band: 		[],
		fmodulo:	0,
	...
	this.parse = function(sr) {
		...
		for (var i = 0; i < 2; ++i) {
			...
			this.data.band[i].attIn = 	this.frameMap[n++].data[i];
			...
			this.data.band[i].hpa_pol_inv = (k & 0x20) != 0;
		}
		...
		this.data.inpAtt = 	this.frameMap[n++].data[0];
		this.data.trackNoise = 	this.frameMap[n++].data[0];
		this.data.persist = 	this.frameMap[n++].data[0];
	}
	this.offOnOptions = ["OFF", "ON"];
	this.dacspectOptions = ["INVERTED", "NORMAL"];
	this.hpamonOptions = [["PA", "FPGA", "FIP413"], ["PA", "FPGA", "FIP413"]];
	...
	this.createForm = function() {
		...
		for (var i = 0; i < this.factbandmapLen; ++i) {
			...
			if (this.frameMap[i].id == 'powerLim') {
				...
				cell = document.createElement("th");
				row.appendChild(cell);
				cell.innerHTML = 'CHANNEL&nbsp;REDUCTION&nbspENABLE&nbsp;';
				cell = document.createElement("td");
				row.appendChild(cell);
				el = document.createElement("select"); 
				el.className = "centered";
			...
			if (this.frameMap[i].id == 'gainLim') {
				...
				cell = document.createElement("th");
				row.appendChild(cell);
				cell.innerHTML = 'PA&nbsp;POLARITY&nbsp;INVERT&nbsp;';
				for (var j = 0; j < 2; ++j) {
					cell = document.createElement("td");
					row.appendChild(cell);
					el = document.createElement("input");
		...
		for (var i = this.factbandmapLen; i < this.frameMap.length; ++i) {
			...
			el.name = el.id;
			if (typeof(this.frameMap[i].title) !== 'undefined') {
				el.title = this.frameMap[i].title;
			}
			cell.appendChild(el);
	...
	this.render = function() {
		...
		try {
			for (var i = 0; i < 2; ++i) {
				...
				el = document.getElementById(this.frameMap[n++].id + i);
				el.value = this.data.band[i].attIn;
				el = document.getElementById(this.frameMap[n++].id + i);
			...
			for (var i = 0; i < 2; ++i) {
				el = document.getElementById("paPolInv_"+i);
				el.checked = this.data.band[i].hpa_pol_inv;				
			}
			...
			el = document.getElementById(this.frameMap[n++].id);
			el.value = this.data.inpAtt;
			el = document.getElementById(this.frameMap[n++].id);
			el.value = this.data.trackNoise;
			el = document.getElementById(this.frameMap[n++].id);
			el.value = this.data.persist;
		} catch(err) { }
	}
		...
		try {
			...
			for (var n = 0; n < this.factbandmapLen; n++) {
				...
				for (var i = 0; i < 2; ++i) {
					...
					else if (this.frameMap[n].id == 'attIn') 	num = parseInt(el.value);
			...
			for (var n = this.factbandmapLen; n < this.frameMap.length; n++) {
				...
				else if (this.frameMap[n].id == 'inpAtt') 	num = parseInt(el.value);
				else if (this.frameMap[n].id == 'trackNoise') 	num = parseInt(el.value);
				else if (this.frameMap[n].id == 'persist') 	num = parseInt(el.value);
				str = isNaN(num) ? currentStr.substr(p, sz) : hexformat(this.frameMap[n].format(num), sz);
				p += sz;
				frame += str;
			}
	...
	this.isFipPaMon = function() {
		return this.data.band[0].fipmon;
	}
}
// -->
