		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState==4) {
				if (this.status === 200) {
					...
					if (typeof(spectrum) === "undefined" || !spectrum)
						spectrum = new Spectrum(maxChNr);
					if (serverResponse.length != (2*spectrum.nchannels + 1)*2) {
...
function Spectrum(maxChNr) {
	...
	this.createChannelBars = function(box) {
		...
		for (var c = 0; c < config.maxChannels; ++c) {
			...
			var fr = config.computeChFreq(frNr, b);
	...
	this.computeInput = function(rawData) {
		for (var i = 0; i <= this.nchannels; ++i) {
			...
			this.inputSpectrum[i] = rawData[i] + this.computeInputCorrection(fr);
		}
	}
	this.computeInputCorrection = function(fr) {
		var b = 0;
		var frqNr = config.computeChNum(fr, b);
		var fstepfactor = ~~Math.round(this.FreqStepHz / factory.data.fstep);
		if (fstepfactor == 0) fstepfactor = 1;
		var idx = this.findNearestActiveChannelIndexInRange(frqNr, 80*fstepfactor);
		if (idx < 0)
			return 0;
		var g2alpha = Math.pow(10, (monit.stat.band[b].ch[idx].gain - monit.stat.band[b].ch[idx].isolMod) / 20);
		var bw = config.conf.band[b].ch[idx].bandwidth;
		var tau = 0.0000116 * (bw + 1) + 0.0000034 + 0.000004 * factory.data.band[b].delayOffset;
		var ch = config.conf.band[b].ch[idx].frqNr;
		var ch_offset = ~~Math.round((frqNr - config.conf.band[b].ch[idx].frqNr)/fstepfactor);
		var abs_ch_offset = Math.abs(ch_offset);
		var chcorravg = new Array(2);
		var chcorr = 0;
		chcorravg[0] = 0;
		chcorravg[1] = 0;
		var freq = -(this.FreqStepHz * ch_offset);
		var ph = monit.stat.band[b].ch[idx].isolPh * Math.PI / 180;
		chcorravg[0] = 1 + this.tableFilters[bw][abs_ch_offset] * g2alpha * Math.cos(ph + 2 * Math.PI * freq * tau);
		chcorravg[1] = -this.tableFilters[bw][abs_ch_offset] * g2alpha * Math.sin(ph + 2 * Math.PI * freq * tau);
		chcorr += chcorravg[0] * chcorravg[0] + chcorravg[1] * chcorravg[1];
		fcorr = -10 * Math.log(chcorr+0.1) / Math.LN10;
		return fcorr;
	}
	this.findNearestActiveChannelIndexInRange = function(chNr, range) {
		var b = 0;
		var minFrqNr = config.computeChNum(factory.data.band[b].fStart, b);
		var maxFrqNr = config.computeChNum(factory.data.band[b].fStop, b);
		for (var i = 0; i < range; ++i) {
			for (j = 0; j < 2; ++j) {
				var num = chNr + i*(j ? 1 : -1);
				if (num < minFrqNr || num > maxFrqNr)
					continue;
				var idxList = config.indexOfChannel(b, num);
	...
	this.isChannelOnAndActive = function(idx) {
		var b = 0
		return config.isChannelOnAndActive(b, idx, monit.stat.band[b].ch[idx].signalIn);
	}
	...
	this.computeGain = function(fr) {
		...
		for (var i = 0; i < this.maxChNr; ++i) {
			...
			var gainch = Math.pow(10, monit.stat.band[b].ch[i].gain / 20);
			if (i == idx) {
				var bw = config.conf.band[b].ch[idx].bandwidth;
				var ch_offset = ~~Math.round(Math.abs(frqNr - config.conf.band[b].ch[idx].frqNr) / fstepfactor);
	...
	this.createBandLimitsForm = function() {
		...
		var fstartspec = localStorage.getItem("start_spec_1a2"+window.location.host);
		var fstopspec = localStorage.getItem("stop_spec_1a2"+window.location.host);
		var b = 0;
		if (fstartspec==null)
			fstartspec=factory.data.band[b].fStart;
		else{
			fstartspec = fstartspec<factory.data.band[b].fStart?factory.data.band[b].fStart:fstartspec;
			fstartspec = fstartspec>factory.data.band[b].fStop?factory.data.band[b].fStart:fstartspec;
		}
		if (fstopspec==null)
			fstopspec=factory.data.band[b].fStop;
		else{
			fstopspec = fstopspec>factory.data.band[b].fStop?factory.data.band[b].fStop:fstopspec;	
			fstopspec = fstopspec<factory.data.band[b].fStart?factory.data.band[b].fStop:fstopspec;	
		}
		...
		localStorage.setItem("start_spec_1a2"+window.location.host,this.FreqMinHz);
		localStorage.setItem("stop_spec_1a2"+window.location.host,this.FreqMaxHz);
	};
	...
	this.reSetBandLimits = function(ev) {
		...
		if (!target || (target && target.id == "fstart")) {
			...
			if (fs < factory.data.band[b].fStart) {
				fs = factory.data.band[b].fStart;
			} else if (fs >= factory.data.band[b].fStop) {
				fs = this.FreqMinHz;
			}
			...
			fp = fs + diff*1e6;
			if (fp > factory.data.band[b].fStop) {
				fp = factory.data.band[b].fStop;
				fs = fp - diff*1e6;
			}
		} else {
			...
			if (fp > factory.data.band[b].fStop) {
				fp = factory.data.band[b].fStop;
			} else if (fp <= factory.data.band[b].fStart) {
				fp = this.FreqMaxHz;
			}
		...
		localStorage.setItem("start_spec_1a2"+window.location.host,this.newFreqMinHz);
		localStorage.setItem("stop_spec_1a2"+window.location.host,this.newFreqMaxHz);
		var el = document.getElementById("fstart");
