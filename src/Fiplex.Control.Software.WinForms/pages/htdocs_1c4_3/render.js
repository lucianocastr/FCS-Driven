...
function Page() {
	...
	this.createShowFiltTable = function() {
		...
		var nch = self.isAdj() ? self.confs.CHNR : self.confs.CHFPGANR / 2;
		var ncols = 0;
		for (var r = 0, c = 0; r < nrows; ++r) {
			...
			if (self.isAdj()) {
				...
				str += (r == 0? "1":(self.confs.CHFPGANR / 2+1))+"-"+(r == 0? (self.confs.CHFPGANR / 2):self.confs.CHFPGANR)+")";
			}
			head.innerHTML = str;
	...
	this.createFilterTable = function(b) {
		...
		for (var r = 0; r < this.maxChannels; r++) {
			...
			if (r >= self.confs.CHFPGANR) do_show = false;
			if (!do_show) {
				row.style.display ="none";
			}
			tb.appendChild(row);
	...
	this.isFiltEnabled = function(c) {
		...
		try {
			var el = document.getElementById("filtEn_"+c);
			if (!self.isAdj() && c >= self.confs.CHFPGANR && c<self.confs.CHNR){ //Se limita al max num de canales FPGA
				on = false;
			}else{
				on = el.checked;
			}
	...
	this.BWtable = [
		{ix:   0, include: true, value:   0, txt: "150KHz", khz:  150},
		{ix:   1, include: true, value:   1, txt: "100KHz", khz:  100},
		{ix:   2, include: true, value:   2, txt: "75KHz", khz:  75},
		{ix:   3, include: true, value:   3, txt: "62.5KHz", khz:  62.5},
		{ix:   4, include: true, value:   4, txt: "50KHz", khz:  50},
		{ix:   5, include: true, value:   5, txt: "37.5KHz", khz:  37.5},
		{ix:   6, include: true, value:   6, txt: "25KHz", khz:  25},
	...
	this.isFilterOverlap = function(freq, b, n, c) {
		...
		if (b1 == 150 && b2 == 150) {
			...
			band = ~~Math.round((b1 + b2) * 1000 / 2 * 1.6);
			return (filtSep < band);
		}
	}
