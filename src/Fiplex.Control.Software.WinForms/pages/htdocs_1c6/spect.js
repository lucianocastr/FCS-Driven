...
function Spectrum() {
	...
	this.createChannelBars = function(box) {
		...
		for (var c = 0; c < config.maxChannels; ++c) {
			...
			if (config.bwType == 0) {
				...
				fr = config.computeChFreq(frNr, b);
			} else {
				var chnr = [];
	...
	this.createBandLimitsForm = function() {
		...
		for (var i = 0; i < 2; ++i) {
			...
			el.onchange = function(ev) {
				...
				localStorage.setItem("spect_fs_1c3_"+self.chain+window.location.host, self.newFreqMinHz);
				localStorage.setItem("spect_fp_1c3_"+self.chain+window.location.host, self.newFreqMaxHz);
			}
	...
	this.reSetBandLimits = function(ev) {
		...
		if (fi < factory.data.band[b].fStart || fi > factory.data.band[b].fStop) {
			fi = factory.data.band[b].fStart;
		}
		...
		if (self.zSpanSt) {
			...
			if (fp < factory.data.band[b].fStart || fp > factory.data.band[b].fStop) {
		...
		if (span < this.spanMinMHz*1e6 && span != 0) {
			if (!target || (target && target.id == "fstart")) {
				...
				if (fi < factory.data.band[b].fStart) {
					fi = factory.data.band[b].fStart;
	...
	this.createRbwForm = function() {
		...
		el.onchange = function(ev) {
			localStorage.setItem("spect_rbw_1c3"+window.location.host, this.selectedIndex);
		}
		var t = document.createElement("span");
		t.innerHTML = str;
		t.style.fontWeight = "bold";
	...
	this.createAvgForm = function() {
		...
		el.onchange = function(ev) {
			...
			this.value = v;
			localStorage.setItem("spect_avg_1c3"+window.location.host, v);
		}
		var t = document.createElement("span");
		t.innerHTML = "Avg.";
		t.style.fontWeight = "bold";
	...
	this.createChainForm = function() {
		...
		for (var i = 0; i < 2; ++i) {
			...
			el[i].onclick = function(ev) {
				...
				localStorage.setItem("spect_chain_1c3_"+window.location.host, self.chain);
				self.initSettings();
		...
		for (var i = 0; i < 2; ++i) {
			...
			el[i].onclick = function(ev) {
				try {
					...
					localStorage.setItem("spect_io_1c3_0"+window.location.host, io[0]? 1 : 0);
					localStorage.setItem("spect_io_1c3_1"+window.location.host, io[1]? 1 : 0);
				} catch(err) {}
			}
			t.push(document.createElement("span"));
	...
	this.createZspanForm = function() {
		...
		el.onclick = function(ev) {
			localStorage.setItem("spect_zspan_1c3_"+window.location.host, this.checked ? 1 : 0);
	...
	this.initSettings = function() {
		this.io = 1;
		var b = parseInt(localStorage.getItem("spect_chain_1c3_"+window.location.host));
		if (isNaN(b) || b < 0 || b > 1) {
			b = 1;
			localStorage.setItem("spect_chain_1c3_"+window.location.host, b);
		}
		this.chain = b;
		var fi = parseInt(localStorage.getItem("spect_fs_1c3_"+b+window.location.host));
		var fp = parseInt(localStorage.getItem("spect_fp_1c3_"+b+window.location.host));
		var span;
		if (!isNaN(fi) && !isNaN(fp) && fp >= fi
		    && fi >= factory.data.band[b].fStart && fi <= factory.data.band[b].fStop
		    && fp >= factory.data.band[b].fStart && fp <= factory.data.band[b].fStop) {
			span = fp - fi;
			this.FreqMinHz = this.newFreqMinHz = fi;
			this.FreqMaxHz = this.newFreqMaxHz = fp;
		} else {
			this.FreqMinHz = this.newFreqMinHz = factory.data.band[b].fStart;
			this.FreqMaxHz = this.newFreqMaxHz = factory.data.band[b].fStop;
			localStorage.setItem("spect_fs_1c3_"+b+window.location.host, this.newFreqMinHz);
			localStorage.setItem("spect_fp_1c3_"+b+window.location.host, this.newFreqMaxHz);
		}
		var zs = parseInt(localStorage.getItem("spect_zspan_1c3_"+window.location.host));
		if (isNaN(zs) || zs < 0 || zs > 1) {
			zs = 0;
			localStorage.setItem("spect_zspan_1c3_"+window.location.host, zs);
		}
		this.zSpanSt = zs != 0;
		var rbw = parseInt(localStorage.getItem("spect_rbw_1c3"+window.location.host));
		if (isNaN(rbw) || rbw < 0 || rbw >= this.MAXRES) {
			rbw = this.MAXRES - 1;
			localStorage.setItem("spect_rbw_1c3"+window.location.host, rbw);
		}
		this.resBw = rbw;
		this.spanStep = this.resBw;
		var v = parseInt(localStorage.getItem("spect_avg_1c3"+window.location.host));
		...
		this.ioSet = [];
		for (var i = 0; i < 2; ++i) {
			var io = parseInt(localStorage.getItem("spect_io_1c3_"+i+window.location.host));
			if (isNaN(io) || (io != 0 && io != 1)) {
				io = 1;
				localStorage.setItem("spect_io_1c3_"+i+window.location.host, io);
			}
			this.ioSet.push(io);
		}
	}
