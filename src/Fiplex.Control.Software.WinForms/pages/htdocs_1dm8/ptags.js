...
function parse_serials(serverResponse){
	...
	{
		var hfound = false;
		for (var i=1;i<serialstrs.length;i++){
			if (parseInt(serialstrs[i].substr(2,2),16)==n && serialstrs[i].substr(0,2)=="00"){
				serials[n] = serialstrs[i].substr(-15,15);
				hfound = true;
			}
		}
		if (!hfound) serials[n]="";
	...
	for (n=1; n<=window.top.remotesNr; ++n)
	{
		...
		if (!isNaN(n) && !(n < 0 || n > (window.top.MaxRemotesNr+window.top.MaxExpansorNr)))
		{
			var hfound = false;
			for (var i=1;i<serialstrs.length;i++){							
				if (parseInt(serialstrs[i].substr(0,2),16)==k && serialstrs[i].substr(2,2)=="01"){
					serials[k+window.top.MaxExpansorNr] = serialstrs[i].substr(-15,15);
					hfound = true;
				}
			}
			if (!hfound) serials[k+window.top.MaxExpansorNr]="";
		}
	}
}
function parse_tags(serverResponse) {
	try {
		...
		locs[0] = (tagstrs[0].substr(4+TAGLEN,LOCLEN));
		//expansores
		for (n=1; n<=window.top.expansorNr; ++n)
		{
			var hfound = false;
			for (var i=1;i<tagstrs.length;i++){
				if (parseInt(tagstrs[i].substr(2,2),16)==n && tagstrs[i].substr(0,2)=="00"){
					...
					locs[n] = (tagstrs[i].substr(4+TAGLEN,LOCLEN));
					hfound = true;
				}
			}
		}
			...
			if (!isNaN(n) && !(n < 0 || n > (window.top.MaxRemotesNr+window.top.MaxExpansorNr)))
			{
				...
				for (var i=1;i<tagstrs.length;i++){
					if (parseInt(tagstrs[i].substr(0,2),16)==k && tagstrs[i].substr(2,2)=="01"){
						...
						locs[k+window.top.MaxExpansorNr] = (tagstrs[i].substr(4+TAGLEN,LOCLEN));
						hfound = true;
					}
				}
				if (!hfound){
					tags[k+window.top.MaxExpansorNr] = "";
					locs[k+window.top.MaxExpansorNr] = "";
				}
			}
		}
	} catch(e) {}
...
function tagRender(n) {
	...
	else
	{
		...
		setLoc(n, locs[k+window.top.MaxExpansorNr])
	}
}
...
function setLoc(nr, name) {
	try {
		var loc = document.getElementById("locName"+nr);
		loc.value = name.trim();
	} catch (err) {}
}
	...
	try {
		...
		for (var i = 0; i <= MaxRemotesNr+window.top.MaxExpansorNr; ++i) {
			...
			/*if (document.getElementById("tagName"+i).value.length==0)
				continue;*/
			if (!document.getElementById("locName"+i))
				continue;			
			var tagstr, tag, loc;
			...
			{	
				...
				loc = document.getElementById("locName"+i).value;
			}
			else
			{
...
function getRedundedFiberRemotes(isIndex){
	var statstr = localStorage.getItem("stat_"+prjname+window.location.host);
