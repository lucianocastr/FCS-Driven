...
function Spectrum() {
	...
	this.createRbwForm = function() {
		...
		el.onchange = function(ev) {
			localStorage.setItem("spect_rbw"+prjname+window.location.host, this.selectedIndex);
		}
		var t = document.createElement("span");
		t.innerHTML = str;
		t.style.fontWeight = "bold";
	...
	this.createAvgForm = function() {
		...
		el.onchange = function(ev) {
			...
			this.value = v;
			localStorage.setItem("spect_avg_"+prjname+window.location.host, v);
		}
		var t = document.createElement("span");
		t.innerHTML = "Avg.";
		t.style.fontWeight = "bold";
	...
	this.createChainForm = function() {
		...
		for (var i = 0; i < 2; ++i) {
			...
			el[i].onclick = function(ev) {
				...
				localStorage.setItem("spect_chain_"+prjname+window.location.host, self.chain);
				self.initSettings();
		...
		for (var i = 0; i < 3; ++i) {
			...
			el[i].onclick = function(ev) {
				try {
					...
					localStorage.setItem("spect_io_"+prjname+"_0"+window.location.host, io[0]? 1 : 0);
					localStorage.setItem("spect_io_"+prjname+"_1"+window.location.host, io[1]? 1 : 0);
					localStorage.setItem("spect_io_"+prjname+"_2"+window.location.host, io[2]? 1 : 0);
				} catch(err) {}
			}
			t.push(document.createElement("span"));
	...
	this.createZspanForm = function() {
		...
		el.onclick = function(ev) {
			localStorage.setItem("spect_zspan_"+prjname+window.location.host, this.checked ? 1 : 0);
	...
	this.createRemoteForm = function() {
		...
		el.onchange = function(ev) {
			...
			localStorage.setItem("spect_remote_"+prjname+window.location.host, val);
		}
	...
	this.initSettings = function() {
		this.io = 1;
		var b = parseInt(localStorage.getItem("spect_chain_"+prjname+window.location.host));
		if (isNaN(b) || b < 0 || b > 1) {
			b = 1;
			localStorage.setItem("spect_chain_"+prjname+window.location.host, b);
		}
		this.chain = b;
		if (b == 1) {
			var fStart = factory.data.band[0].fStartDL;
		...
		if (!isNaN(fi) && !isNaN(fp) && fp >= (fi+200000)
			...
			localStorage.setItem("spect_fs_"+prjname+"_"+b+window.location.host, this.newFreqMinHz);
			localStorage.setItem("spect_fp_"+prjname+"_"+b+window.location.host, this.newFreqMaxHz);
		}
		...
		var zs = parseInt(localStorage.getItem("spect_zspan_"+prjname+window.location.host));
		if (isNaN(zs) || zs < 0 || zs > 1) {
			zs = 0;
			localStorage.setItem("spect_zspan_"+prjname+window.location.host, zs);
		}
		this.zSpanSt = zs != 0;
		var rbw = parseInt(localStorage.getItem("spect_rbw_"+prjname+window.location.host));
		if (isNaN(rbw) || rbw < 0 || rbw >= this.MAXRES) {
			rbw = this.MAXRES - 1;
			localStorage.setItem("spect_rbw_"+prjname+window.location.host, rbw);
		}
		this.resBw = rbw;
		this.spanStep = this.resBw;
		var v = parseInt(localStorage.getItem("spect_avg_"+prjname+window.location.host));
		...
		var allnotset = true;
		for (var i = 0; i < 3; ++i) {
			var io = parseInt(localStorage.getItem("spect_io_"+prjname+"_"+i+window.location.host));
			if (isNaN(io) || (io < 0 || io >1 )) {
				io = 1;
				localStorage.setItem("spect_io_"+prjname+"_"+i+window.location.host, io);
			}
		...
		if ((this.ioSet[0]==1 && this.ioSet[2]==1) || (this.chain==0 && this.ioSet[2]==1)){
			this.ioSet[2]=0;
			localStorage.setItem("spect_io_"+prjname+"_2"+window.location.host, 0);
			try{
		...
		if (allnotset){
			this.ioSet[0]=1;
			localStorage.setItem("spect_io_"+prjname+"_0"+window.location.host, 1);
			try{
				document.getElementById("ioSel_0").checked = true;
			} catch(err) {}
		}
		this.remote = localStorage.getItem("spect_remote_"+prjname+window.location.host);
	}
	var self = this;
...
function Tags() {
	...
	this.parse = function(str) {
		try {
			...
			for (var i = 0; i < arr.length; ++i) {
				...
				var tag = arr[i].substr(4,this.TAGLEN);
	...
	var tagstr = localStorage.getItem("tags_"+prjname+window.location.host);
