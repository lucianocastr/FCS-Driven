...
function h(t) {
	var an = window.parent.navi.document.anchors;
...
function isKeyContrl(c) {
	return (c == 8		// backspace
		|| c == 9	// tab
		|| c == 16	// shift
		|| c == 20	// caps lock
		|| c == 35	// end
...
function isKeyPlusSign(c) {
	return (c == 43		// plus
		|| c == 107	// numpad add
		);
}
function isKeyDotSign(c) {
	return (c == 190	// period
		|| c == 110	// numpad point
		);
}
...
function MovingIcon(el) {
	this.redraw = function() {
		...
		var self = this;
		this.tmr = setTimeout(function(ev) { self.redraw(); }, 500);
	}
...
function guiBlocked(doBlock) {
	if (doBlock) {
		...
		inputDisable(true);
		try {
			window.parent.content.autoUpdateSetClickable(false); //se modifica para quitar autoupdateremote
		} catch(err) {}
		cursorWait();
		isGuiBlockedSv = true;
		//se ignora guiblocked para cambiar de sección IP, SNMP, etc.. (poder acceder al RESET del rabbit)
		try {
			var an = window.parent.navi.document.anchors;
			for (var i = 0; i < an.length; ++i) {
				if (!an[i]) {
