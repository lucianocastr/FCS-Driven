...
function renderPage(remotesNr,expansorNr) {
	...
	for (var i = 0; i <= remotesNr; ++i) {
		...
		try{
			if (i>0) dInfo.remote[idx-1].updateUIStatus(i);
		} catch (err) { }
	}
...
function rfRedunded() {
	...
	this.readStream = function() {
		try {
			...
			var srs = localStorage.getItem("serial_"+prjname+window.location.host).split('\t');
...
function renderUnitContent(nr, parent) {
	...
	if (isMaster)
		...
		if (window.top.PathLossAnalyzer[nr]){
			cell.rowSpan = 3;
			var row1bis = document.createElement("tr");
			tab.appendChild(row1bis);
		}else
			cell.rowSpan = 2;
	}
	...
	if (isMaster) {
		...
		row1.appendChild(cell);
		cell.rowSpan = window.top.PathLossAnalyzer[nr]?3:2;
		el = createFiberTable(nr);
	}
	cell.appendChild(el);

	if (!isMaster && window.top.PathLossAnalyzer[nr]){
		cell = document.createElement("td");
		cell.className = "contentcell";		
		cell.colSpan = 4;
		row1bis.appendChild(cell);
		el = createPathLossAnalyzer(nr);
		cell.appendChild(el);
	}
	cell = document.createElement("td");
	cell.className = "contentcell";
...
function createPathLossAnalyzer(nr){
	...
	el.innerHTML = "PATH&nbsp;LOSS&nbsp;ANALYZER&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Enabled&nbsp;";
	box.appendChild(el);
	el = document.createElement("input");
	el.type = "checkbox";
	el.id = el.name = "plaEnable"+nr;
	el.onclick = function(ev) {
		var show = this.checked;
		plaCtlDisplay(nr,show);
	}
	...
	tb.appendChild(row);	
	var cell = document.createElement("td");
	cell.appendChild(createPlaUpdate(nr));
	row.appendChild(cell);
	row.appendChild(createPlaPeriod(nr));
	var cell = document.createElement("td");
	cell.style.minWidth = "5px";
	row.appendChild(cell);
	row.appendChild(createPlaMeas(nr));
	cell = document.createElement("td");
	row.appendChild(cell);	
	cell.appendChild(createPlaRemainingTime(nr));
	return pathBox;
}
function plaCtlDisplay(nr,show) {
	var el = document.getElementById("plaCtl"+nr);
	el.style.display = (show ? "table" : "none");
}

function plaEnableSet(nr,on) {
	var el = document.getElementById("plaEnable"+nr);
	try {
...
function isPlaEnableSet(nr) {
	var el = document.getElementById("plaEnable"+nr);
	try {
		return el.checked;
	} catch(e) { return false; }
}
function plaSet(nr, n, val, alrm) {
	try {
		...
		var ovl = (val == 100);
		var na = (val == 101);
		var elMet = document.getElementById("plaMet_"+n+"_"+nr).mMeter;
		var elTxt = document.getElementById("plaText_"+n+"_"+nr);
		var elLab = document.getElementById("plaLabel_"+n+"_"+nr);
		if (ovl) {
		...
		ledSetColor("plaLed_"+n+"_"+nr, color);
	} catch(e) {}
}
function plaTimeSet(nr,val) {
	try {
		var el = document.getElementById("plaRemainingTime"+nr);
		var confstrs = window.top.fiplexConfStr.split("\t");
		var t = parseInt(confstrs[nr].substr(186, 4), 16);
		var en = parseInt(confstrs[nr].substr(8, 2), 16) & 0x4;
...
function createPlaRemainingTime(nr) {
	var box = document.createElement("td");
	box.colSpan = 4;
	box.id = "plaRemainingTimeBox"+nr;
	box.style.textAlign = "center";
...
function createPlaMeas(nr) {
	var plameas = document.createElement("td");
	td.rowSpan = 2;
	var box = document.createElement("div");
	box.id = "plaMeasBox";
	...
	for (var i = 0; i < 2; ++i) {
		...
		row.appendChild(cell);
		cell = document.createElement("td");
		cell.id = "plaText_"+i+"_"+nr;
		cell.style.minWidth = "40px";
		cell.style.textAlign = "right";
		cell.className = "tabval";
		row.appendChild(cell);
		cell = document.createElement("td");
		cell.id = "plaLabel_"+i+"_"+nr;
		...
		cell.style.color = "black";
		row.appendChild(cell);
		cell = createLedBox("plaLed_"+i+"_"+nr);
		cell.style.paddingLeft = "10px";
		row.appendChild(cell);
		tb.appendChild(row);
	}
	box.appendChild(tbl);
	plameas.appendChild(box);
	return plameas;
}
function createPlaPeriod(nr) {
	var cell = document.createElement("td");
	var box = document.createElement("span");
	var label = document.createElement("span");
	label.innerHTML = "Meas.&nbsp;Interval";
	...
	el.name = el.id;
	el.size = 5;
	el.style.marginLeft = el.style.marginRight = "5px";
...
function setPlaPeriod(nr,val) {
	try {
		var el = document.getElementById("plaPeriod"+nr);
		if (!el || isNaN(val)) {
			return;
		}
		val /= 60;
...
function createPlaUpdate(nr) {
	var el = document.createElement("input");
	el.type = "button";
	el.value = "UPDATE"
	el.id = "plaUpdate"+nr;
	el.onclick = function(ev) {
		submitUpdatePlaMeas(nr);
	}
...
function pathLossSetDisable(nr, disable) {
	try {
		var el = document.getElementById("plaPeriod"+nr);
		el.disabled = disable? true : false;
		el.style.backgroundColor = disable ? "#CCCCCC" : "white";
		el = document.getElementById("plaEnable"+nr);
		el.disabled = disable? true : false;
		el.style.backgroundColor = disable ? "#CCCCCC" : "white";	
	} catch (err) {}

}
function pathLossInvalidData(nr){
	try {
		for (var i=0;i<2;i++){
			el = document.getElementById("plaMet_"+i+"_"+nr).mMeter;
			el.valueSet(-100);			
			el = document.getElementById("plaText_"+i+"_"+nr);
			el.innerHTML = "---";
			el = document.getElementById("plaLabel_"+i+"_"+nr);
			el.innerHTML = "";			
			ledSetColor("plaLed_"+i+"_"+nr, "grey");
		}		
		el = document.getElementById("plaRemainingTime"+nr);		
		el.innerHTML = "---";
	} catch (err) {}	
}
function createAutoUpdateTable() {
	...
	tb.appendChild(row);
	//Se sube la celda auto filter enable a la primera fila
	cell = document.createElement("td");
	row.appendChild(cell);
	...
	//no se utiliza autoupdateremote
	/*
	cell = document.createElement("td");
	row.appendChild(cell);
	...
	updt.onclick = function(ev) {
		autoUpdateSave(false)//se modifica para quitar autoupdateremote antes :(this.checked)
	}
...
function autoUpdateSave(val) {
	...
	localStorage.setItem("fiplexAutoUpdate_"+prjname+window.location.host, val ? true : false);
}

function autoUpdateRead() {
	var val = localStorage.getItem("fiplexAutoUpdate_"+prjname+window.location.host);
...
function createWarnRemote() {
	...
	//t.innerHTML = "Synchronizing. Wait few seconds ...."
...
function autoFiltEnSave(val) {
	...
	localStorage.setItem("fiplexAutoFiltEn_"+prjname+window.location.host, val ? true : false);
}

function autoFiltEnRead() {
	var val = localStorage.getItem("fiplexAutoFiltEn_"+prjname+window.location.host);
	var v = (val == "true" ? true : false);
	return v;
}
...
function createRfOutTable(nr) {
	...
	if (!isMaster) {
		...
		td.innerHTML = Texts['DLPOWAL'];
		td.style.textAlign = "left";
		td.colSpan = 2;
		trNode.appendChild(td);
		td = createLedBox("DLPowAlarm"+nr);
		trNode.appendChild(td);
	}
	trNode = document.createElement("tr");
	tbodyNode.appendChild(trNode);
...
function rfOutPowSet(nr, val, color) {
	try {
...
function createPLAInterval(nr) {
	var tdNode = document.createElement("td");
	var interv = document.createElement("input");
	var id = "PLAinterv"+nr;
	interv.title = "";
	interv.id = id;
	interv.name = id;
	interv.type = "text";
	interv.size = "2";
	interv.className = "number";
	interv.align = "center";
	interv.onkeypress = function(ev) {	return numbersOnly(ev);	}
	tdNode.appendChild(interv);
	return tdNode;
}
...
function createStatusTable(nr) {
	...
	if (isMaster) {
		...
		td.style.paddingRight = "10px";
		tr.appendChild(td);
	}else{ //Door open de remotos
		tr = document.createElement("tr");
		tb.appendChild(tr);
		td = document.createElement("th");
		td.innerHTML = "Door&nbsp;Open";
		td.id = "doorOpenLabel_"+nr;
		td.style.textAlign = "left";
		tr.appendChild(td);
		td = createLedBox("doorAlarmLed_"+nr);
		td.id = "doorOpenLedBox_"+nr;
		td.style.paddingLeft = "10px";
		td.style.paddingRight = "10px";	
		tr.appendChild(td);
	}
	...
	if (isMaster) {
		...
		td = document.createElement("th");
		td.innerHTML = "Door&nbsp;Open";
		td.id = "doorOpenLabel_0";
		td.style.textAlign = "left";
		tr.appendChild(td);
		td = createLedBox("doorAlarmLed_0");
		td.id = "doorOpenLedBox_0";
		td.style.paddingLeft = "10px";
		td.style.paddingRight = "10px";	
		tr.appendChild(td);
	}
...
function boardTempSet(nr, val, color) {
	try {
...
function DLOutputPowerLedSet(nr, val) {
	ledSetColor("DLPowAlarm"+nr, val);
}
...
function createHpaAlarmCurrent(tbodyNode) {
	...
	tbodyNode.appendChild(trNode);
	tdNode = document.createElement("td");
	tdNode.innerHTML = Texts['ULPAFAIL'];
	tdNode.style.textAlign = "left";
	tdNode.style.fontWeight = "bold";
	tdNode.colSpan = 2;
	trNode.appendChild(tdNode);
...
function createHpaTable(nr) {
	...
	if (window.top.NFPAMonitor[nr]!=0){
		...
		tbodyNode.appendChild(trNode);
		tdNode = document.createElement("td");
		tdNode.innerHTML = Texts['DLPAFAIL'];
		tdNode.style.textAlign = "left";
		tdNode.style.fontWeight = "bold";
		trNode.appendChild(tdNode);
...
function setDoorAlarm(n, nfpaMonitor, fipCommErr, doorCapable, doorAlarm) {
	var ledId = "doorAlarmLed_"+n;
	var labelId = "doorOpenLabel_"+n;
	var ledBoxId = "doorOpenLedBox_"+n;
	var labelBox = document.getElementById(labelId);
	var ledBox = document.getElementById(ledBoxId);
...
function newPopup(nr,ne,url) {
	...
	var fmask = localStorage.getItem("fibermask_"+prjname+window.location.host);
...
function createFiberTable(nr) {
	...
	tdNode = document.createElement("th");
	trNode.appendChild(tdNode);
	var nexp = Math.floor((idx-1)/6);
	tdNode.innerHTML = nexp==0?Texts['MASTER']:"EXPAN."+(nexp);
	...
	if (window.top.FiberMask[nr]==0x03) {
		...
		if (isFiberRed){
			...
			tdNode = document.createElement("th");
			tdNode.innerHTML = "PORT"+(((idx-1)%6)+1);
			trNode.appendChild(tdNode);		
		}
	...
	//desaparece el led freq sync en status
	/*trNode = document.createElement("tr");
	tbodyNode.appendChild(trNode);
	tdNode = document.createElement("th");
	trNode.appendChild(tdNode);
	...
	tdNode.style.textAlign = "right";
	tdNode.style.whiteSpace = "nowrap";
	tdNode = createLedBox("optRxCommLed_1_"+nr);
	trNode.appendChild(tdNode);
	if (window.top.FiberMask[nr]==0x03) {
		tdNode = createLedBox("optRxCommLed_2_"+nr);
		trNode.appendChild(tdNode);
	}		
	if (!isFiberRed){
		tdNode = createLedBox("optRxCommLed_0_"+nr);
		trNode.appendChild(tdNode);
	}else{
		numports = (window.top.redundedFiberRemotes[nr]).split("-");
		tdNode = createLedBox((nr==numports[0])?"optRxCommLed_0_"+nr:"optRxCommLed_0_"+numports[0]+"bis");
		trNode.appendChild(tdNode);
		tdNode = createLedBox((nr==numports[0])?"optRxCommLed_0_"+numports[1]+"bis":"optRxCommLed_0_"+nr);
		trNode.appendChild(tdNode);		
	}		
	trNode = document.createElement("tr");
	tbodyNode.appendChild(trNode);
...
function optRxCommLedSet(nr, nfiber, color) {
	ledSetColor("optRxCommLed_"+nfiber+"_"+nr, color);
	if (nfiber==0) ledSetColor("optRxCommLed_"+nfiber+"_"+nr+"bis", color);
}
