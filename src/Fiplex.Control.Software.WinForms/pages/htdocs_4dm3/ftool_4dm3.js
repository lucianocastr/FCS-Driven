function ftoolGui() {
	...
	this.initialize = function() {
		...
		for ( var i = 0; i <= this.nrOfRemotes; i++ ) {
			...
			if (typeof(str) !== 'undefined' && str !== null) {
				this.config[i].parse(str,this.factory[i]);
			}
		}
		...
		this.toolband = [this.factory[0].chBandEnabled[0],this.factory[0].chBandEnabled[1]];
	...
	this.makeSubmitFrm = function() {
		...
		for (var nr=0;nr<=self.nrOfRemotes;nr++){
			...
			self.frms.push({type: 'ctl_conf_str=', frame: header+self.config[nr].getFrm(self.factory[nr])});
		}
	...
	this.makeStrFrm = function() {
		for (var nr=0;nr<=self.nrOfRemotes;nr++){
			var toolConfFrame = self.config[nr].getFrm(self.factory[nr]);
