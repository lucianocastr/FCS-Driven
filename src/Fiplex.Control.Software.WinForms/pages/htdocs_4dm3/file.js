...
var tmrPostDataId;
var nrOfRemotes = 2;
var globalConfigLength = 2908;
var configLength = 1572;
var nfpaConfigLength = 396;
...
[
	...
	{get: "/global_conf.shtml", post:"nosend" , postReg: "global_req", len: globalConfigLength}
];
...
[
	...
	{post:"ctl_conf_str" , postReg: "conf_req", len: remoteHeaderLength + configLength, type: CHECKHEADER},
	{post:"nfpa_str" , postReg: "nfpa_req", len:  remoteHeaderLength + nfpaConfigLength, type: CHECKHEADER},
					...
					if (postget==0){
						...
						if (num == 1) {
							...
							for (var n = 1; n <= nrOfRemotes; n++ ) {
								...
								if (remoteFileConfResponseValid[n]) {
									if (deviceStr[n].length < globalConfigLength+remoteHeaderLength) {
							...
							for (var n = 0; n <= nrOfRemotes; n++ ) {
								...
								if (remoteFileConfResponseValid[n]){
									...
									fdata.push(header+strRepeat('0',configLength)+'\n');
									fdata.push(header+strRepeat('0',nfpaConfigLength)+'\n');
								}
								if (n==0){
						...
						if (num<(saveFileCmd.length-1)){
							...
							if (format==0)
								...
								var bdaOnly = !remoteFileConfResponseValid[1] && !remoteFileConfResponseValid[2];
								if (bdaOnly){
									var strFile = overallConfigToTextFile(settings[0].config,settings[0].NFPAcfg,settings[0].factory,settings[0].tags.getTag(),
									fdata[0],settings[0].serNr.sernr,settings[0].version,0);
									fdata = strFile.split("\n");
									...
									for (var n = 0; n <= nrOfRemotes; n++ ) {
										if (remoteFileConfResponseValid[n]){
											auxdata.push((n==0?masterName:remoteName+" "+n)+"\n");
											auxdata.push("\n");
...
function fileParseGlobalConfig(str,n) {
	...
	settings[n].NFPAcfg.parse(srNFPA);
