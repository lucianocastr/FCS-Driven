function Config() {
	...
	this.parse = function(s,factory) {
		...
		for (var b=0;b<2;b++){ //Band0/1
			...
			//gainDLFilters
			this.gainDLFilters[b] = parseInt(s.substring(ind,ind+2), 16); ind+=2;
			//OSCTIMETH
	...
	this.getFrm = function(factory){
		...
		for (b=0;b<2;b++){ //Band0/1
			...
			cfg += hexformat(mask,2);	
			//gainDLFilters
			cfg += hexformat(this.gainDLFilters[b],2);
			//OSCTIMETH
...
function NFPAconf(){
	...
	this.alarmNames				= [["HW Fail","High.Temp","Antenna Isolation","Oscillation Detection","UL PA Fail","Antenna Disconnection","Fiber Optic 1","Fiber Optic 2",
								   ...
								   ["Overload UL","Overload DL","DL PA Fail","Tx Power Low","VSWR","Rx Power Low","DL AGC Fail","","UL PA Fail","Antenna Disconnection",
								    "","","","","",""]];
	...
	this.parse = function(s) {
		...
		for (var i = 0; i < 2; i++ ) {
			...
			for (var k=0;k<8;k++){
				...
				this.bandAlarmsInstalled[k+8*i] = (i==0 && k<7); //Se fuerzan "available" las 7 primeras del primer byte
			}
		}
		for (var k=0;k<16;k++){
			res = parseInt(s.substring(ind,ind+2), 16); ind+=2;
...
function overallConfigToTextFile(config, nfpacfg, factory, tagstr, ethstr, serial, version, nr){
	...
	cfg += alarmSettingsToText(config,nfpacfg,factory, nr) + "\n";
	if (nr==0 && !factory.ethernetModuleNotInstalled) cfg += ethConfigToText(ethstr)+ "\n";
	cfg += versionToText(version) + "\n";
	cfg += "SERIAL: " + serial + "\n";
	return cfg;
}
function versionToText(version){
	var cfg = "";
	cfg += "VERSION:\n";
...
function alarmSettingsToText(config,nfpacfg,factory, nr){
	var j,k;
	var exist_el;
	var cfg = "";
	...
	var bandAlarmNames = ["Overload UL","Overload DL","DL PA Fail","Tx Power Low","VSWR","RxPower Low / Donor Antenna", "DL AGC Fail","","UL PA Fail","Antenna Disconnection"];
	...
	for (k=0;k<8;k++){
		...
		if (nr==0 && (k==2 || k==3) && DAScentric) nfpacfg.globalAlarmsEnabled[k] = false; //en master Antenna Isolation + Oscillation Detection se fuerzan a false si DAScentric
		cfg+="\tAlarm Enable " + gralAlarmNames[k] + ": " + boolToYN(nfpacfg.globalAlarmsEnabled[k]) + "\n";
	}
	for (k=0;k<10;k++){
		if (k==7) continue; //no existe esta alarma de banda en ningún caso
		if (k>7 && nr>0) continue; //remoto sólo tienen las 7 primeras alarmas de banda
		if (nr>0 && (k==1)) continue;//remotes without overload DL
		if (nr>0 && (k==5)) {
			if (!(config.remoteIsDonnorAntennaCapable && factory.masterRxLowAlarm)) continue;	//remotes  Rx Power Low
		}
...
function rfConfigToText(config, nfpacfg, factory, nr){
	...
	for (i=0;i<2;i++){
		if (factory.chBandEnabled[i] || factory.adjBandEnabled[i]){
			...
			//RF Alarm Settings
			cfg += rfConfigAlarmToText(nfpacfg, i, factory, nr);
		}
	}
...
function rfConfigGeneralBandToText(config,factory,band, nr){
	...
	var cfg = "";
	cfg+="\tGeneral\n";
	cfg+="\t\tMain Gain " + uldl[0]+ " (dB): " + config.gain[band][0] +"\n";
	if (nr==0) cfg+="\t\tGeneral Gain " + uldl[1]+ " (dB): " + config.gainDLFilters[band] +"\n";
	cfg+="\t\tDL Output Attenuation" + " (dB): " + (factory.gainlimit[2*band+1]-config.gain[band][1]) +"\n";
	for (j=0;j<2;j++) {
		if (nr>0 && j>0) continue; // do not show remote's DL power config
		cfg+="\t\tMain Power " + uldl[j]+ " (dBm): " + config.power[band][j]+"\n";
	}
...
function rfConfigNarrowBandFiltersToText(config, factory, band, nr){
	...
	var uldlcnt = 2;
	if (nr>0) uldlcnt=1;//for remotes. Only UL
	...
	if (factory.chBandEnabled[band]){
		...
		if (nr==0) cfg+="\t\tAll filters with same squelch settings DL: " + boolToYN(config.allSameSquelch[band])+"\n";
		for (j=0;j<uldlcnt;j++) cfg+="\t\tAll filters with same Bandwidth " + uldl[j] + ": "+boolToYN(config.allChSameBW[band][j])+"\n";
	}
	return cfg;
}
...
function rfConfigAlarmToText(nfpacfg, band, factory, nr){
	var cfg = "";
