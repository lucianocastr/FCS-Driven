...
function Page() {
	...
	this.createAlarmTableBand = function(nr){
		...
		for (var band=0;band<2;band++){
			...
			if (nr>0){//Overload DL and Rx Power Low do not exist in remote units
				self.NFPA[nr].bandAlarmsInstalled[1] = false;
				var miniDASmaster_supports_remote_donor_antenna_alarm = (self.version.compareSw(1,2) > 0); //both 700/800 and V/U
				self.NFPA[nr].bandAlarmsInstalled[5] = self.config[nr].remoteIsDonnorAntennaCapable && self.factory[0].masterRxLowAlarm
														&& miniDASmaster_supports_remote_donor_antenna_alarm;
			}
	...
	this.createConfigurationOptions = function(){
		...
		var row = document.createElement("tr");
		tab.appendChild(row);
		this.createConfOpt(row,2,"3. General parameters uplink: main gain, power limit");
	...
	this.createFilterTables = function(band,nbadj,nr) {
		...
		mainTbl.style.marginLeft = mainTbl.style.marginRight = "auto";
		mainTbl.id = "TableFilter_"+band+"_"+nbadj+"_"+nr;
		mainTbl.appendChild(tb);
		var row = document.createElement("tr");
		tb.appendChild(row);
		for (var b = 0; b < 3; ++b) {
			if (nr>0 && b!=1) continue;  // remotes only UL
			var cell = document.createElement("td");
			row.appendChild(cell);
		...
		tb.appendChild(row);
		for (var i = 0; i < 3; ++i) {
			if (nr>0 && i!=0) continue;	// remotes only UL
			var cell = document.createElement("td");
			row.appendChild(cell);
	...
	this.showFirstNet = function(nr,on){
		for (var b=0;b<2;b++){
			if (nr>0) {
				if (b!=0) continue;
				if (!self.showFiltersMask[nr][1][0][0]) continue;
			}
	...
	this.createFirstNetCtl = function(nr) {
		...
		el.onclick = function(ev) {
			if (this.checked) {
				...
				for (var b=0;b<2;b++) {
					if (nr>0 && b>0) continue;  // remote only UL
	...
	this.createFilterTable = function(b,band,nbadj,nr) {
		...
		var max = nbadj==0? self.maxChannels[nr] : self.config[nr].ADJNR;
		if (nr==0) {
			this.createFilterTableHeader(row, b, band, nbadj,nr,1);
			for (var r = 0; r < max; r++) {
				row = document.createElement("tr");
				row.style.height = "22px";
		...
		} else {  // remotes only UL
			this.createFilterTableHeader(row, b, band, nbadj,nr,1);
			var numFilts = this.countNumFilts(band, nbadj, nr);
			if (numFilts > 1) {
				var cell = document.createElement("td");
				cell.style.minWidth = (nbadj==0? "55px" : "20px");
				row.appendChild(cell);	
				this.createFilterTableHeader(row, b, band, nbadj,nr,2);
			}
	...
	this.createFilterTableHeader = function(chFiltRow, b, band, nbadj,nr,numCols) {
	...
	this.createFiltFrequency = function(b, c, band, nr) {
		...
		fr.onchange = function(ev) {
			...
			if (!self.isFreqSplitFixed(band,nr) || (nr>0)) return;
			if ((b % 2)==0){
	...
	this.createGfine = function(b, c, band, nbadj, nr) {
		...
		el.onchange = function(ev) {
			...
			self.config[this.nr].fineGainFilter[this.nbadj][this.band][this.path][this.chNr] = num;
		}
		cell.appendChild(el);
		return cell;
	}
	...
	this.isFreqSplitFixed = function(band,nr) {
		try {
			/** it is not shown in remotes. keep unlinked */
			if (nr>0) {
				return false;
			}
			var el = document.getElementById("freqSplit"+band+"_"+nr);
	...
	this.createBandOutBox = function(nr,dn,band) {
		...
		tab.appendChild(body);
		if (nr==0 && dn==1){
			var row = this.createGainMasterDLFilters(band);
			body.appendChild(row);
		}
		var row1 = this.createMainGainLim(dn,band,nr);
		var row2 = this.createMainPowerLim(dn,band,nr);
		if (nr==0 && dn==1){ //master changes visualization order (provisionally)
	...
	this.createEqBw = function(dn, band, nr) {
		...
		el.nr = nr;
		el.onclick = function(ev) {
			self.config[this.nr].allChSameBW[this.band][this.uldl] = this.checked;
			if (this.checked) {
	...
	this.createChSquelchEnable = function(dn,ch,band,nbadj,nr) {
		...
		el.onchange = function(ev) {
			...
			var ch = target.ch;
			var nr = target.nr;
			self.config[nr].sqChEnabled[nbadj][band][dn][ch] = this.checked;
	...
	this.createGainMasterDLFilters = function(band){
		var row = document.createElement("tr");
		var cell = document.createElement("th");
		row.appendChild(cell);
		cell.innerHTML = "General System Gain<br>(Affects fiber fed BDAs)";
		cell.className = "thdrht";
		cell = document.createElement("td");
		row.appendChild(cell);
		var el = document.createElement("input");
		el.id = "mainGainDLFilt_"+band;
		el.name = el.id;
		el.type = "text";
		el.style.width = "32px";
		el.band = band;
		...
		el.className = "number";
		var gmin = self.factory[0].gainlimit[2*band+1]+self.config[0].GmainRange[1];
		var gmax = self.factory[0].gainlimit[2*band+1];
		el.title = "Min: "+gmin+", Max: " + gmax+" dB";
		el.onchange = function(ev) {
			var target = this;
			var band = target.band;
			var num = self.gainMasterDLFiltersGet(band);
			var gmin = self.factory[0].gainlimit[2*band+1]+self.config[0].GmainRange[1];
			var gmax = self.factory[0].gainlimit[2*band+1];
			if (num < gmin) {
				num = gmin;
	...
	this.gainMasterDLFiltersSet = function(band, val) {
		try {
			var el = document.getElementById("mainGainDLFilt_"+band);
			if (!isNaN(val))
				el.value = val.toString();
		} catch (err) { }
	}
	this.gainMasterDLFiltersGet = function(band) {
		try {
			var el = document.getElementById("mainGainDLFilt_"+band);
			return parseInt(el.value);
	...
	this.createMainGainLim = function(dn, band, nr) {
		...
		if (dn==0) {
			...
			//DL att
			cell.innerHTML = "DL Output Attenuation";
		}
		cell.className = "thdrht";
		cell = document.createElement("td");
		row.appendChild(cell);
		...
		if (dn==0) {
			...
			var gmax = self.factory[nr].gainlimit[2*band+dn];
		} else {
			//DL att
			var gmin = 0;
		...
		el.onchange = function(ev) {
			...
			if (dn==0) {
				//UL gain
				var gmin = self.factory[nr].gainlimit[2*band+dn]+self.config[nr].GmainRange[dn];
				var gmax = self.mainGainMax[nr][band][dn];
			} else {
				//DL att
	...
	this.createMainPowerLim = function(dn, band, nr) {
		...
		row.appendChild(cell);
		if (nr==0 && dn==1){
			cell.innerHTML = "General Power Limit<br>(Affects fiber fed BDAs)";
		} else {
			if (self.factory[nr].ULlowGainMode) {
				cell.innerHTML = dn==0?"Maximum Power<br>per channel":Texts['POWERLIM'];
			} else {
	...
	this.remotesMainPowerLimitDLDisable = function() {
		// disable DL remote power configuration
		for (var nr=1;nr<=nrOfRemotes;nr++){
			for (var band=0;band<2;band++){
				try {
					var el = document.getElementById("mainPowerLimit_"+band+"_1_"+nr);
					el.disabled = true;
					el.style.backgroundColor = "#CCCCCC";
				}catch(e){}
			}
	...
	this.equalFreqs = function(cnf){
		for (var nr=1;nr<=nrOfRemotes;nr++){
			...
			if (remoteGlobalConfResponseValid[nr-1]){
				for (var band = 0; band < 2; ++band) {
					//if (cnf[nr].uldlLinkedFreq[band]!=cnf[0].uldlLinkedFreq[band]) return -1; //it is not considered because uldlLinkedFreq does not exist on remotes
					var b = 0; // only UL in remote
					...
					for (var c = 0; c < self.maxChannels[nr]; ++c) {
						...
						if (cnf[nr].filterEnabled[0][band][b][c]){
							if (Math.abs(cnf[nr].freqHz[band][b][c]-cnf[0].freqHz[band][b][c])>(stp/2+5)) return -5;
	...
	this.getFreqs = function(cnf,nr, actualNr) {
		for (var band = 0; band < 2; ++band) {	
			...
			for (var c = 0; c < self.maxChannels[nr]; ++c) {
				...
				for (var b = 1; b >= 0; --b) {
					//if (actualNr>0 && b>0) continue;  // remote only UL channels
					if (!on) continue;
					var fr = self.getFreqCh(b, c, band, nr);
					if (fr == null) continue;
					if (fr < self.factory[nr].fstart[2*band+b]) fr = self.factory[nr].fstart[2*band+b];
					...
					if (cnf.uldlLinkedFreq[band] && nr==0) {
						fr -= self.factory[nr].uldlFreqSplit[band];
						b--;					
						if (fr < self.factory[nr].fstart[2*band+b]) fr = self.factory[nr].fstart[2*band+b];
						if (fr > self.factory[nr].fstop[2*band+b]) fr = self.factory[nr].fstop[2*band+b];
			...
			for (var c = 0; c < self.maxChannels[nr]; ++c) {
				...
				for (var b = 0; b < 2; ++b) {
					...
					if (actualNr>0 && b==1) {  // DL remote in standby
						cnf.filterEnabled[0][band][b][c] = false;
						continue;
					}
					cnf.filterEnabled[0][band][b][c] = ((on !== null) && on);
					if (!cnf.filterEnabled[0][band][b][c]) continue;
	...
	this.equalSquelch = function(cnf){
		for (var nr=1;nr<=nrOfRemotes;nr++){
			if (remoteGlobalConfResponseValid[nr-1]){
				for (var band = 0; band < 2; ++band) {
					/** simplexMode, muteModeLinked, allSameSquelch not applicable to remotes */
					var b = 0;
					for (var nbadj = 0;nbadj<2;nbadj++){
	...
	this.getSqConf = function(cnf,nr){
		for (var band = 0; band < 2; ++band) {
			...
			for (var b = 0; b < 2; ++b) {
				if (nr>0 && b>0) continue;	// remotes only UL
				for (var nbadj = 0;nbadj<2;nbadj++){
					var cmax = nbadj==0?self.config[nr].CHNR:self.config[nr].ADJNR;
					if (nbadj==0 && b==0) cmax=1;
					for (var c=0;c<cmax;c++){
						var on = self.getShowFilter(c, band, nbadj, nr) && self.factory[nr].chBandEnabled[band];
						if (!on && (nbadj==1 || b==1)) continue;
				...
				if (simplex){
					...
					for (var nbadj = 0;nbadj<2;nbadj++){
						...
						for (var c=0;c<cmax;c++) {
							var on = self.getShowFilter(c, band, nbadj, nr) && self.factory[nr].chBandEnabled[band];
							if (!on) continue;
							cnf.sqChEnabled[nbadj][band][b][c] = true;
						}
					}
				}
			}
			...
			if (remoteGlobalConfResponseValid[nr-1]){
				for (var band = 0; band < 2; ++band) {
					for (var b = 0; b < 2; ++b) {
						...
						if (b==0 && cnf[nr].power[band][b]!=cnf[0].power[band][b]) return false;
					}
				}
			}
		}
	...
	this.getGralConf = function(cnf,nr,actualNr){
		for (var band = 0; band < 2; ++band) {
			for (var b = 0; b < 2; ++b) {
				//cnf.paEnabled[band][b] = self.config[confToGet].paEnabled[band][b];
				var gmain = self.mainGainLimGet(b, band, nr);
				if (b>0) {
					/** remote DL gmain is attenuation */
					gmain = self.factory[nr].gainlimit[2*band+b] - gmain;
				}
				...
				if (actualNr==0 && b==1){
					gmain = self.gainMasterDLFiltersGet(band);
					if (gmain > gainMax) gmain = gainMax;
					if (gmain < gainMin) gmain = gainMin;
					cnf.gainDLFilters[band] = gmain;
				}else{
					cnf.gainDLFilters[band] = 20; //gainDLFilters is gainIsolMargin = 20dB for remote units
				}
	...
	this.displayFilters = function(band,nr) {
		...
		if (nr==0) {
			...
			for (var nbadj=0; nbadj<2; nbadj++) {
				el = document.getElementById("filtersRow_"+band+"_"+nbadj+"_"+nr);
				var show = (nbadj==0? shownb : showadj);
	...
	this.showConfs = function() {
		...
		for (var nr=0;nr<=nrOfRemotes;nr++){
			...
			for (var band = 0; band < 2; ++band) {
				if (nr==0) self.gainMasterDLFiltersSet(band,self.config[0].gainDLFilters[band]);
				...
				for (var b = 0; b < 2; ++b) {
					...
					if (b==0) {
						//UL gain
						gatt = self.config[nr].gain[band][b];
					} else {
						//DL att
						gatt = self.factory[nr].gainlimit[2*band+b] - self.config[nr].gain[band][b];
					}
					self.mainGainLimSet(b, band, nr, gatt);

					self.mainPowerLimSet(b, band, nr, self.config[b==0?nr:0].power[band][b]); // b==0?nr:0 --> power limit shows only DL master settings
			...
			for (var nr=0;nr<=nrOfRemotes;nr++){
				for (var k=0;k<16;k++){
					if (nr==0){
						...
						if (k==5) {	// master Rx Lown on remote, display yellow to show the cause of relay ON
							self.bandAlarmSet(band, k, nr, false,monitor[nr].bandAlarm[band][k]);
						} else {
	...
	this.showStatusCh = function(monitor, b, c, band, nbadj, nr) {
		var dn = (nr==0?1:0);	// use UL for filterEnabled in configuration of remotes 
		var isInput = monitor.signalDet[nbadj][band][b][c] && self.config[nr].filterEnabled[nbadj][band][dn][c];
		...
		var powerOut;
		if (nr==0){
			powerOut = monitor.level[nbadj][band][b][c]+monitor.gain[nbadj][band][b][c]
		} else {
			var agc = -monitor.gain[nbadj][band][b][c];
			var gain = self.config[nr].gain[band][b] + self.config[nr].fineGainFilter[nbadj][band][b][c];
			powerOut = monitor.level[nbadj][band][b][c] + gain + agc;
		} 
		self.rfChOutPowSet(b, c, band, nbadj, nr, powerOut, chOutOn);
	...
	this.computeChOutOn = function(b, c, band, nbadj, nr, monitor) {
		...
		var dn = (nr==0?1:0);	// use UL for filterEnabled in configuration of remotes 
		if (!self.config[nr].filterEnabled[nbadj][band][dn][c]) {
			return false;
		}
	...
	this.computeAgc = function(b, c, band, nbadj, nr, monitor) {
		...
		if (nr==0) {
			/** master gives gain */
			if (b==1)
				agc = self.config[nr].gainDLFilters[band] + self.config[nr].fineGainFilter[nbadj][band][b][c]	- monitor.gain[nbadj][band][b][c];
			else
				agc = self.config[nr].gain[band][b] + self.config[nr].fineGainFilter[nbadj][band][b][c]	- monitor.gain[nbadj][band][b][c];
		} else {
			/** remote gives agc */
	...
	this.createAdjFr = function(b, c, s, band, nr) {
		...
		fr.onchange = function(ev) {
			...
			self.setAdjFreqCh(b, c, band, nr, self.computeAdjChFreq(chnr[0], b, band, nr), self.computeAdjChFreq(chnr[1], b, band, nr));
			self.config[nr].fstartHzAdjFilter[band][b][c] = self.computeAdjChFreq(chnr[0], b, band, nr);
			self.config[nr].fstopHzAdjFilter[band][b][c] = self.computeAdjChFreq(chnr[1], b, band, nr);
			if (!self.isFreqSplitFixed(band,nr) || (nr > 0)) {
				return;
			}
	...
	this.getAdjFreq = function(b, c, band, nr) {
		...
		if (error) { // avoid bad frame setting if input form is not displayed
			f[0] = self.config[nr].fstartHzAdjFilter[band][b][c];
			f[1] = self.config[nr].fstopHzAdjFilter[band][b][c];
		}
		return f;
	}
	...
	this.getAdjChConf = function(cnf, c, band, nr) {
		...
		for (var b = 0; b < 2; ++b) {
			if (nr>0 && b>0) continue;  // remote only UL channels
			cnf.filterEnabled[1][band][b][c] = ((on !== null) && on);
			if (!cnf.filterEnabled[1][band][b][c]) continue;
	...
	this.createIsolGain = function(nr,row) {
		...
		var dual = b[0] && b[1];
		if (dual){
			cell.innerHTML = "Max.&nbsp;Allowable&nbsp;UL&nbsp;Gain("+self.factory[nr].bandNames[0]+"/"+self.factory[nr].bandNames[1]+")";
		}else{
			cell.innerHTML = "Max.&nbsp;Allowable&nbsp;UL&nbsp;Gain";
		}
		cell.className = "thdrht";
		cell.style.height = "20px";
		row.appendChild(cell);
	...
	this.createIsolAtt= function(nr,row) {
		var cell = document.createElement("th");
		var b = [false,false];
		for (var k=0;k<2;k++)
			b[k] = self.factory[nr].adjBandEnabled[k]||self.factory[nr].chBandEnabled[k];
		var dual = b[0] && b[1];
		if (dual){
			cell.innerHTML = "Min.&nbsp;Allowable&nbsp;DL&nbsp;Att.("+self.factory[nr].bandNames[0]+"/"+self.factory[nr].bandNames[1]+")";
		}else{
			cell.innerHTML = "Min.&nbsp;Allowable&nbsp;DL&nbsp;Att.";
		}
		cell.className = "thdrht";
		cell.style.height = "20px";
		row.appendChild(cell);
		cell = createTextCell("isolAtt_"+nr,1);
		row.appendChild(cell);
		cell = document.createElement("td");
		cell.innerHTML = "dB";
		cell.style.fontWeight = "normal";
	...
	this.setIsolGain = function(nr,g) {
		try {
			...
			var gainAllow = [0,0];
			var attAllow = [0,0];
			var maxGLimits = [0,0];	// for each band, get maximum Gain Limit of UL/DL
			var uldlMax = [0,0];	// which one is maximum, UL or DL
			// maxAllow gain of UL for each band, g[k] is referred to maximum which in case of UL Low gain happens to be DL
			var gUL = [0,0];
			var gDL = [0,0];
			for (var k=0;k<2;k++){
				for (var j=0;j<2;j++){
				...
				gDL[k] = g[k];
				if (uldlMax[k]==1){
					gUL[k] -= (self.factory[nr].gainlimit[2*k+1]-self.factory[nr].gainlimit[2*k]);
				} else {
					gDL[k] -= (self.factory[nr].gainlimit[2*k]-self.factory[nr].gainlimit[2*k+1])
				}
			}
			for (var k=0;k<2;k++){
				var gmin = self.factory[nr].gainlimit[2*k]+self.config[nr].GmainRange[0];
				isol[k] = ((gDL[k]>=self.factory[nr].gainlimit[2*k+1])?">="+(gDL[k]+m[k]):gDL[k]+m[k])+".0";
				gainAllow[k] = (g[k]<gmin?"<"+gmin:(g[k]>self.factory[nr].gainlimit[2*k]?self.factory[nr].gainlimit[2*k]:g[k]))+".0";
				// DL minimum attenuation
				var gminDL = self.factory[nr].gainlimit[2*k+1]+self.config[nr].GmainRange[0];
				var attenuationMin = self.factory[nr].gainlimit[2*k+1]-gDL[k];
				if (attenuationMin<0) attenuationMin = 0;
				attAllow[k] = (g[k]<gminDL?">"+(-self.config[nr].GmainRange[0]):attenuationMin)+".0";
			}
			var b = [false,false];
			for (k=0;k<2;k++)
				b[k] = self.factory[nr].adjBandEnabled[k]||self.factory[nr].chBandEnabled[k];
			...
			if (dual){
				...
				setTextCell("isolAtt_"+nr,attAllow[0]+ " / " + attAllow[1]);
			}else{
				var ind = b[0]?0:1;
				setTextCell("isol_"+nr,isol[ind]);
				setTextCell("isolGain_"+nr,gainAllow[ind]);
				setTextCell("isolAtt_"+nr,attAllow[ind]);					
			}
		} catch(err) {}
	}
		...
		for (var nr=1;nr<=nrOfRemotes;nr++){
			for (var band=0;band<2;band++){
				for (var dn=0;dn<2;dn++){
					...
					disable_element("mainPowerLimit_"+band+"_"+dn+"_"+nr,(nr>0 && dn==1)?true:val); //disable DL power limit for remotes
				}
			}
		}
	} catch (err) { }
