...
function parseGlobalConfig(str) {
	...
	var globalConfigMasterLength = 2876;
	var globalConfigRemoteLength = 2876;
	...
	if (deviceStr[0].length < globalConfigMasterLength) {
	...
	for (var n = 0; n < nrOfRemotes; n++ ) {
		...
		if (remoteGlobalConfResponseValid[n]) {
			if (deviceStr[remoteNr].length < globalConfigRemoteLength+remoteHeaderLength) {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState==4) {
				if (this.status === 200) {
					...
					// factory.parse(deviceStr[0]);
					factory.parse(serverResponse);
					gui.show(factory);
					window.parent.navi.hideIPconf(factory.ethernetModuleNotInstalled);
					version.render(factory.ethernetModuleNotInstalled);
					factStr = serverResponse;
					localStorage.setItem("Factory_0_"+Prjstr+window.location.host, correctASCIIOnlyText(serverResponse));
					initFormChangeCheck();
					window.parent.navi.filter_help_args();
					window.parent.navi.filterTool();
					guiBlocked(false);
...
function GuiFact() {
	...
	this.rdAttInMinOff = function(fact, b) {
		var el = document.getElementById('attInMin'+'_'+b);
		var value = parseInt(el.value);
		if (value < 0) {
			value = 0;
	...
	this.Map = [
		...
		[
			...
			},
			{
				...
				inputtype: 'text',
				onkey: isKeyDecimalNumber,
				show: this.shAttInMinOff,
				read: this.rdAttInMinOff,
				len: 2
			}
		],
		[	
			...
			},
			{
				...
				inputtype: 'text',
				onkey: isKeyDecimalNumber,
				show: this.shAttInMinOn,
				read: this.rdAttInMinOn,
				len: 2
			}
		],
		[				
