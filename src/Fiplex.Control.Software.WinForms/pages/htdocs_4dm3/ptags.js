...
function parseGlobalConfigUnit(str, n) {
	...
	if (n==0){
		config.parse(srConf,new Factory());
	}
...
function parseGlobalTagResponse (str) {
	var rawStr = "";
	for (var i = 0; i < str.length; i += 2) {
		var hexnum = str.substr(i, 2);
		var num = parseInt(hexnum, 16);
		if (isNaN(num)) {
			rawStr += ' ';
		} else {
			rawStr += String.fromCharCode(num);
		}
	}

	var deviceStr = rawStr.split(frameSeparator);
	if (deviceStr.length != 1+nrOfRemotes) {
		alert("Error retrieving info. Tag error subframes nr="+deviceStr.length);	// debug
		alert(rawStr);
	}
	deviceStr[0] = correctASCII(deviceStr[0]);
	if (deviceStr[0].length < tagLength) {
		alert("Error retrieving info. Tag master length="+deviceStr[0].length);	// debug
	}
	for (var n = 0; n < nrOfRemotes; n++ ) {
		var remoteNr = n+1;
		remoteConfResponseValid[n] = (1 === parseInt(deviceStr[remoteNr].substring(2,4),16));
		if (remoteConfResponseValid[n]) {
			deviceStr[remoteNr] = correctASCII(deviceStr[remoteNr]);
			if (deviceStr[remoteNr].length < tagLength+remoteHeaderLength) {
				//alert("Error retrieving info. Global conf remote length="+deviceStr[remoteNr].length);	// debug
				remoteConfResponseValid[n] = false;
			}
		}
	...
	// master
	tags[0].parseRawText(deviceStr[0]);
	// remotes
	for (var n = 0; n < nrOfRemotes; n++) {
		var remoteNr = n+1;
		if (!remoteConfResponseValid[n]) {
		...
		tags[remoteNr].parseRawText(deviceStr[remoteNr].substr(remoteHeaderLength));
	}
}
...
function createForm() {
	...
	for ( var n = 0; n <= nrOfRemotes; n++ ) {
		...
		if (bdaOnly)
			...
			if (n==0)
				header.innerHTML = masterName;
			else
				header.innerHTML = remoteName + " " + n;
		}
		...
		if (bdaOnly || (n>0 && !remoteConfResponseValid[n-1])) cell.style.display = "none";
		// remote version
		cell = document.createElement("td");
		cell.style.marginRight = "15px";
		row.appendChild(cell);
