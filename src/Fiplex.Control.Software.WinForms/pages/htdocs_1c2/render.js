...
function createBandOutBox(dn) {
	...
	var cell = document.createElement("th");
	row.appendChild(cell);
	cell.innerHTML = (factory.data.band[dn].hpamon == 2)&&(dn==1)?Texts['COMERR']:Texts['PASTATUS'];
	cell.className = "thdrht";
	cell = createLedBox("hpaStatusLed"+dn);
	row.appendChild(cell);
	
	if (dn == 1 && factory.data.band[dn].hpamon == 2) {
		cell = document.createElement("th");
		row.appendChild(cell);
		cell.innerHTML = Texts['VSWR'];
		cell.className = "thdrht";
		cell = createLedBox("hpaVswr"+dn);
		row.appendChild(cell);	
	}
	if (dn == 1) {
		row = document.createElement("tr");
	...
	if (dn == 1 && factory.data.band[dn].hpamon == 2) {
		...
		cell.className = "thdrht";
		cell = createLedBox("hpaTxPwrLow"+dn);
		row.appendChild(cell);
	}
	return box;
}
