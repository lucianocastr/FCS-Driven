...
function NFPAconf(){
	...
	this.parse = function(s) {
		...
		for (var k=0;k<16;k++){
			...
			if (k >=this.relayAssignExternalAlarmStartIndex && k <= this.relayAssignExternalAlarmStartIndex+4) {
				var externalAlarmNr = k - this.relayAssignExternalAlarmStartIndex;
		...
		for (var k=0;k<3;k++){ 
			var index = this.externalAlarmStartIndex;
			this.alarmNames[index+k] = s.substring(ind,ind+30).trim();ind+=30;
			// alert("index="+(index+k)+", name="+this.alarmNames[index+k]);
			// alert("index="+(index+k));
		}
	}
	this.getFrm = function(){
		...
		for (var k=0;k<16;k++){
			...
			if (k >=this.externalAlarmStartIndex && k <= this.externalAlarmStartIndex+4) {
				var externalAlarmNr = k - this.externalAlarmStartIndex;
		...
		this.alarmNames[this.externalAlarmStartIndex+3] = "Force RF OFF";
		for (var k=0;k<4;k++){
			var aux = this.alarmNames[this.externalAlarmStartIndex+k].substring(0,30);
