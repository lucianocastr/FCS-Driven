...
function Page() {
	...
	this.createAlarmTableBand = function(){
		...
		for (var band=0;band<2;band++){
			...
			for (var k=0;k<self.NFPA.NUMALARM;k++){
				...
				tb.appendChild(this.createAlarmLed(k,alarmName,show && self.NFPA.alarmInstalled[n]));
			}
		}	
		return cellb;
	}
	this.createAlarmTable = function(){
		...
		for (var k=0;k<self.NFPA.NUMALARM;k++){
			...
			tb.appendChild(this.createAlarmLed(k,self.NFPA.alarmNames[n],self.NFPA.alarmInstalled[n]));
		}
		return cellb;
	}
