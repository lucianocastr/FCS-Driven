		...
		xhr.onreadystatechange = function(ev) {
			if (this.readyState==4) {
				if (this.status === 200) {
					...
					localStorage.setItem("tags_"+prjname+"_"+window.location.host, serverResponse);
				}
				confReq();
			}
		}
	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState==4) {
				if (this.status === 200) {
					var serverResponse = this.responseText;
					localStorage.setItem("cfg_"+prjname+"_"+window.location.host, serverResponse);
					window.parent.navi.configFrame = serverResponse;
	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState==4) {
				if (this.status === 200) {
					var serverResponse = this.responseText;
					localStorage.setItem("stat_"+prjname+"_"+window.location.host, serverResponse);
...
function submitClearCounters() {
	try {
		...
		confStr = formatClearCounters(confStr);
		document.getElementById("ctl_conf_str").value = confStr;
		clearTimeout(tmrIdStat);
		waitingACK = true;
		showResultIcon(ERR_PENDING);
...
function submitUpdatePlaMeas() {
	try {
		...
		confStr = formatUpdatePlaMeas(confStr);
		document.getElementById("ctl_conf_str").value = confStr;
		clearTimeout(tmrIdStat);
		waitingACK = true;
		showResultIcon(ERR_PENDING);
