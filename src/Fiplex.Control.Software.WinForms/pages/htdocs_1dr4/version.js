...
function Version() {
	...
	this.parse = function(sr) {
		try {
			...
			this.fwMsb = s[i++];
			var swmain = s[i++];
			this.swMain = swmain & 0x0F;
			this.hasEthMod = !!(swmain & this.confOptionBits.VOP_ETH);
			this.swSub = s[i++];
			this.eth = [];
			if (this.hasEth) {
				var allzero = true;
	...
	this.render = function() {
		try {
			...
			var row = window.parent.navi.document.getElementById('spectRow');
			if (typeof(lnk) != 'undefined' && lnk) {
				row.style.display = "table-row";
				lnk.style.display = "inline";
			}
			var lnk = window.parent.navi.document.getElementById('thresholds');
			var row = window.parent.navi.document.getElementById('thrRow');
			if (typeof(lnk) != 'undefined' && lnk) {
				row.style.display = "table-row";
				lnk.style.display = "inline";
			}
			el = window.parent.navi.document.getElementById("ip");
			var row = window.parent.navi.document.getElementById("ipRow");
			if (el) {
				row.style.display = this.hasEthMod ? "table-row" : "none";
				el.style.display = this.hasEthMod ? "inline" : "none";
				el.className = "n";
			}
			el = window.parent.navi.document.getElementById("time");
			var row = window.parent.navi.document.getElementById("timeRow");
			if (el) {
				row.style.display = this.hasEthMod ? "table-row" : "none";
				el.style.display = this.hasEthMod ? "inline" : "none";
				el.className = "n";
	...
	this.compareSwMajor = function(sr) {
		sr = ("00"+sr).substr(-2);
		var n = parseInt(sr, 16);
		if (isNaN(n))
			return -1;
		if (this.swMain < n) {
			return -1;
	...
	this.compareSwMinor = function(sr) {
		sr = ("00"+sr).substr(-2);
		var n = parseInt(sr, 16);
		if (isNaN(n))
			return -1;
	...
	this.swGTnpr = function() {
		if (this.compareSw(this.swVerNPR1) > 0) {
			return true;
		}
		if (this.compareSwMajor(this.swVerNPR2.substr(2)) == 0) {
			if (this.compareSwMinor(this.swVerNPR2.substr(-2)) > 0) {
				return true;
			}
		}
		return false;
