...
function buildGUI(arg){
	...
	$('#stpYes').hide();
	$('#stpNo').hide();
	$('#setupband1').html('Would you like to use Channel or Adj.BW filters?');
	$('#btnA').val('Channel');
	$('#btnB').val('Adj.BW');
	$('#warnpowertr').hide();
	$('#classnotlic').hide();
...
$("#smartwizardc").on("leaveStep", function(e, anchorObject, stepNumber, stepDirection) {
	if ((stepNumber<=2) && stepDirection.search("forward")>=0){
		...
		cnfToSend = new ConfigBDA();
		cnfToSend.parse(config.frm);
		//read control channel both bands
		pageAst.readControlChannel(cnfToSend);
		$('.toolbar-bottom').hide();
		return true;
	}
});
...
function filtexit(resultstep){
	...
	if (resultstep=='B'){
		...
		if (firstnet){
			...
			if (numEnabledFilts(0,0,config,factory)==0 && numEnabledFilts(0,1,cnfToSend,factory)==0){ //no channel filters enabled in 700 and no adj filters enabled (once band14 was disabled)	
			...
			for (var fb=0;fb<2;fb++){ //both filter band arrays are considered due to flexible filters
				for (var na=0;na<2;na++){ //both channel and adj filters
					var N = na==0?config.CHNR:config.ADJNR;
					for (var i=0;i<N;i++){
						if (config.filterBand[na][fb][1][i]!=bandGUI) continue; //skip filters of other band
						if (na==1 && fb==0 && i==0 && config.firstADJisFirstNet) continue; //skip if first ADJ is configured as band14
						cnfToSend.filterEnabled[na][fb][0][i] = false;
						cnfToSend.filterEnabled[na][fb][1][i] = false;
					}
				}
			}
			if (bandGUI==1){
				...
				if (!(config.firstADJisFirstNet && config.filterEnabled[1][0][1][0])){
					cnfToSend.paEnabled[0][0] = false;
					cnfToSend.paEnabled[0][1] = false;
					cnfToSend.forcePaOff[0][0] = true;
					cnfToSend.forcePaOff[0][1] = true;
...
function filtconf(resultstep)
{
	if (firstnet){
		...
		if (ret.length==0){
			...
			c.append($(pageAst.createRSSIFilterTables(bandGUI,nbadjGUI,1))); //implicit tabledID for RSSI tables is 1
			pageAst.refreshFreqs(bandGUI,nbadjGUI,cnfToSend,1);
		...
		if (resultstep=='A'){
			...
			if (!factory.chBandEnabled[bandGUI]){
				$('#confirmClass').hide();
				$('#licmess').html('A license upgrade is required to unlock Channel filter for '+(bandGUI==0?'700':'800')+' Band');
				$('#nbinfo').hide();
				$('#classnotlic').show("swing");
				return;
			}
	...
	pageAst.refreshSquelch(bandGUI,nbadjGUI,firstnet);
...
function redrawSmartWizardC(){
	var nA,nB;
	nA = numEnabledFilts(0,0,config,factory);
	nB = numEnabledFilts(0,1,config,factory);
	if((!factory.chBandEnabled[0] && !factory.adjBandEnabled[0]) || ((nA==0)&&(nB==0)))
		$('#smartwizardc').smartWizard("stepState", [1], "disable");
	else
		$('#smartwizardc').smartWizard("stepState", [1], "enable");
	nA = numEnabledFilts(1,0,config,factory);
	nB = numEnabledFilts(1,1,config,factory);
	if((!factory.chBandEnabled[1] && !factory.adjBandEnabled[1]) || ((nA==0)&&(nB==0)))
		$('#smartwizardc').smartWizard("stepState", [2], "disable");	
	else
		$('#smartwizardc').smartWizard("stepState", [2], "enable");
}
function redrawSmartWizard(na,fn,bandGUI){
	//na: 0-user chose channel band configuration, 1- user chose adj.bw
	...
	if (fn){ //band14
		...
		if (na==1){
			if (bandGUI==0 && config.firstADJisFirstNet && config.filterEnabled[1][0][1][0] && numEnabledFilts(0,1,config,factory)==1) $('#band14warn').show();
			$('#navs2').hide();
...
function runcomm(){
 ...
 function active_step2(idbw)
 {
	...
	r = $('<tr/>');t.append(r);
	c = $('<td/>',{class:"contentcell"}).css("minHeight","50px");r.append(c);
	c.append($(pageAst.createFilterTables(bandGUI,nbadjGUI,num,true,0))); //added tableID = 0 to distinguish from other tables
	pageAst.refreshFreqs(bandGUI,nbadjGUI,config,0);
	t = $('#step-4t');
	t.html('');
	r = $('<tr/>');t.append(r);
	c = $('<td/>',{class:"contentcell"}).css("minHeight","50px");r.append(c);
	c.append($(pageAst.createRSSIFilterTables(bandGUI,nbadjGUI,num))); //implicit tabledID for RSSI tables is 1
	pageAst.refreshFreqs(bandGUI,nbadjGUI,config,1);
...
function active_comm_step12(band){
	...
	var n;
	for (k=0;k<2;k++){
		n = numEnabledFilts(band,k,config,factory);
		if (n>0){
			t = $('#step-'+(band+2)+'ct'+k);
			t.html('');
			r = $('<tr/>');t.append(r);
			c = $('<td/>',{class:"contentcell"}).css("minHeight","50px");r.append(c);
			c.append($(pageAst.createFilterTables(band,k,n,false,2))); //added tableID = 2 to distinguish from other tables
			pageAst.refreshFreqs(band,k,config,2);
		}else{
			//remove table if exists
			if ($("#TableFilter_"+band+"_"+k+"_2").length > 0) {
    			$("#TableFilter_"+band+"_"+k+"_2").remove();
			}
		}
	}
	$('.toolbar-bottom').show();
...
$("#smartwizardc").on("stepContent",function(e, anchorObject, stepIndex, stepDirection) {
	...
	stepc = stepIndex;
	if (stepIndex == 1 || stepIndex == 2){ //lists built in any of those steps to consider that one of those bands are not licensed
		active_comm_step12(0);
		active_comm_step12(1);//se deja lista la tabla del siguiente step
	}
	if (stepIndex == 2 || stepIndex == 3){ //when steps is 2 (700) or 3(800), config is sent if control channel changed
	...
	clearTimeout(tmrIdStat);
		...
		if (stepIndex == 5)
		{
			...
			if (bandGUI==0 && pageAst.checkFirstNetAndFilters()){
				...
				if (firstnet){
					$('#warnpower').html("There are enabled channel filters. Take into account that output power " +
					"will be shared between Band 14 Filter and these channel filters (one half for Band 14 Filter and the other half for channel filters)," +
