 function Factory(factoryFrame) {
	...
	this.extPaCorrection		= [0,0]; //external PA correction factor in 0.1dB steps Band0/1
	...
	this.parse = function(s) {
		...
		for (b=0;b<2;b++){
			...
			if (b==1) {
				this.MMSmode = (temp & 0x02)!=0;
				this.FASrelayTestMode = (temp & 0x04)!=0;
				this.clearDeepDischargeCounter = false;
			}
			temp= parseInt(s.substring(ind,ind+2), 16); ind+=2;	// used only in B0 for BDA, reserved for PLA in Remote
		...
		for (var b=0;b<2;b++){
			this.extPaCorrection[b] = parseInt(s.substring(ind,ind+2),16); ind+=2;
			if (this.extPaCorrection[b] > 127) this.extPaCorrection[b]-=256;
			this.extPaCorrection[b] /= 10.0;
		}
	...
	this.getFrm = function(){
		...
		cal += hexformat(this.unusedCoefPaCurrentDetector,4);
		cal += this.calBoardCurrentDetector.getFrm();
		for (var b=0;b<2;b++){
			res = 10.0 * this.extPaCorrection[b];
			if (res <0) res+=256;
			cal += hexformat(res,2);
		}
	...
	this.isClassB = function(){
		return ((this.adjBandEnabled[0] || this.adjBandEnabled[1]) && !this.chBandEnabled[0] && !this.chBandEnabled[1]);
	}
	this.isMMSmode = function() {
		return this.MMSmode;
	}
