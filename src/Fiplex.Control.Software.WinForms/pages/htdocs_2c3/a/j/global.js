...
function PageAssisted() {
	...
	this.createRSSIFilterTables = function(band,na,num) {
		var t,y,r,c,str,k,j;
		t = $('<table/>',{class:"bt",id:"TableFilter_"+band+"_"+na+"_1"}).css("padding","1px");
		y = $('<tbody/>');t.append(y);
		r = $('<tr/>');y.append(r);
		//header
		if (na==1){
		...
		for (k=0;k<num;k++){
			...
			r = $('<tr/>');y.append(r);
			//Num Filter
			c = $('<td/>',{id:"nrFilt_"+na+"_"+band+"_"+k+"_1"}).css({"textAlign":"center","padding":"2px"});r.append(c);
			//CCH
			r.append(this.createCCHEnable(band,na,k,false,1));
			//Uplink freq
			if (na==0){
				r.append(this.createFiltFrequency(0,band,k,false,1));
				c = $('<td/>').html("MHz").css("textAlign","left");r.append(c);
			}else{
				r.append(this.createAdjFiltFrequency(0,band,k,0,false,1));
				c = $('<td/>').html("MHz").css("textAlign","left");r.append(c);
				r.append(this.createAdjFiltFrequency(0,band,k,1,false,1));
				c = $('<td/>').html("MHz").css("textAlign","left");r.append(c);
			}
			//RSSI
			...
			//Downlink freq
			if (na==0){
				r.append(this.createFiltFrequency(1,band,k,false,1));
				c = $('<td/>').html("MHz").css("textAlign","left");r.append(c);
			}else{
				r.append(this.createAdjFiltFrequency(1,band,k,0,false,1));
				c = $('<td/>').html("MHz").css("textAlign","left");r.append(c);
				r.append(this.createAdjFiltFrequency(1,band,k,1,false,1));
				c = $('<td/>').html("MHz").css("textAlign","left");r.append(c);
			}
			//RSSI
	...
	this.createFilterTables = function(band,na,num,disableFreqs,tableId) {
		var t,y,r,c,str,k,j;
		t = $('<table/>',{class:"bt",id:"TableFilter_"+band+"_"+na+"_"+tableId}).css("padding","1px");
		y = $('<tbody/>');t.append(y);
		r = $('<tr/>');y.append(r);
		//header
		if (na==1){
		...
		for (k=0;k<num;k++){
			r = $('<tr/>');y.append(r);
			//Num Filter
			c = $('<td/>',{id:"nrFilt_"+na+"_"+band+"_"+k+"_"+tableId}).css({"textAlign":"center","padding":"2px"});r.append(c);
			//CCH
			r.append(this.createCCHEnable(band,na,k,true,tableId));
			//Downlink freq
			if (na==0){
				r.append(this.createFiltFrequency(1,band,k,disableFreqs,tableId));
				c = $('<td/>').html("MHz").css("textAlign","left");r.append(c);
			}else{
				r.append(this.createAdjFiltFrequency(1,band,k,0,disableFreqs,tableId));
				c = $('<td/>').html("MHz").css({"textAlign":"left","paddingRight":"10px"});r.append(c);
				r.append(this.createAdjFiltFrequency(1,band,k,1,disableFreqs,tableId));
				c = $('<td/>').html("MHz").css({"textAlign":"left","paddingRight":"10px"});r.append(c);
			}
		}
		return t;
	}
	this.createCCHEnable = function(band,na,c,en,tableId) { //added tableID to avoid duplicate IDs
		var d = $('<td/>').css({"paddingLeft":"20px","paddingRight":"20px"});
		var idc = "cchEn_"+na+"_"+band+"_"+c+"_"+tableId;
		var isFn = false;
		...
		e.prop({"name":e.prop("id"),"band":band,"ch":c,"na":na,"tableId":tableId});
		if (isFn){
		...
		e.change(function(ev) {
			...
			var c = $(this).prop("ch");
			var na = $(this).prop("na");
			var tableId = $(this).prop("tableId");
			for(var k=0;k<2*config.CHNR;k++){
				try{
					if (k!=c) $('#cchEn_'+na+'_'+b+'_'+k+'_'+tableId).prop('checked',false);
				} catch (err) {}
			}
			//los filtros de la otra clase todos no chequeados
			for(var k=0;k<2*config.CHNR;k++){
				try{
					$('#cchEn_'+(1-na)+'_'+b+'_'+k+'_'+tableId).prop('checked',false);
				} catch (err) {}
			}
		});
		return d;
	}
	this.createAdjFiltFrequency = function(b, band, c, ss, en, tableId) { //added tableID to avoid duplicate IDs
		var e = $('<td/>');
		var f = $('<input/>',{id:"chAdjFreq_"+c+"_"+b+"_"+band+"_"+ss+"_"+tableId,class:"number",type:"text"}).prop({"disabled":!en,"size":"10"});e.append(f);
		f.prop({"ch":c,"dn":b,"band":band,"ss":ss,"tableId":tableId});
		...
		f.change(function(ev) {
			...
			if (ss==0){
				var foth = ~~Math.round($("#chAdjFreq_"+c+"_"+dn+"_"+b+"_1_"+tableId).val()*1e6);
				if (fr > (foth-1e5)) fr = (foth-1e5);
			}else{
				var foth = ~~Math.round($("#chAdjFreq_"+c+"_"+dn+"_"+b+"_0_"+tableId).val()*1e6);
				if (fr < (foth+1e5)) fr = (foth+1e5);
			}
	...
	this.createFiltFrequency = function(b, band, c, en, tableId) { //added tableID to avoid duplicate IDs
		var e = $('<td/>');
		var f = $('<input/>',{id:"chFreq_"+c+"_"+b+"_"+band+"_"+tableId,class:"number",type:"text"}).prop({"disabled":!en,"size":"10"});e.append(f);
	...
	this.clearFreqs = function(band,na){
		var N = 2*(na==0?config.CHNR:factory.numADJFilters);
		for (var c=0;c<N;c++){
			$('#cchEn_'+na+'_'+band+'_'+c+'_1').prop('checked',false);
			for (var i=0;i<2;i++){
				if (na==0){
					$("#chFreq_"+c+"_"+i+"_"+band+"_1").val('');
				}else{
					$("#chAdjFreq_"+c+"_"+i+"_"+band+"_0_1").val('');
					$("#chAdjFreq_"+c+"_"+i+"_"+band+"_1_1").val('');
				}
			}
		}
	}
	this.refreshFreqs = function(band,na,cnf,tableId){
		//tableID is 0 for only DL tables on STEP2/STEP3/STEP4 menus (filter configuration)
		//tableID is 1 for UL/DL RSSI tables on STEP2/STEP3/STEP4 menus (filter monitoring)
		//tableID is 2 for only DL tables on STEP7 COMISSIONING
		var fc,f1,f2;
		var nfilt = $('#idcuantoch').val();
		if (tableId==2) nfilt = numEnabledFilts(band,na,cnf,factory);
		for (var fb=0,nch=0;fb<2;fb++){ //it is necessary to consider both band arrays
			var N = na==0?cnf.CHNR:factory.numADJFilters;
			for (var c=0;c<N;c++){
				if (!cnf.filterEnabled[na][fb][1][c] || cnf.filterBand[na][fb][1][c]!=band) continue; //skip disabled and filters of other band
				$("#nrFilt_"+na+"_"+band+"_"+nch+"_"+tableId).html(fb*N+c+1); //update table index
				for (var i=1;i>=(tableId==1?0:1);i--){ //if tableId = 1, UL/DL, only DL otherwise
					if (na==0){
						if (i==1 && (cnf.CHNR*fb+c+1)==cnf.controlChannel[band]){ //CCH is considered only in DL
							$('#cchEn_'+na+'_'+band+'_'+nch+'_'+tableId).prop('checked',true);
						}
						fc = cnf.freqHz[fb][i][c];
						if (fc<factory.fstart[2*band+i]) fc=factory.fstart[2*band+i];
						if (fc>factory.fstop[2*band+i]) fc=factory.fstop[2*band+i];
						$("#chFreq_"+nch+"_"+i+"_"+band+"_"+tableId).val((fc/1e6).toFixed(6));
					}else{
						if (i==1 && (cnf.ADJNR*fb+c+1)==-cnf.controlChannel[band]){ //CCH is considered only in DL
							$('#cchEn_'+na+'_'+band+'_'+nch+'_'+tableId).prop('checked',true);
						}
						f1 = cnf.fstartHzAdjFilter[fb][i][c];
						f2 = cnf.fstopHzAdjFilter[fb][i][c];
						if (f1<factory.fstart[2*band+i] || f1>factory.fstop[2*band+i]){
							f1=factory.fstart[2*band+i];
							f2=factory.fstop[2*band+i];
						}
						$("#chAdjFreq_"+nch+"_"+i+"_"+band+"_0_"+tableId).val((f1/1e6).toFixed(3));
						$("#chAdjFreq_"+nch+"_"+i+"_"+band+"_1_"+tableId).val((f2/1e6).toFixed(3));
					}
				}
				nch++;
				if (nch>=nfilt) return; //all channels filled
			}
		}
		//rest of the channels filled with center freq of adj full band
		for (var i=0;i<2;i++){
			fc = (factory.fstart[2*band+i]+factory.fstop[2*band+i])/2;
			f1 = factory.fstart[2*band+i];
			f2 = factory.fstop[2*band+i];
			for (var c=nch;c<nfilt;c++){
				if (na==0){
					$("#chFreq_"+c+"_"+i+"_"+band+"_"+tableId).val((fc/1e6).toFixed(6));
					console.log("Setting freq ch "+c+" band "+band+" dir "+i+" to "+(fc/1e6).toFixed(6)+" MHz");
				}else{
					$("#chAdjFreq_"+c+"_"+i+"_"+band+"_0_"+tableId).val((f1/1e6).toFixed(3));
					$("#chAdjFreq_"+c+"_"+i+"_"+band+"_1_"+tableId).val((f2/1e6).toFixed(3));
				}	
			}
		}
	}
	this.refreshSquelch = function(band,na,isFn){
		var min = config.sqThrLimits(1,false).MIN;
		var max = config.sqThrLimits(1,false).MAX;
		var e = $('#dlsquelchset');
		//search for sq configured value
		var sq = -80;
		if (typeof(isFn) !== 'undefined' && isFn){
			sq = config.sqChThreshold[1][0][1][0]; //squelch first adj dl 700
		}else{
			var N = na==0?config.CHNR:factory.numADJFilters;
			var found = false;
			for (var fb=0;fb<2;fb++){ //both filter band arrays are considered due to flexible filters
				for (var k=0;k<N;k++){
					if (found) continue; //take the first enabled filter found
					if (config.filterBand[na][fb][1][k]!=band) continue; //skip disabled and filters of other band
					if (na==1 && fb==0 && k==0 && config.firstADJisFirstNet) continue;//skip band14 filter in adj class
					sq = config.sqChThreshold[na][fb][1][k];
					found = true;
				}
			}
		}
		e.val(sq).prop("title","Min: "+min+", Max: "+max+" dBm");
	...
	this.readConf = function(cnf,band,na,isFn){
		...
		//exception to read conf for band14
		if (typeof(isFn) !== 'undefined' && isFn){
			cnf.sqChThreshold[1][0][1][0] = ~~$('#dlsquelchset').val();
			return;
		}
		var N = na==0?config.CHNR:factory.numADJFilters;
		for(var fb=0;fb<2;fb++){ //both filter band arrays are considered due to flexible filters
			for (var k=0;k<N;k++){
				if (cnf.filterBand[na][fb][1][k]!=band) continue; //skip disabled and filters of other band
				if (na==1 && fb==0 && k==0 && config.firstADJisFirstNet) continue;//skip first net filter in adj class
				cnf.sqChThreshold[na][fb][1][k] = ~~$('#dlsquelchset').val(); //SQ DL configurado por usuario
			}
		}
	}
	...
	this.readControlChannel = function(cnf){
		var cch;
		for (var band=0;band<2;band++){
			...
			cnf.controlChannel[band] = 0;	 //CCH reset
			if (!factory.chBandEnabled[band] && !factory.adjBandEnabled[band]) continue; //skip if band not licensed
			for (var na=0;na<2;na++){
				for (var fb=0,nch=0;fb<2;fb++){ //both filter band arrays are considered due to flexible filters
					for (var c=0;c<(na==0?config.CHNR:factory.numADJFilters);c++){
						if (!cnf.filterEnabled[na][fb][1][c] || cnf.filterBand[na][fb][1][c]!=band) continue; //skip disabled and filters of other band
						try{
							if ($('#cchEn_'+na+'_'+band+'_'+nch+'_2').prop('checked')){
								cch = na==0? fb*config.CHNR+c+1 : -(fb*config.ADJNR+c+1);
							}
						} catch (err) {}
						nch++;
					}
			...
			//console.log("cch["+band+"] = "+cnf.controlChannel[band]);
		}
	}
	this.setChConf = function(cnf, c,  band, on, realBand, nstart, bw) {
		for (var b = 0; b < 2; ++b) {
			...
			cnf.isFilterGrouped[band][b][c] = false;//it will be computd later
			cnf.bwIndex[band][b][c] = bw;
			cnf.fineGainFilter[0][band][b][c] = 0; //all filters set to fine gain = 0dB
			cnf.sqChEnabled[0][band][b][c] = true; //squelch always enabled
	...
	this.setAdjConf = function(cnf, c,  band, on, realBand) {
		for (var b = 0; b < 2; ++b) {
			cnf.filterEnabled[1][band][b][c] = on;
			if (!on) {
				continue;
			}
			cnf.filterBand[1][band][b][c] = realBand;
			cnf.fineGainFilter[1][band][b][c] = 0; //all filters set to fine gain = 0dB
			cnf.sqChEnabled[1][band][b][c] = true; //squelch always enabled
			cnf.cn_threshold_adj[band][c] = -90; //default C/N threshold for adj filters
		}
	}
	this.readFreqs = function(cnf,band,na,forceFn) {
		pageAdv.init(factory,config);
		//defaults values for basic user (assisted GUI)
		var lic = [factory.chBandEnabled[0]||factory.adjBandEnabled[0],factory.chBandEnabled[1]||factory.adjBandEnabled[1]];
		cnf.uldlLinkedFreq[band]			= true;
		cnf.allSameSquelch[band]			= true;
		cnf.allChSameBW[band]				= [true,true];
		cnf.squelchAdjBwMode 				= false; //sq mode adj = normal
		//pa off when no license, disable filters belonging to band
		for (var b=0;b<2;b++){
			cnf.agcBandMode[b]= false; //stable coverage
			
			for (var nbadj=0;nbadj<2;nbadj++){
				if (!lic[b]){
					cnf.paEnabled[b] = [false,false];
					//cnf.forcePaOff[b] = [true,true]; //PA OFF is done by micro
				}
			}
		}
		//read GUI
		...
		if ( typeof(forceFn) === 'undefined'){
			//Disable filter of the other type: if assisted GUI is configuring channel filters, adj filters ar disabled and viceversa. Except band14
			for (var fb=0;fb<2;fb++){ //both filter band arrays are considered due to flexible filters
				for (var c=0;c<(na==1?cnf.CHNR:cnf.ADJNR);c++){
					if (cnf.filterBand[1-na][fb][1][c]!=band) continue; //skip filters belonging to band
					if (!cnf.filterEnabled[1-na][fb][1][c]) continue; //skip filters already disabled
					if (fb==0 && na==0 && c==0 && cnf.firstADJisFirstNet && cnf.filterEnabled[1][0][1][0]) continue; //first adj filter enable is preserved when configuring 700 channels if band 14 is enabled
					//if (fb==0 && na==1 && cnf.firstADJisFirstNet && cnf.filterEnabled[1][0][1][0]) continue; //700MHz channels enabled are preserved when configuring band 14

					cnf.filterEnabled[1-na][fb][1][c] = false;
					cnf.filterEnabled[1-na][fb][0][c] = false;
				}
			}
			if (na==1 && band==0) cnf.firstADJisFirstNet = false; //band14 disabled if coniguring adj.bw filters for 700MHz
			//set all filters to default values: squelch always enbled, fine gain = 0dB, bw = selected by user
			var nfilt = $('#idcuantoch').val();
			var otherBand = 1 - band;
			var nstartOwnBand = 0, nstartOtherBand;
			if (band==0){
				nstartOwnBand = 0;
				nstartOtherBand = nfilt;
			}else{
				nstartOtherBand = 0;
				nstartOwnBand = numEnabledFilts(0,na,config,factory);
			}
			//re-organize filters of the other band to avoid conflicts
			na == 0 ? reorganizeChFilters(config,cnf,otherBand,nstartOtherBand) : reorganizeAdjFilters(factory,config,cnf,otherBand,nstartOtherBand);
			var cnfaux = new ConfigBDA(); //aux variable to consider updated config
			var frm = cnf.getFrm();
			cnfaux.parse(frm);//copy reorganized config to cnfaux
			var N = na==0?cnf.CHNR:factory.numADJFilters;
			for (var fb=0,nch=0; fb<2; ++fb){ //search for available filters in both band arrays
				for (var c=0;c<N;c++){
					if (cnfaux.filterEnabled[na][fb][1][c] && cnfaux.filterBand[na][fb][1][c]==otherBand) { //skip filter enabled in the other band
						continue;
					}
					if (fb*N + c < nstartOwnBand) { //skip first nstart filters to avoid interference with existing carriers
						cnf.filterEnabled[na][fb][1][c] = false;
						cnf.filterEnabled[na][fb][0][c] = false;
						continue;
					}
					var on = nch < nfilt;
					if (na==0){
						//read channel freqs
						var el = $("#chFreq_"+nch+"_1_"+band+"_0"); //DL freq input element
						if (on){
							cnf.freqHz[fb][1][c] = el.val()*1e6;
							cnf.freqHz[fb][0][c] = cnf.freqHz[fb][1][c]-factory.fstart[2*band+1]+factory.fstart[2*band];
							if ($('#cchEn_'+na+'_'+band+'_'+nch+'_0').prop('checked')){
								cch = fb*N + c + 1;
							}
						}
						this.setChConf(cnf, c, fb, on, band, nstartOwnBand, bw);
					}else{
						//read adj freqs
						var el1 = $("#chAdjFreq_"+nch+"_1_"+band+"_0_0");
						var el2 = $("#chAdjFreq_"+nch+"_1_"+band+"_1_0");
						if (on){
							if ($('#cchEn_'+na+'_'+band+'_'+nch+'_0').prop('checked')){
								cch = -(fb*config.ADJNR+ c + 1);
							}
							cnf.fstartHzAdjFilter[fb][1][c] = el1.val()*1e6;
							cnf.fstopHzAdjFilter[fb][1][c] = el2.val()*1e6;
							cnf.fstartHzAdjFilter[fb][0][c] = cnf.fstartHzAdjFilter[fb][1][c]-factory.fstart[2*band+1]+factory.fstart[2*band];
							cnf.fstopHzAdjFilter[fb][0][c] = cnf.fstopHzAdjFilter[fb][1][c]-factory.fstart[2*band+1]+factory.fstart[2*band];
							if (band==0 && c==0 && cnf.fstartHzAdjFilter[0][1][0]==758000000 && cnf.fstopHzAdjFilter[0][1][0]==768000000) cnf.firstADJisFirstNet = true;
						}
						this.setAdjConf(cnf, c, fb, on, band, nstartOwnBand, bw);
					}
					nch++;
				}
			}
			//compute grouped filters
			if (na==0){
				for (var fb=0; fb<2; ++fb){
					for (var b=0;b<2;b++){
						...
						for (var c=0;c<cnf.CHNR;c++){
							if (cnf.filterBand[0][fb][1][c]==otherBand) { //skip filter enabled in the other band
								continue;
							}
			...
			//check if there are available adj filters:
			var conflict800 = false;
			if (forceFn){
				conflict800 = numEnabledFilts(1,1,config,factory) == (2*factory.numADJFilters); //all adj filters configured in 800
			}
			if (forceFn && !conflict800){
				reorganizeAdjFilters(factory,config,cnf,1,1); //reorganize 800MHz adj filters to avoid conflicts
			}
			...
			if (forceFn){
				...
				for (var i=0;i<2;i++){
					...
					cnf.filterBand[1][0][i][0] = 0; //forced 1st adj to band 700MHz
				}	
			}
		}
		//control overlap
		...
		var overlappingBWs = pageAdv.overlappingBWs;
		...
		var warn = "";
		if (typeof(conflict800)!=="undefined" && conflict800){
			warn+='<h3>CONFIGURATION CONFLICT. ALL ADJ.BW FILTERS ARE CONFIGURED IN 800MHz BAND:</h3>';
			warn+='<a href="javascript:buildGUI(3);" style="color:#0563c1">GO TO 800MHZ BAND SECTION TO DISABLE SOME FILTERS</a><br><br>';
			return warn;
		}
		//refresh freqs to see filter numbering with the same criteria than overlap message:
		this.refreshFreqs(band,na,cnf,0);
		if (fov[0]['check'] || fov[1]['check'] || fov[2]['check'] || fov[3]['check'] || fov[4]['check'] || fov[5]['check']) {
			warn = pageAdv.warningBox.setWarningMessage(fov, overlappingBWs, filtAdjSepKhz, filtNbAdjSepKhz, 2*config.CHNR, factory, [true,true], false);
			...
			if (typeof(forceFn)!=="undefined" && forceFn) warn+='<br><br><a href="javascript:buildGUI(2);" style="color:#0563c1">GO TO 700MHZ BAND SECTION TO ADJUST ENABLED FILTERS AND THEIR  FREQUENCIES</a>';
...
function getGlobalConfiguration(op){
	...
	$.post( "home.html", { global_req:"1"}).done(function( data ) {
		...
		$.get( "/global_conf.shtml?c="+Date.now(), function( str ) {
			...
			var srarr = str.split('\t');
			if (srarr.length < 7) {
				//alert("Error 4"); sometimes this error happened but the routine worked fine
				showResultIcon(ERR_FAIL,indexHome);
				return;
			}
			var srConf = srarr[0];
...
function load_status(){
	$.get( "/update.shtml?c="+Date.now(), function( data ) {
		...
		statCount++;
		monitor.parse(data);
		var N = (nbadjGUI==0?config.CHNR:factory.numADJFilters);
		try{
			for (var fb=0,nch=0;fb<2;fb++){
				for (var c=0;c<N;c++){
					if (!config.filterEnabled[nbadjGUI][fb][1][c] || config.filterBand[nbadjGUI][fb][1][c]!=bandGUI) continue;
					for (var b=0;b<2;b++){
						try{
						setMetValue("level_"+nbadjGUI+"_"+bandGUI+"_"+b+"_"+nch,monitor.level[nbadjGUI][fb][b][c],"undefined",1,"dBm");
		...
		if (indexHome==4){
			...
			pageAdv.setLastRetryTime(monitor.retryTime);
			...
			setTimeout(function() {
				if (typeof genReport === 'undefined') {
					...
					script.onload = function() {
						// Script loaded, now check if genReport is available
						if (typeof genReport !== 'undefined') {
							genReport(0);
						} else {
							setTimeout(function() {showResultIcon(ERR_FAIL,indexHome);}, 1);
							alert('Error: genReport function not found after loading pdfreport.js');
							setTimeout(function() {showResultIcon(ERR_NONE,indexHome); }, 2000);
						}
					};
					script.onerror = function() {
						setTimeout(function() {showResultIcon(ERR_FAIL,indexHome);}, 1);
						alert('Error loading pdfreport.js file');
						setTimeout(function() {showResultIcon(ERR_NONE,indexHome); }, 2000);
					};
...
function getRSSILevels(){
	var k,max,cchExists,c,na,bcch,cch;
	for (k=0;k<2;k++){
		max=-128;
		cchExists = false;
		cch = config.controlChannel[k];
		if (k==0 && config.firstADJisFirstNet && cch==-1){
			cch=0; //No CCH if it is assigned to band14
			c=0;
		}
		if (cch>0){
			na=0;
			...
			c = c % config.CHNR;
			if (bcch<2) cchExists = (config.filterEnabled[na][bcch][1][c] && config.filterBand[na][bcch][1][c]==k);
		}
		if (cch<0){
			na=1;
			c=-cch-1;
		...
		if (cchExists){
			max = monitor.level[na][bcch][1][c];
			naComm[k]=na;
			if (monitor.signalDet[na][bcch][1][c]) signalComm[k]=true;
		}else{
			for (na=0;na<2;na++){
				var N = (na==0?config.CHNR:factory.numADJFilters);
				for (var fb=0;fb<2;fb++){
					for (c=0;c<N;c++){
						var isFn = (fb==0 && c==0 && na==1 && config.firstADJisFirstNet);
						if (config.filterEnabled[na][fb][1][c] && config.filterBand[na][fb][1][c]==k && monitor.level[na][fb][1][c]>max && !isFn){
							max=monitor.level[na][fb][1][c];
							naComm[k]=na;
							if (monitor.signalDet[na][fb][1][c]) signalComm[k]=true;
						}
					}
				}
			}
...
function drawCHmarks(nX, nY, anchoLinea, color) {
	...
	var i, bw, x1, x2;
	try{
		for (var na=0;na<2;na++){ //loop with 2 filter types
			N = (na==0?config.CHNR:factory.numADJFilters);
			for (var fb=0;fb<2;fb++){//both band array filters must be considered due to flexible band assignment
				for (k=0;k<N;k++){
					if (!config.filterEnabled[na][fb][1][k] || config.filterBand[na][fb][1][k]!=b) continue; //skip disabled filters and filters of other band
					if (na==0){//channel filters
						i = 10.0*(config.freqHz[fb][ud][k]-spec.prevFstart)/(spec.prevFstop-spec.prevFstart);
						bw = config.bwKHz[fb][ud][k]*1e4/(spec.prevFstop-spec.prevFstart);
						x1 = (i-bw/2+1)*canvas.width/(nX+1.5);
						x2 = (i+bw/2+1)*canvas.width/(nX+1.5);
					}else{//adj filters
						i = 10.0*(config.fstartHzAdjFilter[fb][ud][k]-spec.prevFstart)/(spec.prevFstop-spec.prevFstart);
						bw = 10.0*(config.fstopHzAdjFilter[fb][ud][k]-spec.prevFstart)/(spec.prevFstop-spec.prevFstart)-i;
						x1 = (i+1)*canvas.width/(nX+1.5);
						x2 = (i+bw+1)*canvas.width/(nX+1.5);
					}
					if ((x2-x1)<4){ //to show narrow filters, at least 4 pixels width
						var m = (x1+x2)/2;
						x1=m-2;
						x2=m+2;
					}
...
function initBWSel(){
	var e = $('#idbw');
	e.empty();
	for (var k=0;k<FilterTypes.length;k++){
		e.append($("<option>").attr('value',FilterTypes[k].name).text(FilterTypes[k].name+"KHz "+(FilterTypes[k].delay).toFixed(1)+"us"));
	}
}
function initNumChSel(band,na){
	...
	var otherBand = 1-band;
	if (na==0){
		num = 2*config.CHNR - numEnabledFilts(otherBand,na,config,factory);
	}else{
		num = 2*factory.numADJFilters - numEnabledFilts(otherBand,na,config,factory);
	}
	for (k=0;k<=num;k++) e.append($("<option>").attr('value',k).text(k));
	$('#noFiltAvailables').hide();
	var first = true;
	for (fb=0;fb<2;fb++){
		for (k=0;k<=num;k++){
			if (config.filterEnabled[na][fb][1][k] && config.filterBand[na][fb][1][k]==band){
				n++;
				if (na!=0) continue; //in ADJ, BW is not shown
				if (first){
					first = false;
					$('#idbw').val(config.bwKHz[fb][1][k]);
					$('#classBnot').css("display",config.bwKHz[fb][1][k]>75?"table-cell":"none");
				}
			}
		}
	}
	if (num==0){
		$('#noFiltAvailables').show();
		e.val(0);
		return;
	}
	e.val(n);
...
function save_config(doIsolMeas, doClearAlarm, forceDLMaxGain){
	...
	if (doIsolMeas || doClearAlarm){
		...
		if (indexHome == 4){
			...
			pageAst.readConf(cnfToSend,bandGUI,nbadjGUI,firstnet);
		}
	}
...
function setGainAndDisplayCommResults(){
	...
	for (var k=0;k<3;k++){
		...
		n[0] = numEnabledFilts(p,0,config,factory);
		n[1] = numEnabledFilts(p,1,config,factory);
		if (k<2){
	...
	for (var k=0;k<3;k++){
		...
		if (enabled[k]){
			...
			$('#dlconfgain'+k).html(monitor.configGain[p][1]); result.actualDLGain[k] = monitor.configGain[p][1];
			$('#ulconfgain'+k).html(monitor.configGain[p][0]); result.actualULGain[k] = monitor.configGain[p][0];
