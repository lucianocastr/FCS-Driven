...
function PageAssisted() {
	...
	this.relayTable = function(mode){ //mode=0 stat, mode=1 config
		...
		//relay rows
		for (k=0;k<(factory.isMMSmode()?8:4);k++){
			r = $('<tr/>');t.append(r);
	...
	this.relayStateSet = function(nrelay,openclose,onoff){
		var isNormalACPowerRelay = factory.isMMSmode() && NFPAcfg.isRelayAssignNormalACpowerExclusive(nrelay);
	...
	$.get( "/update.shtml?c="+Date.now(), function( data ) {
		...
		if (!commissioning || statCount<NUMCOMMMEAS){
			if (indexHome==3){
				...
				if (!dash) tmrIdStat = setTimeout(function() { load_status(); }, 500);
			}
