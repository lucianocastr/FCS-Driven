...
var saveFileCmd = 
[
	{get: "/global_conf.shtml", post:"nosend" , postReg: "global_req", len: 3090, decode: 0},
	{get: "/update_proj.shtml", post:"nosend" , postReg: "", len: 1460, decode: 0},
...
var loadFileCmd = loadFileCmd_2c1;
...
function fileData2cTo2c1(fdatafr) {
	var conf2c1 = ["","","",""];
	conf2c1[0] = fdatafr[0]+"0001518000015180";	// 8th relay timers to RF data
	conf2c1[2] = convertFileAlarmDataToMMSFormat(fdatafr[2]);	// alarm data
	conf2c1[1] = fdatafr[1];	// tags
	conf2c1[3] = fdatafr[3];	// IP
	return conf2c1;
}
function fileData1c7To2c1(fdatafr) {
	var conf2c1 = ["","","",""];
	// conf rf
	conf2c1[0] = fdatafr[0].slice(0, 1442);	// RF without relays
	conf2c1[0] += "8DA1A1A1A18DA1A1A1A12B009C002B009C002B009C000050";	// from DH7S improvements
	conf2c1[0] += fdatafr[0].slice(1442);	// 7 relays
	conf2c1[0] += "0001518000015180";	// 8th relay timers for MMS
	// conf alarms
	conf2c1[2] = convertFileAlarmDataToMMSFormat(fdatafr[2]);	// alarm data
	// tags & IP conf
	conf2c1[1] = fdatafr[1];	// tags
...
function checkFile(fdatafr){
	...
	if (!isLoadfile_2c1(fdatafr)) {
		if (isLoadfile_2c(fdatafr)) {
			fdatafr = fileData2cTo2c1(fdatafr);
		} else if (isLoadfile_1c7(fdatafr)) {
			fdatafr = fileData1c7To2c1(fdatafr);
		} else {
			return false;
		}
	}
	loadFileCmd = loadFileCmd_2c1;
	frtoload = [];
	for (var k=0;k<fdatafr.length;k++){
					...
					if (postget==0){
						...
						if (num<(saveFileCmd.length-1)){
							...
							if (format==0)
								...
								var strFile = overallConfigToTextFile(settings.config,settings.NFPAcfg,settings.factory,settings.tags.getTag(),fdata[3],settings.serNr.sernr);
								fdata = strFile.split("\n");
