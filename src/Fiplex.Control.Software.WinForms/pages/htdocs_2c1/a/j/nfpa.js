...
function refreshEnables(){
	...
	for (var mode=0;mode<3;mode++){
		for (var num=0;num<(mode==1?8:16);num++){
...
function GUI_NFPA() {
	this.read = function(nfpa){
		...
		for (var k=0;k<16;k++){
			...
			if (k<11) {
				nfpa.bbuAlarmsEnabled[k] = document.getElementById("enabled_"+k+"_2_0").checked;
				for (var j=0;j<8;j++) nfpa.relayAssignBbuAlarm[k][j] = document.getElementById("relay_enabled_"+k+"_2_"+j).checked;
				...
				nfpa.bbuAlarmsEnabled[k] = document.getElementById("enabled_"+11+"_2_0").checked;
				for (var j=0;j<8;j++) nfpa.relayAssignBbuAlarm[k][j] = document.getElementById("relay_enabled_"+11+"_2_"+j).checked;
	...
	this.show = function(nfpa){
		...
		for (var k=0;k<16;k++){
			...
			for (var j=0;j<8;j++) document.getElementById("relay_enabled_"+k+"_0_"+j).checked = nfpa.relayAssignGlobalAlarm[k][j];
			if ( k >= 8 && k <= 11 ) {
				var externalAlarmNr = k - 8;
		...
		for (var k=0;k<8;k++){
			...
			for (var j=0;j<8;j++) document.getElementById("relay_enabled_"+k+"_1_"+j).checked = nfpa.relayAssignBandAlarm[k][j];
		}
		for (var k=0;k<16;k++){
			if (k<11) {
				...
				document.getElementById("AlName_"+k+"_2").value = "Annunciators";
				document.getElementById("enabled_"+k+"_2_0").checked = nfpa.bbuAlarmsEnabled[12];	// enable of annunciator #1
			} else  { // (k>11)
				document.getElementById("AlName_"+k+"_2").value = nfpa.alarmNames[2][k];
				// annunciators installed in annunciator's enable checkboxes
			...
			if (k!=11) {
				for (var j=0;j<8;j++) document.getElementById("relay_enabled_"+k+"_2_"+j).checked = nfpa.relayAssignBbuAlarm[k][j];
			} else {	 // common row for annunciators
				// annunciators relay assign is the same for the four of them and set in row with k==11, used settings of annunciator #1
				for (var j=0;j<8;j++) document.getElementById("relay_enabled_"+k+"_2_"+j).checked = nfpa.relayAssignBbuAlarm[12][j];
			}
		}
	...
	this.createRelayConfig = function(){
		...
		if (factory.isMMSmode()) cell.style.display = "none";
		rowb.appendChild(cell);
		for (var mode = 0; mode < 3; mode++){
			var rowb = document.createElement("tr");
			if (!factory.isMMSmode() && mode==2) rowb.style.display = "none";
			tab.appendChild(rowb);
	...
	this.createGeneralRelayConfig = function(mode){
		...
		cell.colSpan = factory.isMMSmode()?24:16;
		row.appendChild(cell);
		var row = document.createElement("tr");
		tb.appendChild(row);
		for (var k=0;k<2;k++){
			...
			cell.colSpan = (factory.isMMSmode()? 8:4);
			row.appendChild(cell);	
		}
		var row = document.createElement("tr");
		tb.appendChild(row);		
		for (var k=0;k<2;k++){
			for (var j=0;j<(factory.isMMSmode()? 8:4);j++){
		...
		for (var j=0;j<(mode==1?4:8);j++) {
			...
			for (var k=0;k<2;k++){
				...
				// hideCell = hideCell || (mode == 2 && j == 5 && k == 1);	// ocultar BBU external input 2, num=11
				var cell = document.createElement("th");
				if ( isUserAlarm ) {
					...
					var annunciatorAlarmNr = 2*(j-6)+k+1;
				...
				for (var i=0;i<8;i++){
					...
					if (!factory.isMMSmode() && i>=4) cell.style.display="none";
	...
	this.createRelayStatusAlarm = function(){
		...
		cell.className = "cth";
		cell.colSpan = factory.isMMSmode()?11:5;
		var row = document.createElement("tr");
		tb.appendChild(row);		
		for (var num = 0;num<4;num++){
			...
			for (var colNr = 0; colNr < 2; colNr++) {
				...
				var hideCell = (relayNr>=4 && !factory.isMMSmode());
	...
	this.bbuMeasurements = [
		...
		{id:"IndividualBatteryVoltage1", text:"Individual Battery Voltage 1 (V)"},
		...
		{id:"IndividualBatteryVoltage2", text:"Individual Battery Voltage 2 (V)"},
