...
function Page() {
	...
	this.createAlarmTable = function(){
		...
		for (var k=0;k<self.NFPA.bbuAlarmsInstalled.length;k++){
			var show = self.NFPA.bbuAlarmsInstalled[k]&&this.factory.isMMSmode();
	...
	this.createRelayTable = function(){
		...
		for (var k=0;k<8*2;k++){
			var row = document.createElement("tr");
			if (!self.factory.isMMSmode()) {
				if (k>=8) row.style.display = "none";
			}
			tb.appendChild(row);
			if ((k % 2)==0){
				var cell = document.createElement("th");
	...
	this.relayStateSet=function(nrelay,openclose,onoff){
		var isNormalACPowerRelay = self.factory.isMMSmode() && self.NFPA.isRelayAssignNormalACpowerExclusive(nrelay);
	...
	this.load_status = function(){
		$.get( "/update.shtml?c="+Date.now(), function( data ) {
			...
			if (self.statCount<self.NUMCOMMMEAS){
				tmrIdStat = setTimeout(function() { self.load_status(); }, 500);
			} else {
