function ConfigBDA() {
	...
	this.squelchAdjBwMode = [false,false];
	...
	this.agcBandMode 			= []; //band0 ul, band0 dl, band1 ul, band1 dl
	this.controlChannel = [];
	...
	for (var band=0;band<2;band++){
		...
		for (var uldl=0;uldl<2;uldl++){
			...
			this.agcBandMode[band].push(false);			
		}
	}
	for (var band=0;band<2;band++){
	...
	this.parse = function(s) {
		...
		for (var b=0;b<2;b++){ //Band0/1
			...
			this.squelchAdjBwMode[b] = (res & 0x40)!=0;
			...
			if (b==0) this.automatic_ul_gain_running = (res & 0x40)!=0;
			//GAINISOLMARGIN
			...
			for (var i=0;i<2;i++){//UL/DL
				...
				if (i==1) {
					// AGC BAND MODE bits
					for (var k = 0, agcbandmask = 0x04; k < 2; k++, agcbandmask<<=1 ) {
						this.agcBandMode[b][k] = (res & agcbandmask)!=0; // 0 NORMAL, 1 MAX POWER
					}
				}
	...
	this.getFrm = function(){
		...
		for (b=0;b<2;b++){ //Band0/1
			...
			if (factory.isClassB(b)) {
				// modo squelch Adj.BW solamente si factory lo permite
				if (this.squelchAdjBwMode[b]) mask|=0x40;
			}
			cfg += hexformat(mask,2);	
			//OSCFEATURE GENERAL
			...
			for (i=0;i<2;i++){//UL/DL
				...
				if (i==1) {
					// AGC BAND MODE bits
					for (var k = 0, agcbandmask = 0x04; k < 2; k++, agcbandmask<<=1 ) {
						if (this.agcBandMode[b][k]) mask |= agcbandmask;  // 0 NORMAL, 1 MAX POWER
					}
				}
	...
	this.compare = function(cnf) {
		...
		for (var band=0;band<2;band++){
			...
			if (this.muteModeLinked[band]!=cnf.muteModeLinked[band]) return 6;
			if (this.simplexMode[band]!=cnf.simplexMode[band]) return 7;
			if (this.gainIsolMargin[band]!=cnf.gainIsolMargin[band]) return 8;
			if (this.oscFeatureEnabled[band]!=cnf.oscFeatureEnabled[band]) return 9;
			...
			if (this.oscActionAfterAlarm[band]!=cnf.oscActionAfterAlarm[band]) return 12;
			...
			for (var ud=0;ud<2;ud++){
				...
				for (var ch=0;ch<(factory.singleBandEnabled[band]?2*this.CHNR:this.CHNR);ch++){
				...
				if (this.agcBandMode[band][ud]!=cnf.agcBandMode[band][ud]) return 31;
			}
...
function NFPAconf(){
	...
	for (var k=0;k<16;k++){
		...
		for (var j=0;j<8;j++) this.relayAssignGlobalAlarm[k].push(false);
	}
	for (var k = 0; k < 4; k++) {
	...
	this.parse = function(s) {
		...
		for (var k=0;k<16;k++){
			...
			// k==11 ---> External alarm 2 of charger board, not used
			if (k<11) {
	...
	this.isRelayAssignNormalACpowerExclusive = function(relayNr) {
		...
		for (var k=1;k<8;k++){	// skip alarm nr 0, "Normal AC Power"
...
function NFPAstatus(){
	...
	this.parse = function(s) {
		...
		for (var k=0;k<2;k++){
			...
			res = parseInt(s.substr(148+20*k,6), 16);
			this.txLowerPowerTimeHigh[k] = (res & 0xff0000)>>16 | (res & 0xff00) | (res & 0xff)<<16;
			res = parseInt(s.substr(154+20*k,6), 16);
...
function alarmSettingsToText(nfpacfg){
	...
	var exist_el;
	var cfg = "";
	var gralAlarmNames = ["HW Fail","High Temperature","Antenna Isolation","Oscillation Detection","UL PA Fail","Antenna Disconnection"];
	var bandAlarmNames = ["Overload UL","Overload DL","DL PA Fail","Tx Power Low","VSWR","RxPower Low / Donor Antenna", "DL AGC Fail", "C/N UL Low"];
	...
	for (k=0;k<6;k++) cfg+="\tAlarm Enable " + gralAlarmNames[k] + ": " + boolToYN(nfpacfg.globalAlarmsEnabled[k]) + "\n";
	for (k=0;k<8;k++) cfg+="\tAlarm Enable " + bandAlarmNames[k] + ": " + boolToYN(nfpacfg.bandAlarmsEnabled[k]) + "\n";
	if (factory.isMMSmode()) {
		for (k=0;k<12;k++) {
	...
	for (k=0;k<6;k++){
		...
		exist_el = false;
		for (j=0;j<(factory.isMMSmode()? 8:4);j++){
	...
	if (factory.isMMSmode()) {
		for (k=0;k<12;k++){
			exist_el = false;
			if (k < 11) {
				if ((bbuAlarmNames[k].length == 0) || (bbuAlarmNames[k] == "Undefined")) continue;
				cfg+="\tRelay Assigned " + bbuAlarmNames[k] + ": ";
				for (j=0;j<8;j++){
				...
				// annunciators relay assign is the same for the four of them, use annunciator1
				cfg+="\tRelay Assigned " + "Annunciators" + ": ";
				for (j=0;j<8;j++){
					if (nfpacfg.relayAssignBbuAlarm[12][j]){
						cfg+=(j+1)+", ";
						exist_el = true;
					}
				}
	...
	if (factory.isMMSmode()) {
		...
		for (k=0;k<4;k++)
			cfg+=nfpacfg.alarmNames[2][12+k].trim()+", ";
		cfg = cfg.substring(0,cfg.length-2)+"\n";
	}
	return cfg;
}
function rfConfigToText(config, nfpacfg, factory){
	...
	for (i=0;i<2;i++){
		if (factory.chBandEnabled[i] || factory.adjBandEnabled[i]){
			...
			//Base station
			cfg += rfConfigBaseStationToText(config, i);
		}
	}
...
function rfConfigGeneralAlarmToText(config, nfpacfg){
	...
	if (config.extremeTempAction==0)
		...
		cfg+="PA OFF\n";
	cfg+="\tAntenna Disconnection Input Threshold (dBm): " + (nfpacfg.antennaDisconnectionThreshold) + "\n";
...
function rfConfigGeneralBandToText(config,band){
	...
	for (j=0;j<2;j++) cfg+="\t\tAGC Mode " + uldl[j]+ ": " + agcModeText(config.agcBandMode[band][j])+"\n";
	...
	if (!factory.chBandEnabled[band] && factory.adjBandEnabled[band]) {
		cfg+="\t\tAdj.BW Squelch Mode (NORMAL/FILTERS BASED): "+
			(config.squelchAdjBwMode[band]?"FILTERS BASED":"NORMAL")+"\n";
	}
	return cfg;
}
...
function rfConfigAdjBwFiltersToText(config, factory, band){
	...
	if (factory.adjBandEnabled[band]){
		...
		for (j=0;j<2;j++){
			cfg+="\t\tFilter Squelch Threshold " + uldl[j] + " (dBm): ";
			if (!factory.chBandEnabled[band] && factory.adjBandEnabled[band] && config.squelchAdjBwMode[band]){
				cfg+="Based on Narrow Band Filters";
			} else {
		...
		cfg+="\n";
		if (!factory.chBandEnabled[band] && factory.adjBandEnabled[band] && config.squelchAdjBwMode[band]){
			cfg+="\tNarrow Band Filters for Squelch Detection of ADJBW\n";
			var maxch = config.CHNR;
			if (factory.singleBandEnabled[band]) maxch *= 2;
			nfilters = 0;
			var sqfilterson = [];
			...
			//Enable
			cfg+="\t\tFilter ON: ";
			for (j=0;j<nfilters;j++) cfg+=(sqfilterson[j]+1)+", ";
			if (nfilters>0) cfg = cfg.substring(0,cfg.length-2);
			cfg+="\n";	
			//Frequency
			for (j=0;j<2;j++){
				cfg+="\t\tFilter Frequency " + uldl[j] + " (MHz): ";
				for (k=0;k<nfilters;k++) cfg+=(config.freqHz[band][j][sqfilterson[k]]/1e6)+", ";
				if (nfilters>0) cfg = cfg.substring(0,cfg.length-2);
				cfg+="\n";
			}
			//BW
			for (j=0;j<2;j++){
				cfg+="\t\tFilter Bandwidth " + uldl[j] + " (KHz): ";
				for (k=0;k<nfilters;k++) cfg+=(config.bwKHz[band][j][sqfilterson[k]])+", ";
				if (nfilters>0) cfg = cfg.substring(0,cfg.length-2);
				cfg+="\n";
			}
			//Squelch Threshold
			for (j=0;j<2;j++){
				...
				if (j==0)
					...
					for (k=0;k<nfilters;k++) cfg+=(config.sqChThreshold[0][band][j][sqfilterson[k]])+", ";
					if (nfilters>0) cfg = cfg.substring(0,cfg.length-2);
					cfg+="\n";
				}
			}
