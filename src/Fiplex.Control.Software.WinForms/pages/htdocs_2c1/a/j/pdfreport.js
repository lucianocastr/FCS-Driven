...
function writeAlarmConfigTable(doc,y,xn){
	...
	for (k=0;k<NFPAcfg.alarmNames[0].length;k++){
		if (NFPAcfg.alarmNames[0][k].length>0 && NFPAcfg.alarmNames[0][k].search("Undefined")<0){
			for (rel="",p=0;p<(factory.isMMSmode()? 8:4);p++){
	...
	if (factory.isMMSmode()) {
		for (k=0;k<NFPAcfg.alarmNames[2].length;k++){
			if (NFPAcfg.alarmNames[2][k].length>0 && NFPAcfg.alarmNames[2][k].search("Undefined")<0 || (k==11)){
				for (rel="",p=0;p<8;p++){
					if (k < 12) {
						if (NFPAcfg.relayAssignBbuAlarm[k][p]) rel+=(p+1)+", ";
					} else {
						// annunciators relay assign is the same for the four of them, use annunciator1
						if (NFPAcfg.relayAssignBbuAlarm[12][p]) rel+=(p+1)+", ";
					}
				}
				if (rel.length>2)
					rel = rel.substr(0,rel.length-2);
