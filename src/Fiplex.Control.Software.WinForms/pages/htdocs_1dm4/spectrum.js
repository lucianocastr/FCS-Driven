...
function onloadInit() {
	factory = new Factory();
	var factfrm = localStorage.getItem("factory_1dm4"+window.location.host);
	factory.parse(factfrm);
	config = new Config(maxChNr);
	var cfgfrm = localStorage.getItem("cfg_1dm4"+window.location.host);
	config.parse(cfgfrm);
...
function Spectrum(maxChNr) {
	...
	this.createBandLimitsForm = function() {
		...
		if (fstartspec==null)
			fstartspec=factory.data.band[0].fStart;
		else{
		...
		if (fstopspec==null)
			fstopspec=factory.data.band[0].fStop;
		else{
