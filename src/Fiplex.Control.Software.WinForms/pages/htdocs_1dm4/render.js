...
function rfRedunded() {
	...
	this.readStream = function() {
		try {
			...
			var srs = localStorage.getItem("serial_1dm4"+window.location.host).split('\t');
...
function createStatusTable(nr) {
	...
	if (nr == 0) {
		...
		if (window.top.NFPAMonitor[0]!=0){
			...
			td.style.textAlign = "left";
			tr.appendChild(td);
			td = createLedBox("nfpaStatusLed");
			tr.appendChild(td);
			td = document.createElement("td");
			td.id = "nfpaStatusText"
			tr.appendChild(td);
			tr = document.createElement("tr");
			tb.appendChild(tr);
			td = document.createElement("th");
			...
			td.style.textAlign = "left";
			tr.appendChild(td);
			td = createLedBox("hpaTxLowLed"+nr);
			tr.appendChild(td);
			td = document.createElement("td");
			td.id = "nfpaStatusText"
			tr.appendChild(td);			
...
function optPopup(nr,ne,url) {
	...
	var fmask = localStorage.getItem("fibermask_1dm4"+window.location.host);
