...
function optParseStatus(serverResponse, links) {
	...
	for (var side = 0; side < sidemax; side++) {
		...
		shift += 4;
		setText("voltage_"+side_m,volt.toFixed(1),computeFoAlarmVolt(volt));
		...
		var rxlow = (rxpow <= foDataLimitsAlarm.rxpow.min);
		if (sfpalarm & 0x02) {
			rxlow = true;
		}
		setColor("optRxErr_"+side_m, rxlow ? "red":"green");
		var alarm = computeFoAlarmRxpow(rxpow);
		setText("rxPowTxt_"+side_m,rxpow.toFixed(1),alarm);
		shift += 2;
		var rx_is_zero = (~~Math.round(rxpow) == -130);
		if (rx_is_zero) {
			setColor("optModKO_"+side_m, "grey");
			setColor("txrxStat_"+side_m, "grey");
			setColor("optTxErr_"+side_m, "grey");
		} else {
			alarm = computeFoStatAlarm(temp, volt, bias, txpow, rxpow, sfpalarm, gtpalarm);
			setColor("optModKO_"+side_m,(alarm == 2)?"red": (alarm == 1)?"yellow":"green");
			setColor("txrxStat_"+side_m,(gtpalarm & 0x01)?"red":"green");
...
var foDataLimitsWarning = {
	temperature: 	{min: -35,	max: 75	},
	voltage: 	{min: 3.1, 	max: 3.5},
	bias: 		{min: 7, 	max: 55	},
	txpow: 		{min: -5, 	max: 7	},
...
function computeFoStatAlarm(temp, volt, bias, txpow, rxpow, sfpalarm, gtpalarm) {
	...
	if (rxpow > foDataLimitsWarning.rxpow.max) 
	{
		alarm = 1;
	}
...
function computeFoAlarmRxpow(rxpow) {
	...
	if ((rxpow < foDataLimitsWarning.rxpow.min) 
		|| (rxpow > foDataLimitsWarning.rxpow.max))
	{
		alarm = 1;
	}
