...
function NFPAconf(){
	...
	this.parse = function(s) {
		...
		for (var k=0;k<8;k++){
			...
			this.bandAlarmsInstalled[k] = (k<7); //Se fuerzan "available" las 7 primeras
		}
		for (var k=0;k<16;k++){
			res = parseInt(s.substring(ind,ind+2), 16); ind+=2;
			for (var j=0;j<7;j++){
...
function alarmSettingsToText(nfpacfg){
	...
	for (k=0;k<7;k++) cfg+="\tAlarm Enable " + bandAlarmNames[k] + ": " + boolToYN(nfpacfg.bandAlarmsEnabled[k]) + "\n";
	for (k=0;k<6;k++){
