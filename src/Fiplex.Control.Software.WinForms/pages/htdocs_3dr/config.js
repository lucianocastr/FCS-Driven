...
function NFPAconf(){
	...
	this.alarmNames				= [["HW Fail","High Temperature","Antenna Isolation","Oscillation Detection","UL PA Fail","Antenna Disconnection","Fiber Optic 1","Fiber Optic 2",
								    "External Input 1","External Input 2","External Input 3","External Input 4","General System Alarm","Undefined","Undefined","Undefined"],
	...
	this.parse = function(s) {
		...
		for (var k=0;k<16;k++){
			...
			/* el byte 15 se dedicarÃ¡ a las generalSystemAlarm de las alarmas externas
...
function alarmSettingsToText(nfpacfg, factory){
	...
	for (k=0;k<13;k++) {
		...
		if (factory.limitTo32CH && k==12) continue; //General System Alarm does not existe in miniDAS
	...
	if (!factory.limitTo32CH){
		for (k=0;k<13;k++){
			...
			if (k==12) continue; // se excluye General System Alarm
