...
function load_status() {
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState ===4 && this.status === 200) {
				monitor.parse(this.responseText,factory.limitTo32CH);
...
function load_NFPAstatus() {
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState ===4 && this.status === 200) {
				monitorNFPA.parse(this.responseText);
...
function GuiFact() {
	...
	this.shMasterDonnorAntenna = function(fact, b) {
		var el = document.getElementById('masterRxLow'+'_'+b);
		el.checked = fact.masterRxLowAlarm;
	}
	...
	this.rdMasterDonnorAntenna = function(fact, b) {
		var el = document.getElementById('masterRxLow'+'_'+b);
		fact.masterRxLowAlarm = el.checked;
	}
	...
	this.shRelayModeConf = function(fact,b) {
		...
		el.value = fact.relayModeConfiguration;
	}
	...
	this.Map = [
		[
			...
			},
			{
				id:'masterRxLow',
				display: true,
				label:'MASTER&nbsp;DONOR&nbsp;ANTENNA',
				type: 'input',
				inputtype: 'checkbox',
				show: this.shMasterDonnorAntenna,
				read: this.rdMasterDonnorAntenna,
				len: 1
			},
			{
	...
	this.createForm = function() {
		...
		var mainTb = document.createElement("tbody");
		mainTab.appendChild(mainTb);
		var order = [3,1,2,0,4,5];
		var n_elem = [4,2,2,2,6,1];
		var subtitlemode = [1,2,3,4,5,0];
