...
function optOnLoadFunc() {
	...
	config = localStorage.getItem("Conf"+Prjstr+window.location.host);
	twofibers = true;
	mode = 0;

	renderOpticalTable();
	monitor = new Monitor();
	var statStr = localStorage.getItem("Status"+Prjstr+window.location.host);
	limitTo32CH = localStorage.getItem("Factory_limitTo32CH"+Prjstr+window.location.host); 
	monitor.parse(statStr,limitTo32CH);
	optLoadStatus();
}
