	...
	for (var mode=0;mode<2;mode++){
		for (var num=0;num<(mode==0?16:8);num++){
			...
			if ( (mode==1 && (num==1||num==5))){
				//checkbox system alarm deshabilitado en Overload DL ni Rx Power Low
				try {
					var el = document.getElementById("systemAlarm_"+num+"_"+mode+"_0");
					el.checked = false;
					el.disabled = true;
				}catch(e){}
			}
...
function GUI_NFPA() {
	this.read = function(nfpa){
		...
		for (var k=0;k<16;k++){
			...
			nfpa.generalSystemAlarm[0][k] = document.getElementById("systemAlarm_"+k+"_0_0").checked;
			for (var j=0;j<4;j++) {
		...
		for (var k=0;k<8;k++){
			...
			nfpa.generalSystemAlarm[1][k] = document.getElementById("systemAlarm_"+k+"_1_0").checked;
		}
	...
	this.show = function(nfpa){
		...
		for (var k=0;k<16;k++){
			...
			document.getElementById("systemAlarm_"+k+"_0_0").checked = nfpa.generalSystemAlarm[0][k];
		}
		for (var k=0;k<8;k++){
			if (k==5)
			...
			document.getElementById("systemAlarm_"+k+"_1_0").checked = nfpa.generalSystemAlarm[1][k];
		}
		/* antennaDisconnectionThresold solamente banda #0 */
	...
	this.createGeneralRelayConfig = function(tb,mode){
		...
		for (var j=0;j<(mode==0?8:4);j++) {
			...
			tb.appendChild(row);
			if (mode == 0) {
				if ( j > 6 || j == 2 ) // En remoto se oculta fila con PA UL FAIL y ANTENNA DISCONNECTION
				{
			...
			for (var k=0;k<2;k++){
				var hideCell = (mode == 0 && j == 4 && k == 1); // se oculta alarm 8 que no existe
				var ix = k+2*j;
				if (mode==1) {
				...
				if ( factory.limitTo32CH ) {
					// hide "General System Alarm"
					hideCell = hideCell || (mode == 0 && ix==8);
				...
				row.appendChild(cell);
				var cell = this.createInputText("AlName",ix,mode); //Se cambia ordenación (*)
				row.appendChild(cell);
				cell.className = "alarmTable";
				if (hideCell) {
					cell.style.display = "none";
				}
				var cell = this.createEnabledCheckBox(ix,mode,0); //Se cambia ordenación (*)
				row.appendChild(cell);
				cell.className = "alarmTable";
				...
				var cell = this.createSystemCheckBox(ix,mode,0); //Se cambia ordenación (*)
				row.appendChild(cell);
				cell.className = "alarmTable";
				cell.style.textAlign = "center";
				cell.style.minWidth = "40px";
	...
	this.createSystemCheckBox = function(num,mode,enav){
		var cell = document.createElement("td");
		var el = document.createElement("input");
		el.id = "systemAlarm_"+num+"_"+mode+"_"+enav;
		el.name = el.id;
		el.type = "checkbox";
		el.enav = enav;
		cell.appendChild(el);
