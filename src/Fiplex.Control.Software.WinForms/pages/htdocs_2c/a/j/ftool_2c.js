function ftoolGui() {
	...
	this.applyFreqs = function(band) {
		...
		for (var c = 0; c < chList.length; ++c) {
			var chnr = chList[c];
			self.config.freqHz[band][b][c]=self.factory.fref[2*band+b]+self.factory.fstep*chnr;
			chnr = self.computeChNrOtherBand(chnr, b, c, band);
			self.config.freqHz[band][a][c]=self.factory.fref[2*band+a]+self.factory.fstep*chnr;
		}
	}
