...
var saveFileCmd = 
[
	{get: "/global_conf.shtml", post:"nosend" , postReg: "global_req", len: 2886, decode: 0},
...
var loadFileCmd = loadFileCmd_2c;
...
function fileData1c7To2c(fdatafr) {
	var conf2c = ["","","",""];
	// conf rf
	conf2c[0] = fdatafr[0].slice(0, 1442);	// RF without relays
	conf2c[0] += "8DA1A1A1A18DA1A1A1A12B009C002B009C002B009C000050";	// from DH7S improvements
	conf2c[0] += fdatafr[0].slice(1442);	// 7 relays
	// conf alarms
	conf2c[2] = fdatafr[2];	// alarm data
	// tags & IP conf
	conf2c[1] = fdatafr[1];	// tags
	conf2c[3] = fdatafr[3];	// IP
	return conf2c;
}
function checkFile(fdatafr){
	...
	if (!isLoadfile_2c(fdatafr)) {
		if (isLoadfile_1c7(fdatafr)) {
			fdatafr = fileData1c7To2c(fdatafr);
		} else {
			return false;
		}
	}
	loadFileCmd = loadFileCmd_2c;
	frtoload = [];
	for (var k=0;k<fdatafr.length;k++){
