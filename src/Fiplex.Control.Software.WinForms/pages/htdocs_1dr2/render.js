...
function createFilteringTable(nr) {
	...
	if (window.top.FiberMask==0x03)
	{
		var modeCtl = createModeCtl();
		filtBox.appendChild(modeCtl);
	}
...
function createModeCtl(){
	...
	el.onchange = function() {
		window.top.Mode=el.selectedIndex<<3;
		redrawAll(false);
	}
	...
	opt.text = "ADDITION";
	opt.value = 1;
	el.selectedIndex = (window.top.Mode == 0x8) ? 1:0;
...
function createFilterChannel(ch, nr) {
	...
	chGain.type = "text";
	chGain.size = "3";
	chGain.onkeypress = function(ev) { ev = ev || window.event; if (ev.keyCode == 13) {submitform(-1); return false;} else return true; }
