...
function parse_status(serverResponse) {
	...
	shift+=2;
	num = to_float(parseInt(serverResponse.substr(shift,4),16));
	boardTempSet(1, num);
	shift+=4;
	num = to_float(parseInt(serverResponse.substr(shift,4),16));
	rfOutPowSet(1, num);	
	...
	{
		bitmask = parseInt(serverResponse.substr(shift,2),16);
		optFrSyncLedSet(nfib, (bitmask & 0x08)? "red":"green");
		shift+=2;
		...
		alarm = false;
		shift-=24;
		temp = to_float(parseInt(serverResponse.substr(shift,4),16)).toFixed(1);
		if (temp > 75 || temp < -35) {
			if (temp > 85 || temp < -40) {
				alarm = true;
			} else {
		...
		shift+=4;
		volt = parseInt(serverResponse.substr(shift,4),16) * 1e-4;
		...
		shift+=4;
		txpow = 10 * Math.log(parseInt(serverResponse.substr(shift,4),16) + 1e-9) / Math.LN10 - 36;
		if (txpow > 6 || txpow < -5) {
			if (txpow > 7 || txpow < -6) {
				alarm = true;
			} else {
		...
		optRxLedSet(nfib, alarm? "red": (warning? "yellow" : "green"));
		shift+=28;
	}
	shift=70;
	bitmask = parseInt(serverResponse.substr(shift,2),16);
...
function parse_config(serverResponse) {
	...
	stbychannels = parseInt(serverResponse.substr(shift,6),16);
	shift += 6;
	/*for (ch = 1, mask = 1; ch <= ch_nr; ++ch) {
		chEnableMSet(ch, 1, (stbychannels & mask));
		mask <<= 1;
	}*/
	stbychannels = parseInt(serverResponse.substr(shift,6),16);
	shift += 6;
	/*for (ch = 1, mask = 1; ch <= ch_nr; ++ch) {
		chEnableMSet(ch, 2, (stbychannels & mask));
		mask <<= 1;
	}*/
}
...
function format_config(currentConfigFrm, reset_nr) {
	...
	if (false)//(getRemoteMode())
	{
		stbychannels = 0;
		for (var ch = 1, mask = 1; ch <= window.top.MaxChNr; ++ch) {
			var stby = chEnableMIsSet(ch, 1);
			if (stby) {
				stbychannels |= mask;
			}
			mask <<= 1;
