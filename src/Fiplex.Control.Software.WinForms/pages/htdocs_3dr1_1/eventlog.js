	...
	this.Gui = function() {
		...
		this.getLogData = function(fieldSeparator, withBootDelimiters, withHeader) {
			...
			for (var n = 0; n < self.logDataMaxRecords; n++) {
				...
				var alarmName = self.gui.getEventName(r.type, factory, config);
		...
		this.getEventName = function(rtype, factory, config) {
			var alarmName = "";
			var NrAlarmsGeneral = config.alarmNames[0].length;
			var NrAlarmsBand = config.alarmNames[1].length;
			var NrAlarmsBbu = config.alarmNamesBbu.length;
			var index = rtype;
			if ( index >= 0 && index < NrAlarmsGeneral ) {
				alarmName = config.alarmNames[0][index];
			} else {
				index -= NrAlarmsGeneral;
				if ( index >= 0 && index < NrAlarmsBand ) {
					alarmName = config.alarmNames[1][index];
					alarmName += "  ";
					...
					if ( index >= 0 && index < NrAlarmsBand ) {
						...
						if ( index >= 0 && index < NrAlarmsBbu ) {
							alarmName = config.alarmNamesBbu[index];
						}
					}
				}
			}
