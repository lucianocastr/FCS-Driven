	...
	try {
		...
		xhr.onreadystatechange  = function(ev) {
			if (this.readyState === 4 && this.status === 200) {
				setTimeout(function() { loadConfigGlobal(); }, computeTimeoutReqMs());
			}
		}
...
function GuiFact() {
	...
	this.shCommonUlEn = function(fact, b) {
		var el = document.getElementById('commomUlEn'+'_'+b);
		el.checked = fact.commonUl;
	}
	this.shFiberLoopTestModeEn = function(fact, b) {
		var el = document.getElementById('fiberLoopTestModeEn'+'_'+b);
		el.checked = fact.fiberLoopTestMode;
	}
	this.shLimitTo32CH = function(fact, b) {
		var el = document.getElementById('limitTo32CH'+'_'+b);
		el.checked = fact.limitTo32CH;
	}
	this.shTestPowerLoop = function(fact, b) {
		var el = document.getElementById('testPowerLoop'+'_'+b);
		el.checked = fact.testPowerLoopMode;
	}
	...
	this.shExternalPA = function(fact, b) {
		var el = document.getElementById('externalPA'+'_'+b);
		el.selectedIndex = (fact.externalPA? 1 : 0);
	}
	this.shPlaBand = function(fact, b) {
		var el = document.getElementById('plaBand'+'_'+b);
		el.selectedIndex = fact.plaBand & 0x1;
	}
	...
	this.shBandLabel = function(fact, b) {
		...
		var el = document.getElementById('plaBand_0');
		el.options[b].text =  fact.bandNames[b];
	}
	this.shGainBandCorr = function(fact, b) {
		var el = document.getElementById('gainBandCorrection'+'_'+b);
		el.value = fact.gainBandCorrection[b].toFixed(1);
	}
	this.shPowerBandCorr = function(fact, b) {
		var el = document.getElementById('powerBandCorrection'+'_'+b);
		el.value = fact.powerBandCorrection[b].toFixed(1);
	}
	...
	this.rdCommonUlEn = function(fact, b) {
		var el = document.getElementById('commomUlEn'+'_'+b);
		fact.commonUl = el.checked;
	}
	this.rdFiberLoopTestModeEn = function(fact, b) {
		var el = document.getElementById('fiberLoopTestModeEn'+'_'+b);
		fact.fiberLoopTestMode = el.checked;
	}
	this.rdLimitTo32CH = function(fact, b) {
		var el = document.getElementById('limitTo32CH'+'_'+b);
		fact.limitTo32CH = el.checked;
	}
	this.rdTestPowerLoop = function(fact, b) {
		var el = document.getElementById('testPowerLoop'+'_'+b);
		fact.testPowerLoopMode = el.checked;
	}
	...
	this.rdExternalPA = function(fact, b) {
		var el = document.getElementById('externalPA'+'_'+b);
		fact.externalPA = (el.selectedIndex != 0);
	}
	this.rdPlaBand = function(fact, b) {
		var el = document.getElementById('plaBand'+'_'+b);
		fact.plaBand = el.selectedIndex;
	}
	...
	this.rdGainBandCorr = function(fact, b) {
		var el = document.getElementById('gainBandCorrection'+'_'+b);
		fact.gainBandCorrection[b] = parseFloat(el.value);
	}
	this.rdPowerBandCorr = function(fact, b) {
		var el = document.getElementById('powerBandCorrection'+'_'+b);
		fact.powerBandCorrection[b] = parseFloat(el.value);
	}
	...
	this.shPlaAvailable = function(fact, b) {
		var el = document.getElementById('plaAvailable'+'_'+b);
		el.checked = fact.plaAvailable;
	}
	this.rdPlaAvailable = function(fact, b) {
		var el = document.getElementById('plaAvailable'+'_'+b);
		fact.plaAvailable = el.checked;
	}
	this.shPlaTime = function(fact, b) {
		var el = document.getElementById('plaMeasTime'+'_'+b);
		el.value = fact.plaMeasTimeMs;
	}
	this.rdPlaTime = function(fact, b) {
		var el = document.getElementById('plaMeasTime'+'_'+b);
		fact.plaMeasTimeMs = parseInt(el.value);
	}
	...
	this.limitTo32CHevent = function(ev) {
		/** set adj nr of filters to 2 if minidas */
		var target = document.getElementById('limitTo32CH_0');
		var el = document.getElementById('numADJFilters_0');
		if (target.checked) {
			el.selectedIndex = 1;
			el.disabled = true;
			el.style.backgroundColor = "#CCCCCC";
		} else {
			el.selectedIndex = factory.numADJFilters-1;
			el.disabled = false;
	...
	this.shAgcDeltaHybridMode = function(fact, b) {
		var el = document.getElementById('agcDeltaHybridMode_'+b);
		el.value = fact.agcDeltaHybridMode.toFixed(1);
	}
	this.rdAgcDeltaHybridMode = function(fact, b) {
		var el = document.getElementById('agcDeltaHybridMode_'+b);
		var v = parseFloat(el.value)
		if (v < 0) v=0;
		fact.agcDeltaHybridMode = v;
	}
	...
	this.Map = [
		[
			...
			},
			{
				id:'externalPA',
				display: true,
				label:'INTERNAL/EXTERNAL&nbsp;DL&nbsp;PA&nbsp;',
				type: 'select',
				options: [["INTERNAL", "EXTERNAL"]],
				show: this.shExternalPA,
				read: this.rdExternalPA,
				len: 1
			},
			{
				id:'opfEnable',
			...
			},
			{
				...
				type: 'input',
				inputtype: 'checkbox',
				show: this.shCommonUlEn,
				read: this.rdCommonUlEn,
				len: 1
			},
			{
				id:'fiberLoopTestModeEn',
				display: true,
				label:'FIBER&nbsp;LOOP&nbsp;MODE&nbsp;',
				type: 'input',
				inputtype: 'checkbox',
				show: this.shFiberLoopTestModeEn,
				read: this.rdFiberLoopTestModeEn,
				len: 1
			},
			{
				id:'testPowerLoop',
				display: true,
				label:'TEST&nbsp;POWER&nbsp;LOOP&nbsp;MODE',
				type: 'input',
				inputtype: 'checkbox',
				show: this.shTestPowerLoop,
				read: this.rdTestPowerLoop,
				len: 1
			},
			{
				id:'limitTo32CH',
				...
				type: 'input',
				inputtype: 'checkbox',
				onclickev: this.limitTo32CHevent,
				show: this.shLimitTo32CH,
				read: this.rdLimitTo32CH,
				len: 1
			},
			{
			...
			},
			{
				id:'plaAvailable',
				display: true,
				label:'PATH&nbsp;LOSS&nbsp;ANALYZER&nbsp;AVAILABLE',
				type: 'input',
				inputtype: 'checkbox',
				show: this.shPlaAvailable,
				read: this.rdPlaAvailable,
				len: 1
			},
			{
				id:'plaBand',
				display: true,
				label:'PATH&nbsp;LOSS&nbsp;BAND',
				type: 'select',
				options: [["0", "1"]],
				show: this.shPlaBand,
				read: this.rdPlaBand,
			...
			{
				id:'agcDeltaHybridMode',
				display: true,
				label:'MAX&nbsp;AGC&nbsp;REDUCTION&nbsp;HYBRID&nbsp;MODE',
				type: 'input',
				inputtype: 'text',
				onkey: isKeyDecimalNumber,
				show: this.shAgcDeltaHybridMode,
				read: this.rdAgcDeltaHybridMode,
				len: 1
			},
			{
		...
		[				
			...
			{
				id:'gainBandCorrection',
				display: true,
				label:'GAIN&nbsp;BAND&nbsp;CORRECTION&nbsp;',
				type: 'input',
				inputtype: 'text',
				onkey: isKeyDecimalNumber,
				show: this.shGainBandCorr,
				read: this.rdGainBandCorr,
				len: 4
			},
			{
				id:'powerBandCorrection',
				display: true,
				label:'POWER&nbsp;BAND&nbsp;CORRECTION&nbsp;',
				type: 'input',
				inputtype: 'text',
				onkey: isKeyDecimalNumber,
				show: this.shPowerBandCorr,
				read: this.rdPowerBandCorr,
				len: 4
			},	
			{
				id:'attout',
	...
	this.crtElement = function(ent, b) {
		...
		if (type == 'input') {
			...
			if (typeof(ent.onclickev) != 'undefined') {
				el.onclick = function(ev) { return ent.onclickev(ev); }
			}
	...
	this.optionsDisable = function(fact) {
