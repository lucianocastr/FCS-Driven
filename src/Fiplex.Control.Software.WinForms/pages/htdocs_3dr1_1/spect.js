...
function Spectrum() {
	this.parse = function(sr) {
		...
		if (this.chainChanged || this.bandSelectorChanged) {
			this.inputSpectrum = [];
			this.outputSpectrum = [];
		}
	...
	this.createChainForm = function() {
		...
		for (var i = 0; i < 2; ++i) {
			...
			el[i].onclick = function(ev) {
				...
				//Se fuerza UL/DL para que sólo pueda ser ULin/DLout
				document.getElementById("ioSel_"+this.elChain).checked = this.checked;
				document.getElementById("ioSel_"+c).checked = !this.checked;
	...
	this.createIoForm = function(){
		...
		for (var i = 0; i < 2; ++i) {
			...
			el[i].id = "ioSel_"+i;
			el[i].name = el.id;
			el[i].checked = i == this.chain; //se inicializa igual que chain
			el[i].side = i;
			el[i].onclick = function(ev) {
				try {
					...
					//Se fuerza UL/DL para que sólo pueda ser ULin/DLout
					document.getElementById("sigChain_"+this.side).checked = this.checked;
					document.getElementById("sigChain_"+c).checked = !this.checked;
					document.getElementById("sigChain_"+c).onclick(ev);
				} catch(err) {}
			}
			t.push(document.createElement("span"));
