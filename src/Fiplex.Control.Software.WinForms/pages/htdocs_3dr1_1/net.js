...
function submitform(isReset, isolVerif, isolClear, band, forceSend, forcePaOn, forcePaOff, setDefaultRelay, forcePlaMeas) {
	var doReset = false;
	...
	var doPlaMeas = false;
	if (typeof(forcePlaMeas) !== "undefined") {
		doPlaMeas = forcePlaMeas || false;
	}	
	...
	if (!checkChange() && !doReset && !doIsolVerif && !doIsolClear && !doForceSend && !doSetDefaultRelay && !doPlaMeas)	{
		return;
	}
	...
	var frmcnf = page.readConfsFrm(doReset, doIsolVerif, doIsolClear, band, doForceSend, doForcePaOn, doForcePaOff, doSetDefaultRelay, doPlaMeas);
	if (frmcnf.length == 0) {
		waitACK = false;
...
function submitConf(cnf){
	...
	var frms = [];
	frms.push({type: 'ctl_conf_str=', frame: cnf.getFrm()});
	submitFrms(frms);
}
