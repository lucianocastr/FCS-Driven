 function Factory(factoryFrame) {
	...
	this.externalPA				= false; //true-master OTA, false-master rack mount
	...
	this.parse = function(s) {
		...
		for (b=0;b<2;b++){
			...
			if (b==0) {
				...
				temp= parseInt(s.substring(ind,ind+2), 16); ind+=2;
				this.externalPA = (temp & 0x2)!=0;	//bit 0 is reserved for Master OTA
			}
			if (b==1) {
	...
	this.getFrm = function(){
		...
		for (var b=0;b<2;b++){
			...
			if (b==0) {
				...
				cal+=hexformat(mask,2);	
				mask = 0;
				if (this.externalPA) mask=0x2; //bit 0 is reserved for Master OTA
				cal+=hexformat(mask,2);
			}
			if (b==1) {
