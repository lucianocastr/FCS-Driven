...
function Config(maxChNr) {
	...
	this.format = function(currentConfigFrm) {
		...
		num = 0; //sca_m se fuerza mute mode not linked
	...
	this.formatBandCtrl = function(b) {
		...
		var hpaon = hpaIsOn(b); 
		if (b==0) hpaon = 0; //UL siempre a OFF
		num |= hpaon ? this.bandCtrlBits['HPAEN'] : 0;
	...
	this.formatChCtrl = function(b, c) {
		var k = c + 1;
		var en = factory.data.scaTest?(k>2?false:true):(k>1?false:true); //sca_m se fuerza sólo canal 1 activo o solo canal 1/2 en función de scaTest
		var bw = chBwGet(k, b);
		if (en == null || bw == null) {
			en = false;
			bw = 0;
		}
		var num = finePowerLimGet(k, b);
		if (num == null) {
			num = 0;
		} else if (num > 0) {
			num = 0;
		...
		num = 0; //sca_m se fuerza finepower a 0
	...
	this.formatFineGain = function(b, c) {
		...
		num = rSignedByte(num);
		var frame = "00"; //sca_m se fuerza fine gain a 0
		return frame;
	}
	...
	this.serializeCh = function(b, c) {
		...
		num = ~~Math.abs(num) & 0x0F;
		num <<= 4;
		num |= this.conf.band[b].ch[c].enable ? 0 : 0x08;
		num |= this.conf.band[b].ch[c].bandwidth & 0x07;
		frm += hexformat(num, 2);
