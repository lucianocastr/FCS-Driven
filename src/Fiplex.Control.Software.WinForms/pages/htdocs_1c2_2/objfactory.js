...
function Factory() {
	...
	this.parse = function(sr) {
		...
		for (var i = 0; i < 2; ++i) {
			...
			this.data.band[i].hpamon = (k & 0x18) >> 3;
		}
		var n = this.factbandmapLen;
		this.data.fmodulo = 	this.frameMap[n++].data[0];
		this.data.fstep = 	this.frameMap[n++].data[0];
		var aux = this.frameMap[n++].data[0];
		this.data.fastAgc = (aux & this.scaConfBits.FASTAGCBIT) != 0;
		this.data.scaBand = (aux & this.scaConfBits.SCABANDBIT) != 0;
		this.data.scaTest = (aux & this.scaConfBits.SCATESTBIT) != 0;
		this.data.scaTestNFPA = (aux & this.scaConfBits.SCATESTNFPABIT) != 0;
	...
	this.bwOptions = ["90 KHz", "45 KHz", "30 KHz", "20 KHz", "15 KHz", "25 KHz Hi Sel", "12.5 KHz Hi Sel", "6.25 KHz Hi Sel"];
	this.hpamonOptions = ["PA", "FPGA", "FIP413"];
	this.bandOptions = ["NO", "YES"];
	...
	this.createForm = function() {
		...
		for (var i = this.factbandmapLen; i < this.frameMap.length; ++i) {
			...
			if (this.frameMap[i].id == 'fastAgc') {
				...
				cell = document.createElement("th");
				row.appendChild(cell);
				cell.innerHTML = "RX&nbsp;LOW&nbsp;POWER&nbsp;ALARM";
				cell = document.createElement("td");
				row.appendChild(cell);
				el = document.createElement("select"); 
				el.className = "centered";
				for (var k = 0; k < this.bandOptions.length; k++) {
					var opt = document.createElement("option");
					el.options.add(opt);
					opt.text = this.bandOptions[k];
					opt.value = k;
				}
				el.selectedIndex = 0;
				el.id = "scaBand";
				...
				row.appendChild(cell);
				var txt = document.createTextNode("SCA test IP3");
				...
				cell.style.textAlign = "center";
				row.appendChild(cell);
				txt = document.createTextNode("SCA NFPA test");
				cell.appendChild(txt);
				el = document.createElement("input"); 
				el.type = "checkbox";
				el.id = "scaTestNFPA";
	...
	this.render = function() {
		...
		try {
			...
			for (var i = 0; i < 2; ++i) {
				...
				el.selectedIndex = this.data.band[i].hpamon;				
			}
			...
			el = document.getElementById("scaBand");
			el.selectedIndex = this.data.scaBand;
			el = document.getElementById("scaTest");
			el.checked = this.data.scaTest;
			el = document.getElementById("scaTestNFPA");
			el.checked = this.data.scaTestNFPA;			
		...
		try {
			...
			for (var n = 0; n < this.factbandmapLen; n++) {
				...
				for (var i = 0; i < 2; ++i) {
					...
					if (this.frameMap[n].id == 'power') 		num = parseFloat(el.value);
						...
						el = document.getElementById('paMonitorSel_'+i);
						if (el) num |= (el.selectedIndex & 0x03) << 3;
					}
			...
			for (var n = this.factbandmapLen; n < this.frameMap.length; n++) {
				...
				if (this.frameMap[n].id == 'fmodulo') 		num = parseInt(el.value);
					...
					num = (el.selectedIndex == 0 ? this.scaConfBits.FASTAGCBIT : 0);
					el = document.getElementById("scaBand");
					num |= (el.selectedIndex == 1 ? this.scaConfBits.SCABANDBIT: 0)
					el = document.getElementById("scaTest");
					num |= (el.checked ? this.scaConfBits.SCATESTBIT: 0);
					el = document.getElementById("scaTestNFPA");
					num |= (el.checked ? this.scaConfBits.SCATESTNFPABIT: 0)					
	...
	this.serialize = function(currentStr) {
		...
		for (var n = 0; n < this.factbandmapLen; n++) {
			...
			for (var i = 0; i < 2; ++i) {
				if (this.frameMap[n].id == 'power') 		num = this.data.band[i].powerCorr;
					...
					num |= (this.data.band[i].hpamon & 0x03) << 3;
				}
				str = isNaN(num) ? currentStr.substr(p, sz) : hexformat(this.frameMap[n].format(num), sz);
				p += sz;
				frame += str;
		...
		for (var n = this.factbandmapLen; n < this.frameMap.length; n++) {
			...
			if (this.frameMap[n].id == 'fmodulo') 		num = this.data.fmodulo;
				...
				num = this.data.fastAgc ? this.scaConfBits.FASTAGCBIT : 0;
				num |= this.data.scaBand ? this.scaConfBits.SCABANDBIT : 0;
				num |= this.data.scaTest ? this.scaConfBits.SCATESTBIT : 0;
				num |= this.data.scaTestNFPA ? this.scaConfBits.SCATESTNFPABIT : 0;
			}
