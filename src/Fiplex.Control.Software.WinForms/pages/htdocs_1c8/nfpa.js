...
var VSWRsettings = {min:   0, low_alarm: -128, low_warn:  -128, high_warn: 200, high_alarm: 200, max: 25 };
var BBLevelInsettings = {min:  -80, low_alarm: -121, low_warn: -121, high_warn: -20, high_alarm: -20, max: -20 };
var minRFPower = [15,5];
var minVSWRTxPower = [25,15];
...
function GUI_NFPA() {
	...
	this.showStatus = function(nfpa){
		for (var k=0;k<2;k++){
			var v = nfpa.powDirect[k];
			var vmin = minRFPower[factory.fstart[0]>600e6?0:1];
			if (v < vmin){
				setBarValue("rfPowFwd_"+k, v);
				setTxtValue("rfPowFwd_"+k, "---");
			}else{
				setMetValue("rfPowFwd_"+k, v);
			}
			var v = nfpa.powReverse[k];
			if (v < vmin){
				setBarValue("rfPowRev_"+k, v);
				setTxtValue("rfPowRev_"+k, "---");
			}else{
				setMetValue("rfPowRev_"+k, v);
			}
			var v = nfpa.retLoss[k];
			if (v > VSWRsettings.max){
				setBarValue("returnLoss_"+k, v);
				setTxtValue("returnLoss_"+k, "> "+VSWRsettings.max+".0");
			}else{
				setMetValue("returnLoss_"+k, v);
			}
	...
	this.show = function(nfpa){
		...
		for (var k=0;k<8;k++){
			...
			for (var j=0;j<4;j++) document.getElementById("relay_enabled_"+k+"_1_"+j).checked = nfpa.relayAssignBandAlarm[k][j];
		}
	...
	this.createRelayConfig = function(){
		...
		var rowb = document.createElement("tr");
		tab.appendChild(rowb);
		var cell = this.createGeneralRelayConfig();
		cell.className = "contentcell";		
		rowb.appendChild(cell);
		return tab;
	}
	this.createGeneralRelayConfig = function(){
		var cellb = document.createElement("td");
		cellb.className = "contentcell";
		var tbl = document.createElement("table");
		...
		var row = document.createElement("tr");tb.appendChild(row);
		this.createAlarmSemiRow(row,0,0,1,0,false); //hw fail
		this.createAlarmSemiRow(row,1,1,2,1,false); //overload
		
		var row = document.createElement("tr");tb.appendChild(row);
		this.createAlarmSemiRow(row,0,5,3,1,false); //rx power low 
		this.createAlarmSemiRow(row,1,3,4,1,false); //tx power low 
		
		var row = document.createElement("tr");tb.appendChild(row);
		this.createAlarmSemiRow(row,0,4,5,1,false); //vswr
		this.createAlarmSemiRow(row,1,5,6,0,false); //comm.error
		
		var row = document.createElement("tr");tb.appendChild(row);
		this.createAlarmSemiRow(row,0,6,7,1,false); //agc fail
		this.createAlarmSemiRow(row,1,1,8,0,false); //temp high
		
		var row = document.createElement("tr");tb.appendChild(row);
		this.createAlarmSemiRow(row,0,2,9,1,false); //pa fail
		this.createAlarmSemiRow(row,1,0,10,1,true); //ul overload (not visible)
		
		//rest of the unused alarms to avoid js errors
		var row = document.createElement("tr");tb.appendChild(row);row.style.display="none";
		this.createAlarmSemiRow(row,0,2,11,0,true); //ant isol
		this.createAlarmSemiRow(row,1,3,12,0,true); //osc det
		
		var row = document.createElement("tr");tb.appendChild(row);row.style.display="none";
		this.createAlarmSemiRow(row,0,4,13,0,true); //ul pa fail
		this.createAlarmSemiRow(row,1,7,14,1,true); //8th band alarm
		
		for (k=6;k<16;k++){//rest of unused general alarm
			if ((k%2)==0) row = document.createElement("tr");tb.appendChild(row);row.style.display="none";
			this.createAlarmSemiRow(row,k%2,k,k+9,0,true); 
		}
		return cellb;
	}
	this.createAlarmSemiRow = function(row, k, num, nAlarm, mode, hideCell){//k=0 first part of the row,k=1 second part
		var cell = document.createElement("th");
		cell.innerHTML = "ALARM&nbsp;"+nAlarm; 
		...
		row.appendChild(cell);
		var cell = this.createInputText("AlName",num,mode); 
		row.appendChild(cell);
		cell.className = "alarmTable";
		if (hideCell) {
			cell.style.display = "none";
		}
		var cell = this.createEnabledCheckBox(num,mode,0);
		row.appendChild(cell);
		cell.className = "alarmTable";
		...
		for (var i=0;i<4;i++){
			var cell = this.createCheckBox(num,mode,i); 
			row.appendChild(cell);
			cell.className = "alarmTable";
			if (hideCell) {
				cell.style.display = "none";
	...
	this.createVSWRMinPower = function(band) {
		...
		el.min = minVSWRTxPower[factory.fstart[0]>600e6?0:1];
		el.title = "Min: "+el.min+", Max: "+el.max+" dBm";
		el.band = band;
		el.onchange = function(ev) {
			var target = this;
	...
	function setBarValue(id, val, opt) {
		try {
			var met = document.getElementById("met_"+id).mMeter;
			if (met) {
				var color;
