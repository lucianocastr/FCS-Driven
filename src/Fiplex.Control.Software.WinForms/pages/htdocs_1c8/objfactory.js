 function Factory(factoryFrame) {
	...
	this.calUlPowerDetector			= new calDetectorObject(this.POWERTYPE);
	...
	this.secondCH					= false;
	this.uldl						= 0; //0 if only UL, 1 if only DL
	...
	this.parse = function(s) {
		...
		this.oscFeatureEnable = false; //forced SCA
		...
		for (i=0;i<2;i++){
			...
			this.Simplex[i] = false; //forced SCA
			this.agcModeUSA[i] = (temp & (1<<(i+4)))!=0;
		}
		this.extremeTempActionEnable = false; //forced SCA
		...
		for (b=0;b<2;b++){
			...
			for (i=0;i<2;i++){
				...
				if (i==0 && b==0){
					...
					this.ULlowGainMode = false; //forced SCA
				}
				temp= parseInt(s.substring(ind,ind+2), 16); ind+=2;
				if (i==0 && b==0){//SCA settings
					this.uldl = temp & 0x1;
					this.secondCH = (temp & 0x2)!=0;
				}
			}
		...
		//calibración nfpa
		for (var i=0;i<2;i++){
			this.calDlPowerDetector[i].parse(s.substr(ind,20)); ind += 20;
		}
		this.calUlPowerDetector.parse(s.substr(ind,20)); ind += 20;
		this.calPaCurrentDetector.parse(s.substr(ind,20)); ind += 20;
		this.calBoardCurrentDetector.parse(s.substr(ind,20)); ind += 20;
	...
	this.getFrm = function(){
		...
		//if (this.oscFeatureEnable) mask |= 0x10;//forced SCA
		...
		for (i=0;i<2;i++){
			...
			//if (this.Simplex[i]) mask |= (1<<(i+2));//forced SCA
			if (this.agcModeUSA[i]) mask |= (1<<(i+4));
		}
		//if ( this.extremeTempActionEnable ) mask |= 0x40;//forced SCA
		cal+=hexformat(mask,2);	
		...
		for (var b=0;b<2;b++){
			...
			for (i=0;i<2;i++){
				...
				if (i==0 && b==0) {
					...
					//if (this.ULlowGainMode) mask|=0x40;//forced SCA
				}
				cal+=hexformat(mask,2);
				mask = 0;
				if (i==0 && b==0) { //SCA settings
					mask = this.uldl;
					if (this.secondCH) mask|=0x2;
				}
				cal+=hexformat(mask,2);
			} 
		...
		for (var i=0;i<2;i++){
			cal += this.calDlPowerDetector[i].getFrm();
		}
		cal += this.calUlPowerDetector.getFrm();
...
function calDetectorObject(type){
	...
	this.parse = function(s) {
		...
		this.coeff = parseInt(s.substr(ind,4), 16); ind+=4;
	}
	this.getFrm = function(){
		var cal = "";
		var i,res,mask = 0;
