...
function Spectrum() {
	...
	this.createChannelBars = function(box) {
		...
		for (var c = 0; c < config.maxChannels; ++c) {
			if (!config.conf.ch[c].enable)
				continue;
			var fr;
			var frNr = config.conf.ch[c].frqNr;
	...
	this.createBandLimitsForm = function() {
		...
		for (var i = 0; i < 2; ++i) {
			...
			el.onchange = function(ev) {
				...
				localStorage.setItem("spect_fs_1dm4_"+self.chain+window.location.host, self.newFreqMinHz);
				localStorage.setItem("spect_fp_1dm4_"+self.chain+window.location.host, self.newFreqMaxHz);
			}
	...
	this.setBandLimits = function(fi, fp) {
		var fm = [];
		var b = this.chain;
		fm.push(factory.data.band[0].fStart / 1e6);
		fm.push(factory.data.band[0].fStop / 1e6);
		var tip = "Min: "+fm[0].toFixed(6)+"MHz, Max: "+fm[1].toFixed(6)+" MHz";
	...
	this.getBandLimits = function() {
		...
		var nfr = ~~Math.round((this.FreqMinHz - factory.data.band[0].fo) / this.FREQ_STEP_HZ);
		if (nfr < 0) {
			nfr += 0x10000;
		}
		var r = hexformat(nfr, 4);
		nfr = ~~Math.round((this.FreqMaxHz - factory.data.band[0].fo) / this.FREQ_STEP_HZ);
		if (nfr < 0) {
			nfr += 0x10000;
		}
		r += hexformat(nfr, 4);
	...
	this.createRbwForm = function() {
		...
		el.onchange = function(ev) {
			localStorage.setItem("spect_rbw_1dm4"+window.location.host, this.selectedIndex);
		}
		var t = document.createElement("span");
		t.innerHTML = str;
		t.style.fontWeight = "bold";
	...
	this.createAvgForm = function() {
		...
		el.onchange = function(ev) {
			...
			this.value = v;
			localStorage.setItem("spect_avg_1dm4"+window.location.host, v);
		}
		var t = document.createElement("span");
		t.innerHTML = "Avg.";
		t.style.fontWeight = "bold";
	...
	this.createChainForm = function() {
		...
		for (var i = 0; i < 2; ++i) {
			...
			el[i].onclick = function(ev) {
				...
				localStorage.setItem("spect_chain_1dm4_"+window.location.host, self.chain);
				self.initSettings();
		...
		for (var i = 0; i < 2; ++i) {
			...
			el[i].onclick = function(ev) {
				try {
					...
					localStorage.setItem("spect_io_1dm4_0"+window.location.host, io[0]? 1 : 0);
					localStorage.setItem("spect_io_1dm4_1"+window.location.host, io[1]? 1 : 0);
				} catch(err) {}
			}
			t.push(document.createElement("span"));
	...
	this.createZspanForm = function() {
		...
		el.onclick = function(ev) {
			localStorage.setItem("spect_zspan_1dm4_"+window.location.host, this.checked ? 1 : 0);
	...
	this.initSettings = function() {
		...
		var fStop = factory.data.band[0].fStop;
		this.io = 1;
		var b = parseInt(localStorage.getItem("spect_chain_1dm4_"+window.location.host));
		b = 1;
		if (isNaN(b) || b < 0 || b > 1) {
			b = 1;
			localStorage.setItem("spect_chain_1dm4_"+window.location.host, b);
		}
		this.chain = b;
		var fi = parseInt(localStorage.getItem("spect_fs_1dm4_"+b+window.location.host));
		var fp = parseInt(localStorage.getItem("spect_fp_1dm4_"+b+window.location.host));
		var span;
		if (!isNaN(fi) && !isNaN(fp) && fp >= fi
			...
			localStorage.setItem("spect_fs_1dm4_"+b+window.location.host, this.newFreqMinHz);
			localStorage.setItem("spect_fp_1dm4_"+b+window.location.host, this.newFreqMaxHz);
		}
		var zs = parseInt(localStorage.getItem("spect_zspan_1dm4_"+window.location.host));
		if (isNaN(zs) || zs < 0 || zs > 1) {
			zs = 0;
			localStorage.setItem("spect_zspan_1dm4_"+window.location.host, zs);
		}
		this.zSpanSt = zs != 0;
		var rbw = parseInt(localStorage.getItem("spect_rbw_1dm4"+window.location.host));
		if (isNaN(rbw) || rbw < 0 || rbw >= this.MAXRES) {
			rbw = this.MAXRES - 1;
			localStorage.setItem("spect_rbw_1dm4"+window.location.host, rbw);
		}
		this.resBw = rbw;
		this.spanStep = this.resBw;
		var v = parseInt(localStorage.getItem("spect_avg_1dm4"+window.location.host));
		...
		/*
		for (var i = 0; i < 2; ++i) {
			var io = parseInt(localStorage.getItem("spect_io_1dm4_"+i+window.location.host));
			if (isNaN(io) || (io != 0 && io != 1)) {
				io = 1;
				localStorage.setItem("spect_io_1dm4_"+i+window.location.host, io);
			}
			this.ioSet.push(io);
		}
		*/
