...
/update	U1{data}	0	1	Update configuration";
    ...
    private async Task<bool> SendProdConfigAsync(DeviceInfo device, short nchannels, short mode, bool clearROM)
    {
        ...
        try
        {
            ...
            // INIT-015 / VB6 1.12 parity (frmMainW.frm:3087-3089): 2c ndev=1 reads F1 for the MMS
            // flag (selects the J). NON-FATAL — on failure leave MMS=false and continue (VB6 does not abort).
            if (tdev == "2c" && ndev >= 1.0 && ndev < 2.0)
            {
                ProdLog("[F1] Sending F1 pre-read (2c ndev=1)...");
            ...
            var prodConfig = GetProductionConfig(tdev, ndev, nchannels, mode, clearROM, c1Response, f1Response);
    ...
    /// </remarks>
    private ProductionConfigData? GetProductionConfig(string tdev, double ndev, short nchannels, short mode, bool clearROM, string? c1Response = null, string? f1Response = null)
    {
